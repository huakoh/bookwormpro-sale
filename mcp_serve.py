"""
BookwormPRO MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (BookwormPRO-specific extra)

Usage:
    bookworm mcp serve
    bookworm mcp serve --verbose
...
logger = logging.getLogger("bookworm.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using BOOKWORMPRO_HOME."""
    try:
        from bwm_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("BOOKWORMPRO_HOME", Path.home() / ".bookwormpro")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("BOOKWORMPRO_HOME", Path.home() / ".bookwormpro")
...
class EventBridge:
    ...
    This is the BookwormPRO equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("BOOKWORMPRO_HOME", Path.home() / ".bookwormpro")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the BookwormPRO MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "bookworm",
        instructions=(
            "BookwormPRO messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the BookwormPRO MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

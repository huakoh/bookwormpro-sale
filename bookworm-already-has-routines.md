# BookwormPRO Has Had "Routines" Since March
...
BookwormPRO equivalent — works today:
```bash
...
BookwormPRO equivalent — works today:
```bash
bookworm webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working BookwormPRO implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | BookwormPRO |
|---|---|---|
| **Scheduled tasks** | [成功] Schedule-based | [成功] Any cron expression + human-readable intervals |
| **GitHub triggers** | [成功] PR, issue, push events | [成功] Any GitHub event via webhook subscriptions |
| **API triggers** | [成功] POST to unique endpoint | [成功] POST to webhook routes with HMAC auth |
| **MCP connectors** | [成功] Native connectors | [成功] Full MCP client support |
| **Script pre-processing** | [失败] | [成功] Python scripts run before agent, inject context |
| **Skill chaining** | [失败] | [成功] Load multiple skills per automation |
...
## Things BookwormPRO Does That Routines Can't
...
BookwormPRO has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
BookwormPRO is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
bookworm gateway setup    # enable webhooks
bookworm webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [bookwormpro.local/docs/guides/automation-templates](https://bookwormpro.local/docs/guides/automation-templates)

Documentation: [bookwormpro.local](https://bookwormpro.local)

GitHub: [github.com/huakoh/BookwormPRO](https://github.com/huakoh/BookwormPRO)

---

*BookwormPRO is built by [BookwormPRO Project](https://bookwormpro.local). Open source, model-agnostic, runs on your infrastructure.*

...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', () => {
    expect(DEFAULT_THEME.brand.name).toBe('BookwormPRO')
...
describe('detectLightMode', () => {
  ...
  it('honors BOOKWORMPRO_TUI_LIGHT on/off', () => {
    expect(detectLightMode({ BOOKWORMPRO_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ BOOKWORMPRO_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ BOOKWORMPRO_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ BOOKWORMPRO_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ BOOKWORMPRO_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets BOOKWORMPRO_TUI_LIGHT=0 override a light COLORFGBG', () => {
    expect(detectLightMode({ COLORFGBG: '0;15', BOOKWORMPRO_TUI_LIGHT: '0' })).toBe(false)
  })
})

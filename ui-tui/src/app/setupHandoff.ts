import type { RunExternalProcess } from '@bookworm/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`bookworm ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching bookworm: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`bookworm ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

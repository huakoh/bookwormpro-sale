...
def _clean_state():
    ...
    saved = {}
    for k in ("BOOKWORMPRO_INTERACTIVE", "BOOKWORMPRO_GATEWAY_SESSION", "BOOKWORMPRO_EXEC_ASK", "BOOKWORMPRO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["BOOKWORMPRO_INTERACTIVE"] = "1"
        session_key = os.getenv("BOOKWORMPRO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No BOOKWORMPRO_INTERACTIVE or BOOKWORMPRO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

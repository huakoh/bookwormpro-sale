...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "bookwormpro.local"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="bookwormpro-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="bookwormpro-token",
                        api_url="https://firecrawl-gateway.bookwormpro.local",
                    )
        ...
        with patch.dict(os.environ, {
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="bookwormpro-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="bookwormpro-token",
                        api_url="http://firecrawl-gateway.bookwormpro.local",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_hermes_home_override(self, tmp_path):
        """Auth lookup should read from BOOKWORMPRO_HOME/auth.json, not ~/.bookwormpro/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".bookwormpro").mkdir(parents=True)
        ...
        (hermes_home / "auth.json").write_text(json.dumps({
            "providers": {
                "bookwormpro": {
                    "access_token": "bookwormpro-token",
                }
            }
        }))
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "BOOKWORMPRO_HOME": str(hermes_home),
        }, clear=False):
            import tools.web_tools
            importlib.reload(tools.web_tools)
            assert tools.web_tools._read_nous_access_token() == "bookwormpro-token"
...
class TestBackendSelection:
    ...
    ``bookworm tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """

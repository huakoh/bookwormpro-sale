...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps BookwormPRO Portal billing
    ...
    def test_config_quality_setting_is_ignored(self, image_tool):
        ...
        with patch("bwm_cli.config.load_config",
...
class TestManagedGatewayErrorTranslation:
    """4xx from the BookwormPRO managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + bookworm tools."""

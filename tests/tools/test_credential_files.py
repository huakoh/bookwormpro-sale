...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.bookwormpro/token.json"
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.bookwormpro/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        ...
        with patch.dict(os.environ, {"BOOKWORMPRO_HOME": str(hermes_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.bookwormpro")

        assert mounts[0]["container_path"] == "/home/user/.bookwormpro/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.bookwormpro/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.bookwormpro/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape BOOKWORMPRO_HOME."""
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path / ".bookwormpro"))
        (tmp_path / ".bookwormpro").mkdir()
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside BOOKWORMPRO_HOME must still be registered."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of BOOKWORMPRO_HOME must be allowed."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside BOOKWORMPRO_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
        ...
        # The resolved path escapes BOOKWORMPRO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape BOOKWORMPRO_HOME."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside BOOKWORMPRO_HOME via config must still mount."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        assert "/root/.bookwormpro/cache/documents" in paths
        assert "/root/.bookwormpro/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.bookwormpro/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        assert "/root/.bookwormpro/cache/documents" in container_paths
        assert "/root/.bookwormpro/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        assert entries[0]["container_path"] == "/root/.bookwormpro/cache/screenshots/session_abc/screen1.png"

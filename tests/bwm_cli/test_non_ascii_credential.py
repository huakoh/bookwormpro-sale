...
from bwm_cli.config import _check_non_ascii_credential
...
class TestEnvLoaderSanitization:
    ...
    def test_strips_non_ascii_from_api_key(self, monkeypatch):
        from bwm_cli.env_loader import _sanitize_loaded_credentials, _WARNED_KEYS
    ...
    def test_ignores_non_credential_vars(self, monkeypatch):
        from bwm_cli.env_loader import _sanitize_loaded_credentials
    ...
    def test_ascii_control_chars_are_stripped(self, monkeypatch, capsys):
        """ASCII control bytes are stripped and reported, not passed through.

        This test previously asserted the opposite, on the reasoning that
        control bytes are "valid ASCII for HTTP headers even if the provider
        rejects them". That reasoning is wrong: RFC 9110 s5.5 forbids control
        characters in field values, so the request is malformed and the server
        answers 400 at the PARSE stage — it never reaches auth and never
        answers 401.

        Measured against a live relay on 2026-07-15 with a hand-written
        request (urllib refuses to send such a header client-side):

            Bearer '\\x16'                  -> HTTP/1.1 400 Bad Request
            well-formed but invalid token   -> HTTP/1.1 401 Unauthorized

        A 400 where 401 belongs is not a cosmetic difference: it moves the
        blame from the credential to the endpoint, which is exactly how one
        real incident lost a day. Stripping turns it back into a diagnosable
        401.
        """
        from bwm_cli.env_loader import _sanitize_loaded_credentials, _WARNED_KEYS
    ...
    def test_lone_ctrl_v_artifact_is_named(self, monkeypatch, capsys):
        """The real-world shape of the bug: a lone 0x16 and nothing else.

        Ctrl+V in a remote-desktop session without clipboard sync pastes
        nothing; the terminal takes the keystroke itself as text, and Ctrl+V
        is 0x16 (SYN). getpass() echoes nothing, so the field looks empty and
        the value ships silently. The warning must name the keystroke — "a
        control character" alone would not lead anyone to the clipboard.
        """
        from bwm_cli.env_loader import _sanitize_loaded_credentials, _WARNED_KEYS

        _WARNED_KEYS.clear()
        monkeypatch.setenv("ANTHROPIC_API_KEY", "\x16")
        _sanitize_loaded_credentials()
        assert os.environ["ANTHROPIC_API_KEY"] == ""
        err = capsys.readouterr().err
        assert "ANTHROPIC_API_KEY" in err
        assert "Ctrl+V" in err

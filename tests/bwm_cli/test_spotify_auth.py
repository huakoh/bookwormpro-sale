...
def test_store_provider_state_can_skip_active_provider() -> None:
    auth_store = {"active_provider": "bookwormpro", "providers": {}}
    ...
    assert auth_store["active_provider"] == "bookwormpro"
...
def test_resolve_spotify_runtime_credentials_refreshes_without_changing_active_provider(
    ...
    with auth_mod._auth_store_lock():
        ...
        store["active_provider"] = "bookwormpro"
        auth_mod._store_provider_state(
            store,
            "spotify",
            {
    ...
    assert auth_mod.get_active_provider() == "bookwormpro"
...
def test_auth_spotify_status_command_reports_logged_in(capsys, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from bwm_cli.auth_commands import auth_status_command
...
def test_spotify_interactive_setup_persists_client_id(
    ...
    """The wizard writes BOOKWORMPRO_SPOTIFY_CLIENT_ID to .env and returns the value."""
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path))
    ...
    assert "BOOKWORMPRO_SPOTIFY_CLIENT_ID=wizard-client-123" in env_text
    # Default redirect URI should NOT be persisted.
    assert "BOOKWORMPRO_SPOTIFY_REDIRECT_URI" not in env_text
...
def test_spotify_interactive_setup_empty_aborts(
    ...
    env_path = tmp_path / ".env"
    if env_path.exists():
        assert "BOOKWORMPRO_SPOTIFY_CLIENT_ID" not in env_path.read_text()

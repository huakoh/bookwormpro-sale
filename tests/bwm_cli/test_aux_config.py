"""Tests for the auxiliary-model configuration UI in ``bookworm model``.
...
from bwm_cli.config import DEFAULT_CONFIG, load_config
from bwm_cli.main import (
    _AUX_TASKS,
    _format_aux_current,
    _reset_aux_to_auto,
    _save_aux_choice,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"provider": "bookwormpro", "model": "gemini-3-flash"}, "bookwormpro · gemini-3-flash"),
        (
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".bookwormpro").mkdir(exist_ok=True)
...
def test_save_aux_choice_preserves_timeout(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="bookwormpro", model="gemini-3-flash")
...
def test_save_aux_choice_creates_missing_task_entry(tmp_path, monkeypatch):
    ...
    _save_aux_choice("vision", provider="bookwormpro", model="gemini-3-flash")
    cfg = load_config()
    assert cfg["auxiliary"]["vision"]["provider"] == "bookwormpro"
...
def test_reset_aux_to_auto_clears_routing_preserves_timeouts(tmp_path, monkeypatch):
    ...
    _save_aux_choice("compression", provider="bookwormpro", model="gemini-3-flash")
    from bwm_cli.config import save_config

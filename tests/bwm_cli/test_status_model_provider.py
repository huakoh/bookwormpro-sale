"""Tests for bwm_cli.status model/provider display."""
...
def test_show_status_displays_configured_dict_model_and_provider_label(monkeypatch, capsys, tmp_path):
    from bwm_cli import status as status_mod
...
def test_show_status_reports_managed_nous_features(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("bwm_cli.status.managed_nous_tools_enabled", lambda: True)
    from bwm_cli import status as status_mod
    ...
    monkeypatch.setattr(
        status_mod,
        "load_config",
        lambda: {"model": {"default": "claude-opus-4-6", "provider": "bookwormpro"}},
        raising=False,
    )
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "bookwormpro", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "bookwormpro", raising=False)
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "BookwormPRO Portal", raising=False)
    monkeypatch.setattr(
        status_mod,
    ...
    out = capsys.readouterr().out
    assert "BookwormPRO Tool Gateway" in out
    assert "Browser automation" in out
    assert "active via BookwormPRO subscription" in out


def test_show_status_hides_nous_subscription_section_when_feature_flag_is_off(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("bwm_cli.status.managed_nous_tools_enabled", lambda: False)
    from bwm_cli import status as status_mod
    ...
    out = capsys.readouterr().out
    assert "BookwormPRO Tool Gateway" not in out

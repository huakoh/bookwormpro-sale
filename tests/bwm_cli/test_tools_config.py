"""Tests for bwm_cli.tools_config platform tool persistence."""
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("bwm_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/huakoh/BookwormPRO/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (bookworm-cli, bookworm-telegram, etc.) must NOT
    ...
    ``bookworm tools``, but bookworm-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "bookworm-cli",  # <-- the culprit
    ...
    # bookworm-cli must NOT survive — it's a platform default, not an MCP server
    assert "bookworm-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_hermes_telegram():
    """Same bug for Telegram — bookworm-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "bookworm-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "bookworm-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "bookworm-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("bwm_cli.tools_config.managed_nous_tools_enabled", lambda: True)
    config = {"model": {"provider": "bookwormpro"}}

    monkeypatch.setattr(
        "bwm_cli.nous_subscription.get_nous_auth_status",
        lambda: {"logged_in": True},
    )

    providers = _visible_providers(TOOL_CATEGORIES["browser"], config)

    assert providers[0]["name"].startswith("BookwormPRO Subscription")


def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("bwm_cli.tools_config.managed_nous_tools_enabled", lambda: False)
    config = {"model": {"provider": "bookwormpro"}}
    ...
    assert all(not provider["name"].startswith("BookwormPRO Subscription") for provider in providers)
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("bwm_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "bwm_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("bwm_cli.tools_config.save_config", lambda config: None)
    ...
    monkeypatch.setattr(
        "bwm_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "bwm_cli.nous_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "bwm_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from bwm_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """bookworm-gateway includes list should cover all messaging platforms."""
        from bwm_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["bookworm-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from bwm_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from bwm_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/huakoh/BookwormPRO/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from bwm_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both BookwormPRO Subscription and FAL.ai providers must carry the
        imagegen_backend tag so _configure_provider fires the picker."""
        from bwm_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from bwm_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("bwm_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("bwm_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("bwm_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)

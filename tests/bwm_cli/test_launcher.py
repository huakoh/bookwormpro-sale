"""Tests for the top-level `./bookworm` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./bookworm` should use `bwm_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "bookworm"
    called = []

    fake_main_module = types.ModuleType("bwm_cli.main")

    def fake_main():
        called.append("bwm_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "bwm_cli.main", fake_main_module)

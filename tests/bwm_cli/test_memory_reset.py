"""Tests for the `bookworm memory reset` CLI command.
...
- Profile-scoped reset (uses BOOKWORMPRO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake BOOKWORMPRO_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.bookwormpro/bookwormpro\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `bookworm memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `bookworm memory reset` subcommand."""
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's BOOKWORMPRO_HOME."""
        ...
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(profile_home))

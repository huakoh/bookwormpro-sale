...
import bwm_cli.doctor as doctor
import bwm_cli.gateway as gateway_cli
from bwm_cli import doctor as doctor_mod
from bwm_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "BOOKWORMPRO_HOME", hermes_home)
    monkeypatch.delenv("BOOKWORMPRO_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("BOOKWORMPRO_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_hermes_home(self, tmp_path, provider=""):
        """Create a minimal BOOKWORMPRO_HOME with config.yaml."""

...
Ensures ``bookworm update`` correctly detects running gateways managed by
...
from bwm_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """bwm_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units bookworm-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="bookworm-gateway.service loaded active running BookwormPRO Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>BOOKWORMPRO_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: bookworm gateway run'."""
        ...
        assert "Restart manually: bookworm gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: bookworm gateway run" in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted bookworm-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="bookworm-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "bwm_cli.gateway._graceful_restart_via_sigusr1",
...
class TestServicePidExclusion:
    ...
    where ``bookworm update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.bookwormpro.gateway")
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "launchctl" in joined and "list" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="PID\tStatus\tLabel\n67890\t0\tai.bookwormpro.gateway\n",
                    stderr="",
                )
            return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="bookworm-gateway.service loaded inactive dead BookwormPRO Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.bookwormpro/bookwormpro/venv/bin/python -m bwm_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.bookwormpro/bookwormpro/venv/bin/python -m bwm_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``bookworm update --gateway``, the exit code marker must be
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy bookworm.service warning printed by `bookworm update`.

    Users who installed BookwormPRO before the service rename often have a
    dormant ``bookworm.service`` that starts flap-fighting the current
    ``bookworm-gateway.service`` after PR #5646. Every ``bookworm update``
    should remind them to run ``bookworm gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "bookworm.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy BookwormPRO gateway unit(s) detected" in captured
        assert "bookworm.service" in captured
        assert "bookworm gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy BookwormPRO gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (bookworm-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``bookworm.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "bookworm-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "bookworm-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy BookwormPRO gateway" not in captured
        assert "bookworm-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy BookwormPRO gateway" in captured

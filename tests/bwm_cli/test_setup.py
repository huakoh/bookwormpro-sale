...
from bwm_cli import setup as setup_mod
from bwm_cli.setup import setup_model_provider
...
def _stub_tts(monkeypatch):
    ...
    monkeypatch.setattr("bwm_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("bwm_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_delegates_to_select_provider_and_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("bwm_cli.main.select_provider_and_model", fake_select)
...
def test_setup_syncs_nous_from_disk(tmp_path, monkeypatch):
    """BookwormPRO OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
    ...
    def fake_select():
        _write_model_config(tmp_path, "bookwormpro", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "bookwormpro"
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import bwm_cli.gateway as gateway_mod
    ...
    assert "bookworm gateway" in out
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("bwm_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("bwm_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "bwm_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "bwm_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_nous_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("bwm_cli.setup.managed_nous_tools_enabled", lambda: True)
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("bwm_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("bwm_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("bwm_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "bwm_cli.setup.get_nous_subscription_features",
    ...
    from bwm_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("bwm_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("bwm_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("bwm_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "bwm_cli.setup.get_nous_subscription_features",
        lambda config: type("Features", (), {"nous_auth_present": True})(),
...
def test_resolve_hermes_chat_argv_prefers_which(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod.shutil, "which", lambda name: "/usr/local/bin/bookworm" if name == "bookworm" else None)

    assert setup_mod._resolve_hermes_chat_argv() == ["/usr/local/bin/bookworm", "chat"]
...
def test_resolve_hermes_chat_argv_falls_back_to_module(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod.importlib.util, "find_spec", lambda name: object() if name == "bwm_cli" else None)

    assert setup_mod._resolve_hermes_chat_argv() == [sys.executable, "-m", "bwm_cli.main", "chat"]
...
def test_offer_launch_chat_execs_fresh_process(monkeypatch):
    ...
    monkeypatch.setattr(setup_mod, "_resolve_hermes_chat_argv", lambda: ["/usr/local/bin/bookworm", "chat"])
    ...
    assert exec_calls == [("/usr/local/bin/bookworm", ["/usr/local/bin/bookworm", "chat"])]
...
def test_offer_launch_chat_manual_fallback_when_unresolvable(monkeypatch, capsys):
    ...
    assert "Run 'bookworm chat' manually" in captured.out

...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "bookworm" / "auth.json").read_text())
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path / "bookworm"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("bookwormpro@example.com")
    monkeypatch.setattr(
        "bwm_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["bookwormpro"]
    device_code_entries = [
    ...
    # `bookworm auth add bookwormpro` must also populate providers.bookwormpro so the
    ...
    # raises "BookwormPRO is not logged into BookwormPRO Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`bookworm auth add bookwormpro --type oauth --label <name>` must preserve the
    ...
    pool_entry = payload["credential_pool"]["bookwormpro"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-bookwormpro"

    # … and survives in providers.bookwormpro so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["bookwormpro"]["label"] == "my-bookwormpro"
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "bwm_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from bwm_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from bwm_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`bookworm logout --provider openai-codex` must still clear model.provider.
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `bookworm logout` should target configured Codex if auth has no active provider."""
    hermes_home = tmp_path / "bookworm"
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (hermes_home / "config.yaml").write_text(
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from bwm_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "bwm_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("bwm_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("bwm_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("bwm_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "bwm_cli.auth_commands.time.time",
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from bwm_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from bwm_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `bookworm auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path / "bookworm"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `bookworm auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("bwm_cli.auth._import_codex_cli_tokens", _fake_import)
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`bookworm auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.bookwormpro/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.bookwormpro/.env (not the shell), the
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `bookworm auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from bwm_cli.auth import is_source_suppressed
    from bwm_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `bookworm auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source BookwormPRO reads from has a
...
def test_seed_from_singletons_respects_nous_suppression(tmp_path, monkeypatch):
    """bookwormpro device_code must not re-seed from auth.json when suppressed."""
    hermes_home = tmp_path / "bookworm"
    hermes_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
    ...
    (hermes_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"bookwormpro": {"access_token": "tok", "refresh_token": "r", "expires_at": 9999999999}},
        "suppressed_sources": {"bookwormpro": ["device_code"]},
    }))

    from agent.credential_pool import _seed_from_singletons
    entries = []
    changed, active = _seed_from_singletons("bookwormpro", entries)
    assert changed is False
    assert entries == []
    assert active == set()
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import bwm_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hermes_pkce must not re-seed from ~/.bookwormpro/.anthropic_oauth.json when suppressed."""
    hermes_home = tmp_path / "bookworm"
    hermes_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
...
def test_credential_sources_registry_has_expected_steps():
    ...
    expected = {
        ...
        "~/.bookwormpro/.anthropic_oauth.json",
        "auth.json providers.bookwormpro",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `bookworm auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `bookworm auth add openai-codex`)

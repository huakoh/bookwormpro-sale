"""Tests for the BookwormPRO plugin system (bwm_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <BOOKWORMPRO_HOME>/config.yaml.
        # Config is always read from BOOKWORMPRO_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        hermes_home_str = os.environ.get("BOOKWORMPRO_HOME")
        if hermes_home_str:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.bookwormpro/plugins/ are discovered."""
        plugins_dir = tmp_path / "hermes_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path / "hermes_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("BOOKWORMPRO_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".bookwormpro" / "plugins"
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="bwm_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import bwm_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("bwm_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "hermes_test" / "plugins"
        # Set BOOKWORMPRO_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("bwm_cli.plugins.PluginContext.dispatch_tool.__module__", "bwm_cli.plugins"):

"""Tests for --ignore-user-config and --ignore-rules flags on `bookworm chat`.
...
files. In BookwormPRO the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.bookwormpro/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("BOOKWORMPRO_IGNORE_USER_CONFIG", "BOOKWORMPRO_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``BOOKWORMPRO_IGNORE_USER_CONFIG=1``.
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        """With BOOKWORMPRO_IGNORE_USER_CONFIG=1, user config.yaml is ignored.

        The built-in default ``model.default`` is empty string (no user override),
        and the user's ``agent.system_prompt`` is not seen.
        """
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("BOOKWORMPRO_IGNORE_USER_CONFIG", "1")
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("BOOKWORMPRO_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting BOOKWORMPRO_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        monkeypatch.setenv("BOOKWORMPRO_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("BOOKWORMPRO_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("BOOKWORMPRO_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.HermesCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``bwm_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("BOOKWORMPRO_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("BOOKWORMPRO_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("BOOKWORMPRO_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("BOOKWORMPRO_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("BOOKWORMPRO_IGNORE_USER_CONFIG") == "1"
        assert "BOOKWORMPRO_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "BOOKWORMPRO_IGNORE_USER_CONFIG" not in os.environ
        assert "BOOKWORMPRO_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real bwm_cli/main.py parser accepts both flags.

        We invoke the real argparse tree builder from bwm_cli.main.
        """
        import bwm_cli.main as hm
        ...
        assert "BOOKWORMPRO_IGNORE_USER_CONFIG" in src
        assert "BOOKWORMPRO_IGNORE_RULES" in src

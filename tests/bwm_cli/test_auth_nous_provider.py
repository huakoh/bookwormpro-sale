"""Regression tests for BookwormPRO OAuth refresh + agent-key mint interactions."""
...
from bwm_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from bwm_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("BOOKWORMPRO_CA_BUNDLE", raising=False)
    ...
    def test_missing_hermes_ca_bundle_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("BOOKWORMPRO_CA_BUNDLE", "/nonexistent/bookworm-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    auth_store = {
        "version": 1,
        "active_provider": "bookwormpro",
        "providers": {
            "bookwormpro": {
                "portal_base_url": "https://portal.example.com",
                "inference_base_url": "https://inference.example.com/v1",
                "client_id": "bookworm-cli",
                "token_type": "Bearer",
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_nous_auth_status() should find BookwormPRO credentials in the pool
    even when the auth store has no BookwormPRO provider entry — this is the
    ...
    hermes_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no BookwormPRO provider entry
    (hermes_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))

    # Seed the credential pool with a BookwormPRO entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("bookwormpro")
    entry = PooledCredential.from_dict("bookwormpro", {
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
    monkeypatch.setattr(
        "bwm_cli.auth.resolve_nous_runtime_credentials",
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    pool = load_pool("bookwormpro")
    stale = PooledCredential.from_dict("bookwormpro", {
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    def _boom(min_key_ttl_seconds=60):
        raise AuthError("Refresh session has been revoked", provider="bookwormpro", relogin_required=True)

    monkeypatch.setattr("bwm_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        ...
        if mint_calls["count"] == 1:
            raise AuthError("credits exhausted", provider="bookwormpro", code="insufficient_credits")
        return _mint_payload(api_key="agent-key-2")

    monkeypatch.setattr("bwm_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("bwm_cli.auth._mint_agent_key", _fake_mint_agent_key)

    with pytest.raises(AuthError) as exc:
        resolve_nous_runtime_credentials(min_key_ttl_seconds=300)
    assert exc.value.code == "insufficient_credits"

    state_after_failure = get_provider_auth_state("bookwormpro")
...
def test_mint_retry_uses_latest_rotated_refresh_token(tmp_path, monkeypatch):
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        mint_calls["count"] += 1
        if mint_calls["count"] == 1:
            raise AuthError("stale access token", provider="bookwormpro", code="invalid_token")
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `bookworm model` → BookwormPRO Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "bookwormpro" while
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import bwm_cli.models as models_mod
        import bwm_cli.nous_subscription as ns

        fake_auth_state = {
            "access_token": "fake-bookwormpro-token",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, BookwormPRO creds still saved."""
        import argparse
        import yaml
        from bwm_cli.auth import PROVIDER_REGISTRY, _login_nous
        ...
        _login_nous(args, PROVIDER_REGISTRY["bookwormpro"])
        ...
        # auth.json: active_provider restored to openrouter, but BookwormPRO creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "bookwormpro" in auth_after["providers"]
        assert auth_after["providers"]["bookwormpro"]["access_token"] == "fake-bookwormpro-token"
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a BookwormPRO model → provider flips to bookwormpro with that model."""
        import argparse
        import yaml
        from bwm_cli.auth import PROVIDER_REGISTRY, _login_nous
        ...
        cfg_after = yaml.safe_load(config_path.read_text())
        assert cfg_after["model"]["provider"] == "bookwormpro"
        assert cfg_after["model"]["default"] == "xiaomi/mimo-v2-pro"

        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "bookwormpro"
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as bookwormpro."""
        import argparse
        import yaml
        from bwm_cli.auth import PROVIDER_REGISTRY, _login_nous
        ...
        auth_path = hermes_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "bookwormpro" after Skip
        assert auth_after.get("active_provider") in (None, "")
        # But BookwormPRO creds are still saved
        assert "bookwormpro" in auth_after.get("providers", {})
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.bookwormpro AND providers.bookwormpro.

    Regression guard: before this helper existed, `bookworm auth add bookwormpro`
    wrote only the pool. After the BookwormPRO agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_nous_runtime_credentials
    which reads providers.bookwormpro, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from bwm_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
    ...
    assert entry.provider == "bookwormpro"
    assert entry.source == NOUS_DEVICE_CODE_SOURCE

    payload = json.loads((hermes_home / "auth.json").read_text())

    # providers.bookwormpro populated with the full state (new behaviour)
    singleton = payload["providers"]["bookwormpro"]
    ...
    # credential_pool.bookwormpro has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["bookwormpro"]
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "BookwormPRO is not logged into BookwormPRO Portal").

    This is the exact path that run_agent.py's `_try_refresh_nous_client_credentials`
    calls after a BookwormPRO 401 — before the fix it would raise AuthError because
    providers.bookwormpro was empty.
    """
    from bwm_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "BookwormPRO is not logged into BookwormPRO Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.bookwormpro and lets seeding
    ...
    # providers.bookwormpro reflects the latest write (singleton semantics)
    assert payload["providers"]["bookwormpro"]["access_token"] == "access-second"
    assert payload["providers"]["bookwormpro"]["agent_key"] == "agent-key-second"

    # credential_pool.bookwormpro has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["bookwormpro"]
    assert len(pool_entries) == 1, pool_entries
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    """User-supplied ``--label`` round-trips through providers.bookwormpro and the pool.

    Previously `bookworm auth add bookwormpro --type oauth --label <name>` silently
    ...
    fix stashes the label inside providers.bookwormpro so seeding prefers it.
    """
    from bwm_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
    ...
    # providers.bookwormpro carries the label so re-seeding on the next load_pool
    # doesn't overwrite it with the auto-derived fingerprint.
    payload = json.loads((hermes_home / "auth.json").read_text())
    assert payload["providers"]["bookwormpro"]["label"] == "my-personal"
...
def test_persist_nous_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    """
    from bwm_cli.auth import persist_nous_credentials
...
def test_persist_nous_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    # No "label" key embedded in providers.bookwormpro when the caller didn't supply one.
    payload = json.loads((hermes_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["bookwormpro"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the BookwormPRO Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", BookwormPRO must surface an
    ...
    re-authenticate" string led users to report this as a BookwormPRO persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from bwm_cli.auth import _refresh_access_token
    ...
    assert "bookworm auth add bookwormpro" in message.lower()

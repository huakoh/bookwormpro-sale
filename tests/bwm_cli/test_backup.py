"""Tests for bookworm backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.bookwormpro directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "bwm_state.db").write_bytes(b"fake-state")
    ...
    # bookwormpro repo (should be EXCLUDED)
    (root / "bookwormpro").mkdir(exist_ok=True)
    (root / "bookwormpro" / "run_agent.py").write_text("# big file\n")
    (root / "bookwormpro" / ".git").mkdir()
    (root / "bookwormpro" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_bookwormpro(self):
        from bwm_cli.backup import _should_exclude
        assert _should_exclude(Path("bookwormpro/run_agent.py"))
        assert _should_exclude(Path("bookwormpro/.git/HEAD"))
...
class TestBackup:
    ...
    def test_excludes_bookwormpro(self, tmp_path, monkeypatch):
        """Backup does NOT include bookwormpro/ directory."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        _make_hermes_tree(hermes_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "bookwormpro" in n]
            assert agent_files == [], f"bookwormpro files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/bookworm-backup-*.zip."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        ...
        zips = list(tmp_path.glob("bookworm-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid BookwormPRO backup."""
        from bwm_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only bwm_state.db (old wrong name) is rejected."""
        from bwm_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["bwm_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into bookworm home."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_hermes_prefix(self, tmp_path, monkeypatch):
        """Import strips .bookwormpro/ prefix if all entries share it."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".bookwormpro/config.yaml": "model: test\n",
            ".bookwormpro/skills/a/SKILL.md": "# A\n",
        })
    ...
    def test_rejects_non_hermes_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a bookworm backup."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside bookworm home
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".bookwormpro"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("BOOKWORMPRO_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from bwm_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".bookwormpro"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(dst_home))
        ...
        # bookwormpro should NOT be present
        assert not (dst_home / "bookwormpro").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from bwm_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without bookworm markers fails validation."""
        import io
        from bwm_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        """Detects .bookwormpro/ prefix wrapping all entries."""
        import io
        from bwm_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".bookwormpro/config.yaml", "test")
            zf.writestr(".bookwormpro/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".bookwormpro/"
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            ...
            zf.writestr(".bookwormpro/", "")
            zf.writestr(".bookwormpro/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ""
...
class TestBackupEdgeCases:
    def test_nonexistent_hermes_home(self, tmp_path, monkeypatch):
        """Backup exits when bookworm home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".bookwormpro"
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("bookworm-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_hermes_home(self, tmp_path, monkeypatch):
        """Backup handles empty bookworm home (no files to back up)."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
    ...
    def test_skips_output_zip_inside_hermes(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside bookworm root."""
        hermes_home = tmp_path / ".bookwormpro"
        hermes_home.mkdir()
        (hermes_home / "config.yaml").write_text("model: test\n")
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "bookworm -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import bwm_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "bwm_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from bwm_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def hermes_home(self, tmp_path):
        """Create a fake BOOKWORMPRO_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, hermes_home):
        from bwm_cli.backup import create_quick_snapshot
    ...
    def test_list_snapshots(self, hermes_home):
        from bwm_cli.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, hermes_home):
        from bwm_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from bwm_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from bwm_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from bwm_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)

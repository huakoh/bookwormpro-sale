"""Tests for bwm_cli/completion.py — shell completion script generation."""
...
from bwm_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real bookworm structure."""
    p = argparse.ArgumentParser(prog="bookworm")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "complete -F _hermes_completion bookworm" in out
...
class TestGenerateZsh:
    def test_contains_compdef_header(self):
        ...
        assert "#compdef bookworm" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c bookworm -f" in out
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        ...
        assert 'profiles_dir="$HOME/.bookwormpro/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'bookworm profile use', complete with profile names."""
        out = generate_bash(_make_parser())
    ...
    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_hermes_profiles()" in out
        assert "$HOME/.bookwormpro/profiles" in out

...
def container_env(tmp_path, monkeypatch):
    """Set up a fake BOOKWORMPRO_HOME with .container-mode file."""
    hermes_home = tmp_path / ".bookwormpro"
    hermes_home.mkdir()
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(hermes_home))
    monkeypatch.delenv("BOOKWORMPRO_DEV", raising=False)
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("bwm_constants.is_container", return_value=False):
...
def test_get_container_exec_info_none_inside_container(container_env):
    ...
    with patch("bwm_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_hermes_dev(container_env, monkeypatch):
    """Returns None when BOOKWORMPRO_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("BOOKWORMPRO_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hermes_dev_zero(container_env, monkeypatch):
    """BOOKWORMPRO_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("BOOKWORMPRO_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        hermes_home = Path(tmpdir) / ".bookwormpro"
        hermes_home.mkdir()
        ...
        with patch("bwm_constants.is_container", return_value=False), \
             patch("bwm_cli.config.get_hermes_home", return_value=hermes_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("BOOKWORMPRO_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=bookworm-custom\n"
        "exec_user=myuser\n"
        "hermes_bin=/opt/bookworm/bin/bookworm\n"
    )
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from bwm_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "bookworm"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "bookwormpro" in cmd
    assert "/data/current-package/bin/bookworm" in cmd
    assert "chat" in cmd

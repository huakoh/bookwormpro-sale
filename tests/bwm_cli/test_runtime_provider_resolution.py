from bwm_cli import runtime_provider as rp
...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "base_url": "",
            "api_key": "bookwormpro-runtime-key",
            "source": "portal",
            "expires_at": None,
        },
    )

    resolved = rp.resolve_runtime_provider(requested="bookwormpro")

    assert resolved["provider"] == "bookwormpro"
    assert resolved["base_url"] == ""
    assert resolved["api_key"] == "bookwormpro-runtime-key"
    assert resolved["requested_provider"] == "bookwormpro"
...
def test_resolve_requested_provider_precedence(monkeypatch):
    monkeypatch.setenv("BOOKWORMPRO_INFERENCE_PROVIDER", "bookwormpro")
    monkeypatch.setattr(rp, "_get_model_config", lambda: {"provider": "openai-codex"})
    assert rp.resolve_requested_provider("openrouter") == "openrouter"
    assert rp.resolve_requested_provider() == "openai-codex"

    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
    assert rp.resolve_requested_provider() == "bookwormpro"

    monkeypatch.delenv("BOOKWORMPRO_INFERENCE_PROVIDER", raising=False)
...
def test_resolve_provider_custom_returns_custom():
    ...
    from bwm_cli.auth import resolve_provider
...
def test_auto_detected_nous_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks BookwormPRO but credentials are revoked, fall through to OpenRouter."""
    from bwm_cli.auth import AuthError
    ...
    # resolve_provider returns "bookwormpro" (stale active_provider in auth.json)
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "bookwormpro")
    ...
    # BookwormPRO credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_nous_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="bookwormpro", code="invalid_grant", relogin_required=True)
        ),
    )
...
def test_explicit_nous_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests BookwormPRO and auth fails, the error should propagate."""
    from bwm_cli.auth import AuthError
    import pytest
    ...
    # With explicit "bookwormpro", should raise — don't silently switch providers
    with pytest.raises(AuthError, match="Refresh session has been revoked"):
        rp.resolve_runtime_provider(requested="bookwormpro")
...
def test_get_named_custom_provider_matches_grouped_picker_slug(monkeypatch):
    """Regression: the /model picker groups custom_providers by endpoint and
    strips the per-model suffix ("中转站 — claude-…" → "中转站") to build its slug
    ("custom:中转站"). The resolver must accept that same suffix-free group slug;
    otherwise switching TO a grouped provider raised "Unknown provider
    'custom:中转站'" — the picker emitted a slug it could not itself resolve.
    """
    monkeypatch.setattr(rp, "load_config", lambda: {
        "model": {"default": "claude-sonnet-4-6"},
        "custom_providers": [
            {
                "name": "中转站 — claude-sonnet-4-6",
    ...
    # Suffix-free group slug emitted by custom_provider_slug("中转站")
    result = rp._get_named_custom_provider("custom:中转站")
    assert result is not None
    assert result["base_url"] == "https://relay.example.com/v1"
    assert result["api_key"] == "test-key"
    # Grouped (provider_key / group-slug) match intentionally does NOT pin a
    # per-entry model — the model is deferred to model.default / target_model,
    # identical to how the provider_key slug ("custom:BWW") resolves.
    assert "model" not in result

    # A non-existent group must still fail closed (no over-matching)
    assert rp._get_named_custom_provider("custom:不存在") is None

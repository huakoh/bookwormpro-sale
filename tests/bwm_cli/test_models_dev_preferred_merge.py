...
These guard the contract:
  ...
  * OpenRouter and BookwormPRO Portal are NEVER merged — they keep their curated
    (OpenRouter) or live-Portal (BookwormPRO) semantics.
...
appear in ``/model`` without a BookwormPRO release.
"""
...
class TestProviderModelIdsPreferred:
    ...
    def test_opencode_go_offline_falls_back_to_curated(self):
        ...
        with patch("agent.models_dev.list_agentic_models", return_value=[]):
            out = provider_model_ids("opencode-go")
        # Curated floor (see bwm_cli/models.py _PROVIDER_MODELS["opencode-go"])
        assert "mimo-v2-pro" in out
        assert "kimi-k2.6" in out
...
class TestOpenRouterAndNousUnchanged:
    """Per Teknium: openrouter and bookwormpro are NEVER merged with models.dev."""

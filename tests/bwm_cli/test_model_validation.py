"""Tests for provider-aware `/model` validation in bwm_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("bwm_cli.models.fetch_api_models", return_value=api_models), \
         patch("bwm_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_provider_colon_model_switches_provider(self):
        provider, model = parse_model_input("openrouter:anthropic/claude-sonnet-4.5", "bookwormpro")
        assert provider == "openrouter"
        assert model == "anthropic/claude-sonnet-4.5"
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("bookwormpro:bookworm-3", "openrouter")
        assert provider == "bookwormpro"
        assert model == "bookworm-3"

    def test_empty_model_after_colon_keeps_current(self):
        provider, model = parse_model_input("openrouter:", "bookwormpro")
        assert provider == "bookwormpro"
...
class TestCuratedModelsForProvider:
    def test_openrouter_returns_curated_list(self):
        with patch(
            "bwm_cli.models.fetch_openrouter_models",
            return_value=[
...
class TestProviderModelIds:
    ...
    def test_stepfun_prefers_live_catalog(self):
        with patch(
            "bwm_cli.auth.resolve_api_key_provider_credentials",
            return_value={"api_key": "***", "base_url": "https://api.stepfun.com/step_plan/v1"},
        ), patch(
            "bwm_cli.models.fetch_api_models",
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("bwm_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("bwm_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_falls_back_to_curated_defaults_without_stale_opus(self):
        with patch("bwm_cli.models._resolve_copilot_catalog_api_key", return_value="gh-token"), \
             patch("bwm_cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("bwm_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("bwm_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("bwm_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("bwm_cli.models.urllib.request.urlopen", return_value=_Resp()):
...
class TestValidateApiFallback:
    ...
    def test_known_model_accepted_via_catalog_when_api_down(self):
        ...
        with patch(
            "bwm_cli.models.provider_model_ids",
    ...
    def test_unknown_provider_soft_accepted_when_api_down(self):
        ...
        with patch("bwm_cli.models.provider_model_ids", return_value=[]):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("bwm_cli.models.provider_model_ids", return_value=codex_models):
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a BookwormPRO User-Agent.
    ...
    def test_probe_sends_hermes_user_agent(self):
        ...
        with patch(
            "bwm_cli.models.urllib.request.urlopen",
        ...
        assert ua.startswith("bookworm-cli/"), (
            f"User-Agent must advertise bookworm-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("bookworm-cli/")
        # No Authorization was set, but UA must still be present.
        assert req.get_header("Authorization") is None


# -- named custom provider (custom:<label>) live probe -----------------------

def test_named_custom_provider_probes_live_models():
    """Regression: a NAMED custom provider ("custom:中转站") must enter the live
    /models probe branch. normalize_provider keeps the full slug (!= "custom"),
    so before the fix it skipped probing and always emitted a misleading
    "could not reach ... API" note. Endpoint reachable + model present →
    recognized with no warning.
    """
    result = _validate(
        "claude-opus-4-8",
        provider="custom:中转站",
        api_models=["claude-opus-4-8", "gpt-4o"],
        api_key="k",
...
def test_named_custom_provider_lenient_when_models_unreachable():
    """A named (user-configured) custom provider must still be accepted when its
    /models endpoint is unreachable — it is explicitly saved, so a probe miss
    ...
    assert result["accepted"] is True
    assert result["recognized"] is False
    assert result["message"]  # a warning note is present

"""Unit tests for bwm_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf bookworm-ok"])
        try:
            output = _read_until(bridge, b"bookworm-ok")
            assert b"bookworm-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$BOOKWORMPRO_PTY_TEST\""],
            env={**os.environ, "BOOKWORMPRO_PTY_TEST": "pty-env-works"},
        )
        try:

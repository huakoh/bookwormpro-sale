"""Tests for bwm_cli.auth._update_config_for_provider clearing stale fields.
...
from bwm_cli.auth import _update_config_for_provider
from bwm_cli.config import get_config_path
...
class TestUpdateConfigForProviderClearsStaleCustomFields:
    ...
    def test_switching_to_nous_clears_stale_api_mode(self):
        _seed_custom_provider_config()
        _update_config_for_provider("bookwormpro", "")
        model_cfg = _read_model_cfg()
        assert model_cfg.get("provider") == "bookwormpro"
        assert "api_mode" not in model_cfg
        assert "api_key" not in model_cfg

...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "bookworm" / "config.yaml"
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "bwm_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "bookworm" / "auth.json").read_text())
...
def test_load_pool_migrates_nous_provider_state_preserves_tls(tmp_path, monkeypatch):
        ...
        tmp_path,
        {
            ...
            "providers": {
                "bookwormpro": {
                    ...
                    "tls": {
                        "insecure": True,
                        "ca_bundle": "/tmp/bookwormpro-ca.pem",
                    },
                }
            },
        },
    ...
    auth_payload = json.loads((tmp_path / "bookworm" / "auth.json").read_text())
    assert auth_payload["credential_pool"]["bookwormpro"][0]["tls"] == {
        "insecure": True,
        "ca_bundle": "/tmp/bookwormpro-ca.pem",
    }
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("bwm_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path / "bookworm"))
    (tmp_path / "bookworm").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "bookworm" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "bwm_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "bwm_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_nous_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    ...
    the ``providers.bookwormpro`` singleton, it must carry the mint/refresh
        ...
        tmp_path,
        {
            ...
            "providers": {
                "bookwormpro": {
                    ...
                    "client_id": "bookworm-cli",
                    "portal_base_url": "",
                    "inference_base_url": "https://inference.bookwormpro.local/v1",
                    "token_type": "Bearer",
                    "scope": "openid profile",
                    "obtained_at": "2026-04-24T10:00:00+00:00",
                    "expires_at": "2026-04-24T11:00:00+00:00",
                    "expires_in": 3600,
                    "agent_key": "sk-bookwormpro-AAAA",
                    "agent_key_id": "ak_123",
    ...
    assert e.agent_key == "sk-bookwormpro-AAAA"
...
# ── PR #10160 salvage: BookwormPRO OAuth cross-process sync tests ─────────────────
...
def test_nous_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    """An exhausted BookwormPRO entry should recover when auth.json has newer tokens."""
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path / "bookworm"))

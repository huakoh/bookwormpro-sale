...
from bwm_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("bwm_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_hermes_branding(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="BookwormPRO v0.1.0 (test)"):
            banner = _build_compact_banner()

        assert "NOUS BOOKWORMPRO" in banner
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_hermes(self):
        ...
        assert "NOUS BOOKWORMPRO" not in banner
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="BookwormPRO v1.0 (test) · upstream abc12345"):
            banner = _build_compact_banner()

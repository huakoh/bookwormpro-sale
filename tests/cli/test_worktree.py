...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("bookwormpro/cli")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned bookworm/* and pr-* branches."""
    ...
    def test_prunes_orphaned_hermes_branch(self, git_repo):
        ...
        subprocess.run(
            ["git", "branch", "bookwormpro/cli", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "bookwormpro/cli"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "bookwormpro/cli" in result.stdout
        ...
        assert "bookwormpro/cli" in orphaned
        ...
        assert "bookwormpro/cli" not in result.stdout

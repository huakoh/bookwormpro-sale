...
def session_db(tmp_path):
    ...
    os.environ["BOOKWORMPRO_HOME"] = str(tmp_path / ".bookwormpro")
    os.makedirs(tmp_path / ".bookwormpro", exist_ok=True)
    from bwm_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".bookwormpro" / "test_sessions.db")
    yield db
    db.close()
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from bwm_cli.commands import COMMAND_REGISTRY

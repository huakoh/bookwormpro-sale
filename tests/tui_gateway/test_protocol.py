...
def server():
    with patch.dict("sys.modules", {
        "bwm_constants": MagicMock(get_hermes_home=MagicMock(return_value="/tmp/hermes_test")),
        "bwm_cli.env_loader": MagicMock(),
        "bwm_cli.banner": MagicMock(),
        "bwm_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/bookwormpro-dev": {"name": "bookwormpro-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "bookwormpro-dev", "session_id": sid},
        })
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "bookwormpro-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "bookwormpro-dev"

...
"""
    ...
    assert "Tyler Williams > Active Projects: BookwormPRO" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `bookworm cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.bookwormpro/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "BookwormPRO prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told BookwormPRO to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "BookwormPRO config is great"
    assert mod.rebrand_text("openclaw should always respond concisely") == "BookwormPRO should always respond concisely"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "BookwormPRO uses tools well"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "BookwormPRO remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "BookwormPRO prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "BookwormPRO was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "BookwormPRO uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "BookwormPRO said to ask BookwormPRO about BookwormPRO settings"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are BookwormPRO" in result

...
class TestHermesConstantsFallback:
    """Tests for _hermes_home.py fallback when bwm_constants is unavailable."""
    ...
    def _load_helper(self, monkeypatch):
        """Load _hermes_home.py with bwm_constants blocked."""
        monkeypatch.setitem(sys.modules, "bwm_constants", None)
    ...
    def test_fallback_uses_hermes_home_env_var(self, monkeypatch, tmp_path):
        """When bwm_constants is missing, BOOKWORMPRO_HOME comes from env var."""
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(tmp_path / "custom-bookworm"))
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == tmp_path / "custom-bookworm"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When bwm_constants is missing and BOOKWORMPRO_HOME unset, default to ~/.bookwormpro."""
        monkeypatch.delenv("BOOKWORMPRO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == Path.home() / ".bookwormpro"

    def test_fallback_ignores_empty_hermes_home(self, monkeypatch):
        """Empty/whitespace BOOKWORMPRO_HOME is treated as unset."""
        monkeypatch.setenv("BOOKWORMPRO_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == Path.home() / ".bookwormpro"
    ...
    def test_fallback_display_hermes_home_shortens_path(self, monkeypatch):
        ...
        monkeypatch.delenv("BOOKWORMPRO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.bookwormpro"
    ...
    def test_fallback_display_hermes_home_profile_path(self, monkeypatch):
        ...
        monkeypatch.setenv("BOOKWORMPRO_HOME", str(Path.home() / ".bookwormpro/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.bookwormpro/profiles/coder"
    ...
    def test_fallback_display_hermes_home_custom_path(self, monkeypatch):
        ...
        monkeypatch.setenv("BOOKWORMPRO_HOME", "/opt/bookworm-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "/opt/bookworm-custom"

    def test_delegates_to_hermes_constants_when_available(self):
        """When bwm_constants IS importable, _hermes_home delegates to it."""
        ...
        assert module.get_hermes_home is bwm_constants.get_hermes_home
        assert module.display_hermes_home is bwm_constants.display_hermes_home

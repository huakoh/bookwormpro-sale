...
- 96vc: ACP didn't set BOOKWORMPRO_EXEC_ASK, so `check_all_command_guards`
...
1. Setting BOOKWORMPRO_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set BOOKWORMPRO_INTERACTIVE so
    ...
    (BOOKWORMPRO_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When BOOKWORMPRO_INTERACTIVE is set and an approval callback is
        ...
        # Without BOOKWORMPRO_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without BOOKWORMPRO_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With BOOKWORMPRO_INTERACTIVE: callback IS called, approval flows through it
        monkeypatch.setenv("BOOKWORMPRO_INTERACTIVE", "1")
        called_with.clear()
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert called_with, (
            "with BOOKWORMPRO_INTERACTIVE the approval path should consult the "

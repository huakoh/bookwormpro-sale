"""Shared fixtures for the bookwormpro test suite.
...
2. **Isolated BOOKWORMPRO_HOME.** BOOKWORMPRO_HOME points to a per-test tempdir so
   code reading ``~/.bookwormpro/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".bookwormpro"`` instead of the canonical
   ``get_hermes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No BOOKWORMPRO_SESSION_* inheritance** — the agent's current gateway
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    "BOOKWORMPRO_YOLO_MODE",
    "BOOKWORMPRO_INTERACTIVE",
    "BOOKWORMPRO_QUIET",
    "BOOKWORMPRO_TOOL_PROGRESS",
    "BOOKWORMPRO_TOOL_PROGRESS_MODE",
    "BOOKWORMPRO_MAX_ITERATIONS",
    "BOOKWORMPRO_SESSION_PLATFORM",
    "BOOKWORMPRO_SESSION_CHAT_ID",
    "BOOKWORMPRO_SESSION_CHAT_NAME",
    "BOOKWORMPRO_SESSION_THREAD_ID",
    "BOOKWORMPRO_SESSION_SOURCE",
    "BOOKWORMPRO_SESSION_KEY",
    "BOOKWORMPRO_GATEWAY_SESSION",
    "BOOKWORMPRO_PLATFORM",
    "BOOKWORMPRO_INFERENCE_PROVIDER",
    "BOOKWORMPRO_MANAGED",
    "BOOKWORMPRO_DEV",
    "BOOKWORMPRO_CONTAINER",
    "BOOKWORMPRO_EPHEMERAL_SYSTEM_PROMPT",
    "BOOKWORMPRO_TIMEZONE",
    "BOOKWORMPRO_REDACT_SECRETS",
    "BOOKWORMPRO_BACKGROUND_NOTIFICATIONS",
    "BOOKWORMPRO_EXEC_ASK",
    "BOOKWORMPRO_HOME_MODE",
    "BROWSER_CDP_URL",
    "CAMOFOX_URL",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and BOOKWORMPRO_HOME to per-test tempdirs so code that
    reads ``~/.bookwormpro/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect BOOKWORMPRO_HOME to a per-test tempdir. Code that reads
    #    ``~/.bookwormpro/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.bookwormpro/*`` via
    #    ``Path.home() / ".bookwormpro"`` instead of ``get_hermes_home()``
    ...
    monkeypatch.setenv("BOOKWORMPRO_HOME", str(fake_hermes_home))
    ...
    #    ~/.bookwormpro/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import bwm_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal bookworm config dict suitable for unit tests."""
    return {

...
def main():
    ...
    if args.command == "add":
        ...
        if not issues:
            print(f"[成功] All {len(store.data['evidence'])} evidence entries passed SHA-256 integrity check.")
        else:
            print(f"[失败] {len(issues)} integrity issue(s) detected:")
            for i in issues:

...
## 5. BookwormPRO Configuration

### Config location

`$BOOKWORMPRO_HOME/config.yaml` (defaults to `~/.bookwormpro/config.yaml` when `BOOKWORMPRO_HOME` is unset)
...
Restart the BookwormPRO session for changes to take effect. The MCP connection is

...
OK="${GREEN}✔${NC}"; FAIL="${RED}✘${NC}"; WARN="${YELLOW}[警告]${NC}"

TWOZERO_URL="https://www.404zero.com/pisang/twozero.tox"
TOX_PATH="$HOME/Downloads/twozero.tox"
BOOKWORMPRO_HOME_DIR="${BOOKWORMPRO_HOME:-$HOME/.bookwormpro}"
BOOKWORMPRO_CFG="${BOOKWORMPRO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure BookwormPRO config has twozero_td MCP entry ──
if [[ ! -f "$BOOKWORMPRO_CFG" ]]; then
    echo -e " ${FAIL} BookwormPRO config not found at ${BOOKWORMPRO_CFG}"
    manual_steps+=("Create ${BOOKWORMPRO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$BOOKWORMPRO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in BookwormPRO config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to BookwormPRO config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$BOOKWORMPRO_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${BOOKWORMPRO_CFG} manually"); }
    manual_steps+=("Restart BookwormPRO session to pick up config change")
fi

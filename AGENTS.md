# BookwormPRO - Development Guide

Instructions for AI coding assistants and developers working on the bookwormpro codebase.
...
`$HOME/.bookwormpro/bookwormpro/venv` (for worktrees that share a venv with the
main checkout).
...
├── bwm_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
├── ui-tui/               # Ink (React) terminal UI — `bookworm --tui`
...
- **Slack** — `slack_subcommand_map()` generates `/bookworm` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `bwm_cli/commands.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `bookworm --tui` or `BOOKWORMPRO_TUI=1`.
...
npm run dev       # watch mode (rebuilds bookworm-ink + tsx --watch)
npm start         # production
npm run build     # full build (bookworm-ink + tsc)
...
### TUI in the Dashboard (`bookworm dashboard` → `/chat`)

The dashboard embeds the real `bookworm --tui` — **not** a rewrite.  See `bwm_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `bwm_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `bookworm --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `bookworm --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `bwm_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `bwm_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `bookworm tools`, `bookworm setup`, most CLI subcommands | `bwm_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`bwm_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
bwm_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.bookwormpro/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic BookwormPRO gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `bwm_cli/skin_engine.py`:
...
Users create `~/.bookwormpro/skins/<name>.yaml`:
...
BookwormPRO has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.bookwormpro/plugins/` and pip-installed entry points.

### General plugins (`bwm_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.bookwormpro/plugins/`, `./.bookwormpro/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `bookworm` at startup so
  `bookworm <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `bookworm <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `bookworm --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `bwm_cli/main.py`, etc.).
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `bookworm skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.bookwormpro.tags`, `metadata.bookwormpro.category`,
`metadata.bookwormpro.config` (config.yaml settings the skill needs — stored
...
BookwormPRO-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `BOOKWORMPRO_BACKGROUND_NOTIFICATIONS` env var):
...
BookwormPRO supports **profiles** — multiple fully isolated instances, each with its own
...
Existing call sites in `bwm_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `bwm_cli/curses_ui.py` — see
`bwm_cli/tools_config.py` for the canonical pattern.
...
| HOME / `~/.bookwormpro/` | Your real config+auth.json | Temp dir per test |

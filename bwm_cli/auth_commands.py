...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "bookwormpro", "openai-codex", "qwen-oauth", "google-gemini-cli"}
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "bookwormpro", "openai-codex", "qwen-oauth", "google-gemini-cli"} else AUTH_TYPE_API_KEY
    ...
    if provider == "bookwormpro":
        ...
        # Honor `--label <name>` so bookwormpro matches other providers' UX.  The
        # helper embeds this into providers.bookwormpro so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("bookwormpro").
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `bookworm auth
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `bookworm auth status spotify`.")

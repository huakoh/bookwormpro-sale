"""
Profile management for multiple isolated BookwormPRO instances.

Each profile is a fully independent BOOKWORMPRO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.bookwormpro/profiles/<name>/`` by default.

The "default" profile is ``~/.bookwormpro`` itself — backward compatible,
zero migration needed.

Usage::

    bookworm profile create coder          # fresh profile + bundled skills
    bookworm profile create coder --clone  # also copy config, .env, SOUL.md
    bookworm profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    bookworm -p coder chat                 # or via flag
    bookworm profile use coder             # set as sticky default
    bookworm profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See bwm_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.bookwormpro) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "bookwormpro",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "bookworm", "default", "test", "tmp", "root", "sudo",
})

# BookwormPRO subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the bookworm root, NOT to the current BOOKWORMPRO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where BOOKWORMPRO_HOME points outside
    ``~/.bookwormpro``, profiles live under ``BOOKWORMPRO_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) BOOKWORMPRO_HOME path.

    In standard deployments this is ``~/.bookwormpro``.
    In Docker/custom deployments where BOOKWORMPRO_HOME is outside ``~/.bookwormpro``
    (e.g. ``/opt/data``), returns BOOKWORMPRO_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.bookwormpro
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its BOOKWORMPRO_HOME directory."""
    if name == "default":
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, bookworm subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HERMES_SUBCOMMANDS:
        return f"'{name}' conflicts with a bookworm subcommand"
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.bookwormpro)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches BOOKWORMPRO_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "BOOKWORMPRO_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.bookwormpro).\n"
            "To remove everything, use: bookworm uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set BOOKWORMPRO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("BOOKWORMPRO_HOME")
    try:
        os.environ["BOOKWORMPRO_HOME"] = str(profile_dir)
        from bwm_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        elif "BOOKWORMPRO_HOME" in os.environ:
            del os.environ["BOOKWORMPRO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.bookwormpro/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: bookworm profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from BOOKWORMPRO_HOME.

    Returns ``"default"`` if BOOKWORMPRO_HOME is not set or points to ``~/.bookwormpro``.
    Returns the profile name if BOOKWORMPRO_HOME points into ``~/.bookwormpro/profiles/<name>``.
    Returns ``"custom"`` if BOOKWORMPRO_HOME is set to an unrecognized path.
    """
    from bwm_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.bookwormpro itself — its parent is ~/ and its
        # directory name is ".bookwormpro", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: bookworm profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.bookwormpro itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.bookwormpro). "
            "Specify a different name: bookworm profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for bookworm profile names."""
    return '''# BookwormPRO profile completion
# Add to ~/.bashrc: eval "$(bookworm completion bash)"

_hermes_profiles() {
    local profiles_dir="$HOME/.bookwormpro/profiles"
...
complete -F _hermes_completion bookworm
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for bookworm profile names."""
    return '''#compdef bookworm
# BookwormPRO profile completion
# Add to ~/.zshrc: eval "$(bookworm completion zsh)"

_hermes() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.bookwormpro/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.bookwormpro/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a BOOKWORMPRO_HOME path string.

    Called early in the CLI entry point, before any bookworm modules
    are imported, to set the BOOKWORMPRO_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: bookworm profile create {profile_name}"
        )

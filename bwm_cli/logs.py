"""``bookworm logs`` — view and filter BookwormPRO log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.bookwormpro/logs/``.

Usage examples::

    bookworm logs                    # last 50 lines of agent.log
    bookworm logs -f                 # follow agent.log in real time
    bookworm logs errors             # last 50 lines of errors.log
    bookworm logs gateway -n 100    # last 100 lines of gateway.log
    bookworm logs --level WARNING    # only WARNING+ lines
    bookworm logs --session abc123   # filter by session ID substring
    bookworm logs --component tools  # only tool-related lines
    bookworm logs --since 1h         # lines from the last hour
    bookworm logs --since 30m -f     # follow, starting 30 min ago
"""

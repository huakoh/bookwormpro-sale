"""
Gateway subcommand for bookworm CLI.

Handles: bookworm gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when bwm_constants is cached from a pre-update version during `bookworm update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "bookworm-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "bwm_cli.main gateway",
        "bwm_cli.main --profile",
        "bwm_cli.main -p",
        "bwm_cli/main.py gateway",
        "bwm_cli/main.py --profile",
        "bwm_cli/main.py -p",
        "bookworm gateway",
        "gateway/run.py",
    ]
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"BOOKWORMPRO_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "BOOKWORMPRO_HOME=" in command and f"BOOKWORMPRO_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``bookworm update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            BookwormPRO profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        f"[警告] {scope_label} service did not become active within {int(timeout)}s.\n"
        f"  Check status: {'sudo ' if system else ''}bookworm gateway status\n"
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (BOOKWORMPRO_HOME-scoped).
...
def _profile_suffix() -> str:
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary BOOKWORMPRO_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when BOOKWORMPRO_HOME is a named profile.

    For ``~/.bookwormpro/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit BOOKWORMPRO_HOME path. Defaults to the current
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this BOOKWORMPRO_HOME.

    Default ``~/.bookwormpro`` returns ``bookworm-gateway`` (backward compatible).
    Profile ``~/.bookwormpro/profiles/coder`` returns ``bookworm-gateway-coder``.
    Any other BOOKWORMPRO_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older BookwormPRO installs that predate the
# bookworm-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (bookworm-gateway-*.service) and unrelated third-party
# "bookworm" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("bookworm.service",)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy BookwormPRO gateway units.

    Detects unit files installed by older BookwormPRO versions that used a
    different service name (e.g. ``bookworm.service`` before the rename to
    ``bookworm-gateway.service``). When both a legacy unit and the current
    ``bookworm-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``bookworm-gateway-coder.service`` and unrelated third-party
      ``bookworm-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``bookworm.service`` running an unrelated
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy BookwormPRO gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy BookwormPRO gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy BookwormPRO gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current bookworm-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    bookworm gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo bookworm gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    sudo bookworm gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so BookwormPRO can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo bookworm gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo bookworm gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo bookworm gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode BookwormPRO often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.bookwormpro`` → ``ai.bookwormpro.gateway.plist`` (backward compatible).
    Profile ``~/.bookwormpro/profiles/coder`` → ``ai.bookwormpro.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.bookwormpro.gateway-{suffix}" if suffix else "ai.bookwormpro.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.bookwormpro/bookwormpro  -> /home/alice/.bookwormpro/bookwormpro
      /opt/bookworm                 -> /opt/bookworm  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current BOOKWORMPRO_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.bookwormpro                    → /home/alice/.bookwormpro
      /root/.bookwormpro/profiles/coder     → /home/alice/.bookwormpro/profiles/coder
      /opt/custom-bookworm               → /opt/custom-bookworm  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".bookwormpro").resolve()
    target_default = Path(target_home_dir) / ".bookwormpro"

    # Default ~/.bookwormpro → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.bookwormpro → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.bookwormpro) — keep as-is
...
User={username}
Group={group_name}
ExecStart={python_path} -m bwm_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="BOOKWORMPRO_HOME={hermes_home}"
Restart=on-failure
RestartSec=30
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (bookworm.service from pre-rename installs)
    # before installing the new bookworm-gateway.service. If both remain, they
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `bookworm gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.bookwormpro.gateway-{suffix}" if suffix else "ai.bookwormpro.gateway"
...
def launchd_stop():
    ...
    # `bookworm gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "   [警告] Without message.channels the bot will ONLY work in DMs!",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and BookwormPRO will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @bookworm:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. bookworm) and copy the bot token",
            "3. Works with any self-hosted Mattermost instance — enter your server URL",
            "4. To find your user ID: click your avatar (top-left) → Profile",
            "   Your user ID is displayed there — click it to copy.",
            "   [警告] This is NOT your username — it's a 26-character alphanumeric ID.",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where BookwormPRO delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your BookwormPRO agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address BookwormPRO will use (e.g., bookworm@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. BookwormPRO connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: bookworm pairing generate bluebubbles",
...
def _runtime_health_lines() -> list[str]:
    ...
    for platform, pdata in platforms.items():
        if pdata.get("state") == "fatal":
            ...
            lines.append(f"[警告] {platform}: {message}")

    if gateway_state == "startup_failed" and exit_reason:
        lines.append(f"[警告] Last startup issue: {exit_reason}")
    elif gateway_state == "draining":
        action = "restart" if restart_requested else "shutdown"
        count = int(active_agents or 0)
        lines.append(f"[等待] Gateway draining for {action} ({count} active agent(s))")
    elif gateway_state == "stopped" and exit_reason:
        lines.append(f"[警告] Last shutdown reason: {exit_reason}")
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'bookworm pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: bookworm pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'bookworm gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from bwm_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from bwm_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    print()
    print_success("[对话] WeCom configured!")
...
def _setup_weixin():
    ...
    print_info("  3. BookwormPRO will store the returned account_id/token in ~/.bookwormpro/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `bookworm gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `bookworm pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `bookworm pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"BookwormPRO\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: bookworm gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  bookworm gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: bookworm gateway run")
                print_info("  For persistence:   tmux new -s bookworm 'bookworm gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: bookworm gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup bookworm gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: bookworm gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'bookworm gateway setup' when ready.")
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info("  Consider running in foreground instead: bookworm gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s bookworm 'bookworm gateway run'")
                print()
        ...
        # pre-rename installs (e.g. bookworm.service). Profile units and
        # unrelated third-party services are never touched.

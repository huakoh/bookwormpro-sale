...
Add, remove, or reorder entries here — both `bookworm setup` and
`bookworm` provider-selection will pick up the change automatically.
"""
...
_HERMES_USER_AGENT = f"bookworm-cli/{_HERMES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `bookworm model`
    flow without maintaining a separate static list.
    """
    from bwm_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# ---------------------------------------------------------------------------
# BookwormPRO Portal free-model helper
# ---------------------------------------------------------------------------
# The BookwormPRO Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# BookwormPRO Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user's BookwormPRO Portal account/subscription info.
...
def partition_nous_models_by_tier(
    ...
    """Split BookwormPRO models into (selectable, unavailable) based on user tier.
...
def check_nous_free_tier() -> bool:
    """Check if the current BookwormPRO Portal user is on a free (unpaid) tier.
    ...
    try:
        from bwm_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# ---------------------------------------------------------------------------
# BookwormPRO Portal recommended models
#
...
NOUS_RECOMMENDED_MODELS_PATH = "/api/bookwormpro/recommended-models"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the BookwormPRO Portal's curated recommended-models payload.

    Hits ``<portal>/api/bookwormpro/recommended-models``. The endpoint is public —
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        state = get_provider_auth_state("bookwormpro") or {}
...
# this list:  bookworm model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `bookworm model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `bookworm model` TUI


CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("bookwormpro",           "BookwormPRO Portal",              "BookwormPRO Portal (BookwormPRO Project subscription)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``bookworm model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``bookworm auth add openai-codex`` without
    ``bookworm model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    bookwormpro is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (BookwormPRO Portal, private mirrors, older catalog snapshots) don't populate
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — bookwormpro
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from bwm_constants import AI_GATEWAY_BASE_URL
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, BookwormPRO Portal).
    """
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return bookworm-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; bookworm's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for BookwormPRO Portal pricing, or empty strings."""
    try:
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, bookwormpro, ai-gateway)."""
    ...
    if normalized == "bookwormpro":
        ...
        if base_url:
            # BookwormPRO base_url typically looks like 
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``bookworm model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from bwm_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        bookwormpro:bookworm-3                           →  ("bookwormpro", "bookworm-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, BookwormPRO, etc. all support /models)
...
def detect_provider_for_model(
    ...
    # If someone types `/model bookwormpro` or `/model anthropic`, treat it as a
    ...
    _AGGREGATORS = {"bookwormpro", "openrouter", "ai-gateway", "copilot", "kilocode"}
    ...
    if direct_match:
        ...
        if not has_creds:
            try:
                from bwm_cli.auth import _load_auth_store
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to BookwormPRO' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``bwm_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether BookwormPRO should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from bwm_cli.auth import resolve_api_key_provider_credentials
...
#   - "bookwormpro": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, BookwormPRO),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a BookwormPRO release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a BookwormPRO release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "bookwormpro":
        # Try live BookwormPRO Portal /models endpoint
        try:
            from bwm_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from bwm_cli.copilot_auth import copilot_request_headers
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: BookwormPRO' default Claude IDs elsewhere use
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        # Named custom providers (``custom:中转站``) reach here too — probe their
        # live /models endpoint instead of falling through to the generic catalog
        # path below, which never probes and emits a misleading "could not reach"
        # note even when the endpoint is healthy.
        # Try probing with correct auth for the api_mode.
        # Model switch is an explicit, user-initiated one-shot where correctness
        # beats speed, so allow more headroom than the 5s default: slow relays
        # (e.g. aggregating gateways behind a proxy) legitimately answer /models
        # in 6-8s, and a 5s cap turns a healthy endpoint into a false
        # "could not reach" warning.
        _switch_probe_timeout = 12.0
        if api_mode == "anthropic_messages":
            probe = probe_api_models(
                api_key, base_url, timeout=_switch_probe_timeout, api_mode=api_mode
            )
        else:
            probe = probe_api_models(
                api_key, base_url, timeout=_switch_probe_timeout
            )
        ...
        if api_models is not None:
            ...
            # 用户在 config 显式配置的 custom 模型，即使不在 /models 列表也应接受（正如 message 所述可能是隐藏/别名模型如 claude-fable-5）。
            # 警告但放行，与 message 文案(It may still work)一致。
            return {
                "accepted": True,
                "persist": True,
                "recognized": False,
        ...
        message = (
            ...
            f"BookwormPRO will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        return {
            # Named custom providers are explicitly user-configured; accept them
            # even when /models is unreachable (matches the pre-existing lenient
            # path) so a saved provider never fails the switch on a probe miss.
            "accepted": api_mode == "anthropic_messages" or normalized.startswith("custom:"),
            "persist": True,
            "recognized": False,
            "message": message,
        }
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so BookwormPRO cannot verify the model name."
    ...
    if api_models is not None:
        ...
        # 用户在 config 中显式配置的模型，即使不在 /models 目录也应接受。
        # 许多中转站/代理有影子模型（可调用但不在 /models 列表，如 claude-fable-5）。
        # 与上方注释(warn but allow)及 Anthropic 分支行为一致：警告但放行。
        return {
            "accepted": True,
            "persist": True,
            "recognized": False,
            "message": (
                f"Note: `{requested}` 不在此 endpoint 的 /models 列表中，"
                f"但已按配置接受（可能是未公开/影子模型）。"
                f"{suggestion_text}"
            ),
        }

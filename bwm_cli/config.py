...
Configuration management for BookwormPRO.



Config files are stored in ~/.bookwormpro/ for easy access:

- ~/.bookwormpro/config.yaml  - All settings (model, toolsets, terminal, etc.)
...
- bookworm config set      - Set a specific value

- bookworm config wizard   - Re-run setup wizard
...
from bwm_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("BOOKWORMPRO_MANAGED", "").strip()
...
def is_managed() -> bool:
    ...
    """Check if BookwormPRO is running in package-manager-managed mode.



    Two signals: the BOOKWORMPRO_MANAGED env var (set by the systemd service),

    or a .managed marker file in BOOKWORMPRO_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        ...
        return "brew upgrade bookwormpro"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "bookworm update"





def format_managed_message(action: str = "modify this BookwormPRO installation") -> str:

    """Build a user-facing error for managed installs."""

    managed_system = get_managed_system() or "a package manager"

    raw = os.getenv("BOOKWORMPRO_MANAGED", "").strip().lower()
...
def get_container_exec_info() -> Optional[dict]:
    ...
    """Read container mode metadata from BOOKWORMPRO_HOME/.container-mode.
    ...
    container, or BOOKWORMPRO_DEV=1 is set.
    ...
    if os.environ.get("BOOKWORMPRO_DEV") == "1":
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the bookworm group can

    share state with the gateway service.



    The mode can be overridden via the BOOKWORMPRO_HOME_MODE environment variable

    (e.g. BOOKWORMPRO_HOME_MODE=0701) for deployments where a web server (nginx,

    caddy, etc.) needs to traverse BOOKWORMPRO_HOME to reach a served subdirectory.
    ...
    try:
        ...
        mode_str = os.environ.get("BOOKWORMPRO_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When BookwormPRO runs in a container with volume-mounted config files, forcing
    ...
    if os.environ.get("BOOKWORMPRO_CONTAINER") or os.environ.get("BOOKWORMPRO_SKIP_CHMOD"):
...
def _secure_file(path):
    ...
    permissions.  Set BOOKWORMPRO_SKIP_CHMOD=1 to force-skip on other systems.
...
def _ensure_default_soul_md(home: Path) -> None:
    ...
    """Seed a default SOUL.md into BOOKWORMPRO_HOME if the user doesn't have one yet."""
...
def ensure_hermes_home():
    ...
    """Ensure ~/.bookwormpro directory structure exists with secure permissions.
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        ...
        raise RuntimeError(
            ...
            f"BOOKWORMPRO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the BookwormPRO-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, BookwormPRO auto-sources ``~/.profile``,
        ...
        # When true (default), BookwormPRO sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when BookwormPRO
    ...
    "browser": {
        ...
        "camofox": {
            ...
            # When true, BookwormPRO sends a stable profile-scoped userId to Camofox
    ...
    # single read_file page exceeds these limits, BookwormPRO truncates the
    ...
    "auxiliary": {
        ...
        "vision": {
            ...
            "provider": "auto",    # auto | openrouter | bookwormpro | codex | custom
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.bookwormpro/plugins/.
    ...
    # configured providers (OpenRouter, BookwormPRO, Z.ai, Kimi, etc.) are supported.
    ...
    # always goes to ~/.bookwormpro/skills/.

    "skills": {

        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]

        # Substitute ${BOOKWORMPRO_SKILL_DIR} and ${BOOKWORMPRO_SESSION_ID} in SKILL.md
    ...
    # This section is only needed for bookworm-specific overrides; everything else
    ...
    "discord": {
        ...
        "reactions": True,             # Add 👀/[成功]/[失败] reactions to messages during processing
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "[BWM] *BookwormPRO*" header.

        # Set to "" (empty string) to disable the header entirely.

        # Supports \n for newlines, e.g. "[模型] *My Bot*\n──────\n"
    ...
    # stored approval from ~/.bookwormpro/shell-hooks-allowlist.json.
    ...
    # toggleable per-invocation via --accept-hooks or BOOKWORMPRO_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via BOOKWORMPRO_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     bookwormpro's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.bookwormpro/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.bookwormpro/state.db.
    ...
    "sessions": {
        ...
        # default of ``bookworm sessions prune``.
...
# selection step (BookwormPRO Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        ...
        "description": "BookwormPRO Portal base URL override",

        "prompt": "BookwormPRO Portal base URL (leave empty for default)",
    ...
    "BOOKWORMPRO_GEMINI_CLIENT_SECRET": {
    ...
    "FIRECRAWL_GATEWAY_URL": {
        ...
        "description": "Exact Firecrawl tool-gateway origin override for BookwormPRO Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        ...
        "description": "Shared tool-gateway domain suffix for BookwormPRO Subscribers only, used to derive vendor hosts, e.g. bookwormpro.local -> firecrawl-gateway.bookwormpro.local",
    ...
    "TOOL_GATEWAY_SCHEME": {
        ...
        "description": "Shared tool-gateway URL scheme for BookwormPRO Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        ...
        "description": "Explicit BookwormPRO Subscriber access token for tool-gateway requests (optional; otherwise read from the BookwormPRO auth store)",
    ...
    "MATRIX_USER_ID": {
        ...
        "description": "Matrix user ID (e.g. @bookworm:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        ...
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. BOOKWORMPRO_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        ...
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'bookwormpro' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        ...
        "description": "URL of a remote BookwormPRO API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",

        "prompt": "Remote BookwormPRO API server URL (e.g. http://192.168.1.100:8642)",
    ...
    "GATEWAY_PROXY_KEY": {
        ...
        "description": "Bearer token for authenticating with the remote BookwormPRO API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # BOOKWORMPRO_TOOL_PROGRESS and BOOKWORMPRO_TOOL_PROGRESS_MODE are deprecated —

    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).

    # Gateway falls back to these env vars for backward compatibility.

    "BOOKWORMPRO_TOOL_PROGRESS": {
    ...
    "BOOKWORMPRO_TOOL_PROGRESS_MODE": {
    ...
    "BOOKWORMPRO_PREFILL_MESSAGES_FILE": {
    ...
    "BOOKWORMPRO_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.bookwormpro.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    # Use entry-level provider_key if present, else fall back to parameter
    entry_provider_key = str(entry.get("provider_key", "") or "").strip()
    effective_key = entry_provider_key or provider_key.strip()
    if effective_key:
        normalized["provider_key"] = effective_key
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older BookwormPRO versions) write ``models`` as
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        ...
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'bookworm setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        ...
        issues.append(ConfigIssue(
            ...
            "custom_providers defined but no 'model' section — BookwormPRO won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines = ["\033[33m[警告] Config issues detected in config.yaml:\033[0m"]

    for ci in issues:

        marker = "\033[31m[失败]\033[0m" if ci.severity == "error" else "\033[33m[警告]\033[0m"

        lines.append(f"  {marker} {ci.message}")

    lines.append("  \033[2mRun 'bookworm doctor' for fix suggestions.\033[0m")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if messaging_cwd:
        ...
        lines.append(
            ...
            f"  \033[33m[警告]\033[0m MESSAGING_CWD={messaging_cwd} found in .env — "
    ...
    if terminal_cwd_env and not config_has_explicit_cwd:
        ...
        lines.append(
            ...
            f"  \033[33m[警告]\033[0m TERMINAL_CWD={terminal_cwd_env} found in .env — "
    ...
    if lines:
        ...
        hint_path = os.environ.get("BOOKWORMPRO_HOME", "~/.bookwormpro")

        lines.insert(0, "\033[33m[警告] Deprecated .env settings detected:\033[0m")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            ...
            old_enabled = get_env_value("BOOKWORMPRO_TOOL_PROGRESS")

            old_mode = get_env_value("BOOKWORMPRO_TOOL_PROGRESS_MODE")

            if old_enabled and old_enabled.lower() in ("false", "0", "no"):

                display["tool_progress"] = "off"

                results["config_added"].append("display.tool_progress=off (from BOOKWORMPRO_TOOL_PROGRESS=false)")

            elif old_mode and old_mode.lower() in ("new", "all"):

                display["tool_progress"] = old_mode.lower()

                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from BOOKWORMPRO_TOOL_PROGRESS_MODE)")
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            ...
            old_tz = os.getenv("BOOKWORMPRO_TIMEZONE", "")

            if old_tz and old_tz.strip():

                config["timezone"] = old_tz.strip()

                results["config_added"].append(f"timezone={old_tz.strip()} (from BOOKWORMPRO_TIMEZONE)")
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$BOOKWORMPRO_HOME/plugins/`` for currently installed user plugins.
    ...
    # metadata.bookwormpro.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    ...
    """Read ~/.bookwormpro/config.yaml as-is, without merging defaults or migrating.
...
#   openai-codex (OAuth — bookworm auth) — OpenAI Codex

#   bookwormpro         (OAuth — bookworm auth) — BookwormPRO Portal
...
def save_config(config: Dict[str, Any]):
    ...
    """Save configuration to ~/.bookwormpro/config.yaml."""
...
def load_env() -> Dict[str, str]:
    ...
    """Load environment variables from ~/.bookwormpro/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real BookwormPRO env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    ...
    """Read, sanitize, and rewrite ~/.bookwormpro/.env in place.
...
# C0 (0x00-0x1F), DEL (0x7F) and C1 (0x80-0x9F) control characters are illegal
# in HTTP header field values (RFC 9110 s5.5). A credential carrying one makes
# the whole request malformed, so the server answers 400 at the parse stage and
# never reaches auth -> you get 400 where you would expect 401, which sends
# diagnosis chasing the endpoint instead of the key.
#
# The usual source is not a weird key: it is Ctrl+V pressed in a remote-desktop
# session with no clipboard sync. Nothing pastes; the terminal takes the
# keystroke itself as text. Ctrl+V is 0x16 (SYN). getpass() echoes nothing, so
# the field looks empty and the value ships silently.
_CONTROL_CHAR_RE = re.compile(r"[\x00-\x1f\x7f-\x9f]")


def describe_control_chars(value: str) -> list[str]:
    """Human-readable account of every control char in *value*."""
    out: list[str] = []
    for i, ch in enumerate(value):
        code = ord(ch)
        if not _CONTROL_CHAR_RE.match(ch):
            continue
        # 0x01-0x1A are exactly what Ctrl+A .. Ctrl+Z put on the wire.
        if 0x01 <= code <= 0x1A:
            hint = f" - this is the Ctrl+{chr(code + 64)} keystroke itself"
            if code == 0x16:
                hint += " (your paste never arrived: no clipboard sync?)"
        elif code == 0x1B:
            hint = " - ESC, usually a stray terminal escape sequence"
        else:
            hint = ""
        out.append(f"position {i}: 0x{code:02X}{hint}")
    return out


def strip_control_chars(value: str) -> str:
    """Remove every control character from *value*."""
    return _CONTROL_CHAR_RE.sub("", value)


def _emit_control_char_warning(key: str, described: list[str]) -> None:
    """Explain a control char in a credential, in terms of how it got there."""
    import sys as _sys
    lines = [
        "",
        f"  Warning: {key} contains control characters. They have been removed.",
        "",
        "  A control character makes the HTTP request malformed, so the server",
        "  replies 400 (bad request) instead of 401 (bad credential) - which",
        "  makes this look like an endpoint problem rather than a key problem.",
        "",
    ]
    lines += [f"    {d}" for d in described[:5]]
    if len(described) > 5:
        lines.append("    ... and more")
    lines += [
        "",
        "  Re-enter the key by typing it or via a file - not Ctrl+V over a",
        "  remote session, which is the usual way this happens.",
        "",
    ]
...
def _check_non_ascii_credential(key: str, value: str) -> str:
    ...
    try:
        ...
        # Valid ASCII is NOT sufficient: control bytes are ASCII too, and they
        # are what actually break the request. Check them before returning.
        _ctl = describe_control_chars(value)

        if _ctl:

            _emit_control_char_warning(key, _ctl)

            return strip_control_chars(value)

        return value  # all ASCII, no control chars — nothing to do
...
def save_env_value(key: str, value: str):
    ...
    """Save or update a value in ~/.bookwormpro/.env."""
...
def remove_env_value(key: str) -> bool:
    ...
    """Remove a key from ~/.bookwormpro/.env and os.environ.
...
def reload_env() -> int:
    ...
    """Re-read ~/.bookwormpro/.env into os.environ. Returns count of vars updated.



    Adds/updates vars that changed and removes vars that were deleted from

    the .env file (but only vars known to BookwormPRO — OPTIONAL_ENV_VARS and
    ...
    # Remove known BookwormPRO vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    ...
    """Get a value from ~/.bookwormpro/.env or environment."""

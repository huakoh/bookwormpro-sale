...
def run_doctor(args):
    ...
    print(color("│                 [体检] BookwormPRO Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'bookworm setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                issues.append("Run 'bookworm setup' to create .env")
            ...
            if provider and provider != "auto":
                if canonical_provider is None or (known_providers and canonical_provider not in known_providers):
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'bookworm config set model.provider <valid_provider>'"
                    )
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            ...
                            issues.append(
                                ...
                                f"Run 'bookworm setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'bookworm config set model.provider <name>'"
                            )
                except Exception:
                    pass
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'bookworm setup' to migrate config")
                else:
                    issues.append("Run 'bookworm doctor --fix' or 'bookworm setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'bookworm doctor --fix'")
        except Exception:
            pass
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give BookwormPRO a custom personality)")
        if should_fix:
        ...
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'bookworm doctor --fix' to checkpoint")
        ...
        if _venv_bin is None:
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/bookworm → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/bookworm points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/bookworm → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/bookworm — run 'bookworm doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/bookworm exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/bookworm not found",
                    "(bookworm command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/bookworm → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/bookworm symlink — run 'bookworm doctor --fix'")
    ...
    if openrouter_key:
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"\r  {color('[成功]', Colors.GREEN)} OpenRouter API                          ")
            elif response.status_code == 401:
                print(f"\r  {color('[失败]', Colors.RED)} OpenRouter API {color('(invalid API key)', Colors.DIM)}                ")
                issues.append("Check OPENROUTER_API_KEY in .env")
            elif response.status_code == 402:
                print(f"\r  {color('[失败]', Colors.RED)} OpenRouter API {color('(out of credits — payment required)', Colors.DIM)}")
                issues.append(
                ...
                print(f"\r  {color('[失败]', Colors.RED)} OpenRouter API {color('(rate limited)', Colors.DIM)}                ")
                issues.append("OpenRouter rate limit hit — consider switching to a different provider or waiting")
            else:
                print(f"\r  {color('[失败]', Colors.RED)} OpenRouter API {color(f'(HTTP {response.status_code})', Colors.DIM)}                ")
        except Exception as e:
            print(f"\r  {color('[失败]', Colors.RED)} OpenRouter API {color(f'({e})', Colors.DIM)}                ")
    ...
    if anthropic_key:
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"\r  {color('[成功]', Colors.GREEN)} Anthropic API                           ")
            elif response.status_code == 401:
                print(f"\r  {color('[失败]', Colors.RED)} Anthropic API {color('(invalid API key)', Colors.DIM)}                 ")
            else:
                msg = "(couldn't verify)"
                print(f"\r  {color('[警告]', Colors.YELLOW)} Anthropic API {color(msg, Colors.DIM)}                 ")
        except Exception as e:
            print(f"\r  {color('[警告]', Colors.YELLOW)} Anthropic API {color(f'({e})', Colors.DIM)}                 ")
    ...
    for _pname, _env_vars, _default_url, _base_env, _supports_health_check in _apikey_providers:
        ...
        if _key:
            ...
            if not _supports_health_check:
                print(f"  {color('[成功]', Colors.GREEN)} {_label} {color('(key configured)', Colors.DIM)}")
                continue
            ...
            try:
                ...
                if _resp.status_code == 200:
                    print(f"\r  {color('[成功]', Colors.GREEN)} {_label}                          ")
                elif _resp.status_code == 401:
                    print(f"\r  {color('[失败]', Colors.RED)} {_label} {color('(invalid API key)', Colors.DIM)}           ")
                    issues.append(f"Check {_env_vars[0]} in .env")
                else:
                    print(f"\r  {color('[警告]', Colors.YELLOW)} {_label} {color(f'(HTTP {_resp.status_code})', Colors.DIM)}           ")
            except Exception as _e:
                print(f"\r  {color('[警告]', Colors.YELLOW)} {_label} {color(f'({_e})', Colors.DIM)}           ")
    ...
    try:
        ...
        if has_aws_credentials():
            ...
            try:
                ...
                print(f"\r  {color('[成功]', Colors.GREEN)} {_label} {color(f'({_auth_var}, {_region}, {_model_count} models)', Colors.DIM)}           ")
            except ImportError:
                print(f"\r  {color('[警告]', Colors.YELLOW)} {_label} {color(f'(boto3 not installed — {sys.executable} -m pip install boto3)', Colors.DIM)}           ")
                issues.append(f"Install boto3 for Bedrock: {sys.executable} -m pip install boto3")
            except Exception as _e:
                _err_name = type(_e).__name__
                print(f"\r  {color('[警告]', Colors.YELLOW)} {_label} {color(f'({_err_name}: {_e})', Colors.DIM)}           ")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'bookworm setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                issues.append("No Honcho API key — run 'bookworm memory setup'")
            else:
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: bookworm memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: bookworm memory setup")
        except Exception as _e:
        ...
        if named_profiles:
            ...
            for p in named_profiles:
                ...
                if not (p.path / "config.yaml").exists():
                    parts.append("[警告] missing config")
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'bookworm doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
        print(color("  All checks passed! [完成]", Colors.GREEN, Colors.BOLD))

"""
MCP Server Management CLI — ``bookworm mcp`` subcommand.

Implements ``bookworm mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.bookwormpro/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from bwm_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  bookworm mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  bookworm mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  bookworm mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: bookworm mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `bookworm mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  bookworm mcp add <name> --url <endpoint>')
        _info('  bookworm mcp add <name> --command <cmd> --args <args...>')
        print()
        return
    ...
    for name, cfg in servers.items():
        ...
        status = color("[成功] enabled", Colors.GREEN) if enabled else color("[失败] disabled", Colors.DIM)
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `bookworm mcp remove` + `bookworm mcp add` to reconfigure auth.")
        return
...
def mcp_command(args):
    """Main dispatcher for ``bookworm mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("bookworm mcp add <name> --preset <preset>       Add from a known preset")
        _info("bookworm mcp remove <name>                      Remove a server")
        _info("bookworm mcp list                               List servers")
        _info("bookworm mcp test <name>                        Test connection")
        _info("bookworm mcp configure <name>                   Toggle tools")
        _info("bookworm mcp login <name>                       Re-authenticate OAuth")
        print()

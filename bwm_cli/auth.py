"""
Multi-provider authentication system for BookwormPRO.

Supports OAuth device code flows (BookwormPRO Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.bookwormpro/auth.json with cross-process file locking.
...
# BookwormPRO Portal defaults
DEFAULT_NOUS_PORTAL_URL = ""
DEFAULT_NOUS_INFERENCE_URL = ""
DEFAULT_NOUS_CLIENT_ID = "bookworm-cli"
...
SPOTIFY_DOCS_URL = "https://bookwormpro.local/docs/user-guide/features/spotify"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "bookwormpro": ProviderConfig(
        id="bookwormpro",
        name="BookwormPRO Portal",
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from bwm_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `bookworm model` to re-authenticate."

    if error.code == "subscription_required":
        return (
            "No active paid subscription found on BookwormPRO Portal. "
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits in BookwormPRO Portal, then retry."
        )
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("BOOKWORMPRO_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.bookwormpro/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hermes_home() / "auth.json"
    # Seat belt: if pytest is running and BOOKWORMPRO_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch BOOKWORMPRO_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".bookwormpro" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set BOOKWORMPRO_HOME to a tmp_path in your test fixture, or run "
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in BookwormPRO.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `bookworm logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'bookworm doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'bookworm model' for available providers, or run 'bookworm doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'bookworm model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.bookwormpro/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("BOOKWORMPRO_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.bookwormpro/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `bookworm auth list` / `bookworm status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.bookwormpro/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("BOOKWORMPRO_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set BOOKWORMPRO_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("BOOKWORMPRO_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("BOOKWORMPRO_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("BOOKWORMPRO_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `bookworm auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `bookworm auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `bookworm auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `bookworm auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.bookwormpro/.env, and return it.

    Raises SystemExit if the user aborts or submits an empty value.
    """
    from bwm_cli.config import save_env_value
    ...
    save_env_value("BOOKWORMPRO_SPOTIFY_CLIENT_ID", raw)
...
def login_spotify_command(args) -> None:
    ...
    # with "BOOKWORMPRO_SPOTIFY_CLIENT_ID is required".
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.bookwormpro/auth.json (not ~/.codex/)
#
# BookwormPRO maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from BookwormPRO auth store (~/.bookwormpro/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `bookworm auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `bookworm auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `bookworm auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `bookworm auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to BookwormPRO auth store (~/.bookwormpro/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating BookwormPRO auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `bookworm auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to BookwormPRO auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from BookwormPRO's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("BOOKWORMPRO_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other BookwormPRO processes
    ...
    base_url = (
        os.getenv("BOOKWORMPRO_CODEX_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("BOOKWORMPRO_CA_BUNDLE")
...
# =============================================================================
# BookwormPRO Portal — token refresh, agent key minting, model discovery
# =============================================================================
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="bookwormpro", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="bookwormpro", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the BookwormPRO portal
    ...
    # called POST /api/oauth/token with BookwormPRO's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, BookwormPRO's next refresh uses it, and the whole
    # session chain gets revoked as a token-theft signal (#15099).
    lowered = description.lower()
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "BookwormPRO Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another BookwormPRO install sharing "
            "~/.bookwormpro/auth.json) called POST /api/oauth/token with BookwormPRO's "
            "refresh token without persisting the rotated token back.\n"
            "BookwormPRO refresh tokens are single-use — only BookwormPRO may call the "
            "refresh endpoint. For health checks, use `bookworm auth status` "
            "instead.\n"
            "Re-authenticate with: bookworm auth add bookwormpro"
        )

    raise AuthError(description, provider="bookwormpro", code=code, relogin_required=relogin)
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "api_key" not in payload:
            raise AuthError("Mint response missing api_key",
                            provider="bookwormpro", code="server_error")
        return payload
    ...
    try:
        ...
        raise AuthError("Agent key mint request failed",
                        provider="bookwormpro", code="server_error") from exc
...
def fetch_nous_models(
    *,
    ...
    """Fetch available model IDs from the BookwormPRO inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="bookwormpro", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip BookwormPRO models — they're not reliable for agentic tool-calling
            if "bookworm" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware BookwormPRO Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        ...
        if not state:
            raise AuthError(
                "BookwormPRO is not logged into BookwormPRO Portal.",
        ...
        portal_base_url = (
            ...
            or os.getenv("BOOKWORMPRO_PORTAL_BASE_URL")
        ...
        if not isinstance(access_token, str) or not access_token:
            raise AuthError(
                "No access token found for BookwormPRO Portal login.",
                provider="bookwormpro",
                relogin_required=True,
            )
        ...
        _save_provider_state(auth_store, "bookwormpro", state)
        _save_auth_store(auth_store)
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    """Refresh BookwormPRO OAuth state without mutating auth.json."""
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    """Refresh BookwormPRO OAuth from a state dict. Thin wrapper around refresh_nous_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_nous_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "bookworm-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    """Persist minted BookwormPRO OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    BookwormPRO credentials are read at runtime from two independent locations:

    - ``providers.bookwormpro``: singleton state read by
      ``resolve_nous_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.bookwormpro``: used by the runtime ``pool.select()`` path.

    Historically ``bookworm auth add bookwormpro`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.bookwormpro``.  When the 24h agent_key TTL
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.bookwormpro`` then calls ``load_pool("bookwormpro")`` so
    ...
    ``bookworm auth add bookwormpro --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("bookwormpro")`` instead of the
...
def resolve_nous_runtime_credentials(
    *,
    ...
    """
    Resolve BookwormPRO inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("BookwormPRO is not logged into BookwormPRO Portal.",
                            provider="bookwormpro", relogin_required=True)
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for BookwormPRO Portal login.",
                                provider="bookwormpro", relogin_required=True)
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a BookwormPRO inference API key",
                        provider="bookwormpro", code="server_error")
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for BookwormPRO auth.
    ...
    if state:
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("bookwormpro") or state
            base_status.update(
                {
                    "logged_in": True,
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `bookworm auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `bookworm auth` and
    # `bookworm model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("BOOKWORMPRO_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("BOOKWORMPRO_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set BOOKWORMPRO_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `bookworm logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"bookwormpro", "openai-codex"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from bwm_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from bwm_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.bookwormpro/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing BookwormPRO-owned credentials
    ...
    # Save tokens to BookwormPRO auth store
...
def _nous_device_code_login(
    *,
    ...
    """Run the BookwormPRO device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["bookwormpro"]
    portal_base_url = (
        portal_base_url
        or os.getenv("BOOKWORMPRO_PORTAL_BASE_URL")
        or os.getenv("NOUS_PORTAL_BASE_URL")
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    """BookwormPRO Portal device authorization flow."""
    ...
    try:
        ...
        # overwrites it to "bookwormpro".  If the user picks "Skip (keep current)"
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "bookwormpro", auth_state)
            saved_to = _save_auth_store(auth_store)
        ...
        # them to BookwormPRO with a mismatched model.  The BookwormPRO OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "bookwormpro".  config.yaml model.provider is left
        ...
        config_path = _update_config_for_provider(
            "bookwormpro", inference_base_url, default_model=selected_model,
        )
        if selected_model:

"""bookworm claw — OpenClaw migration commands.

Usage:
    bookworm claw migrate              # Preview then migrate (always shows preview first)
    bookworm claw migrate --dry-run    # Preview only, no changes
    bookworm claw migrate --yes        # Skip confirmation prompt
    bookworm claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    bookworm claw cleanup              # Archive leftover OpenClaw directories
    bookworm claw cleanup --dry-run    # Preview what would be archived
"""
...
from bwm_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "BookwormPRO may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a BookwormPRO gateway is running with connected platforms.
    ...
    print()
    print_error(
        "BookwormPRO gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'bookworm stop'.")
    print()
...
def claw_command(args):
    """Route bookworm claw subcommands."""
...
def _cmd_migrate(args):
    """Run the OpenClaw → BookwormPRO migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: bookworm claw migrate --source /path/to/.openclaw")
        return
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: bookworm claw migrate --yes")
            return
    ...
    # 'bookworm claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: bookworm claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: bookworm claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  bookworm claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  bookworm config set OPENROUTER_API_KEY sk-or-v1-...")

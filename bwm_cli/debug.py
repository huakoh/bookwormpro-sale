"""``bookworm debug`` — debug tools for BookwormPRO.

Currently supports:
    bookworm debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    """Path to ``~/.bookwormpro/pastes/pending.json``.
    ...
    ran ``bookworm debug share`` repeatedly.  We now persist the schedule
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``bookworm debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``bookworm debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_PRIVACY_NOTICE = """\
[警告]  This will upload the following to a public paste service:
  • System info (OS, Python version, BookwormPRO version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    "[警告] **Privacy notice:** This uploads system info + recent log tails "
    "(may contain conversation fragments) to a public paste service. "
    "Full logs are NOT included from the gateway — use `bookworm debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "bookwormpro/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``bookworm debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.bookwormpro/pastes/pending.json``
    and rely on opportunistic sweeps (``_sweep_expired_pastes``) called from
    every ``bookworm debug`` invocation.  If the user never runs ``bookworm debug``
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"bookworm debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "bookwormpro/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----BookwormPRODebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from bwm_cli.logs import LOG_FILES
...
def _capture_dump() -> str:
    """Run ``bookworm dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``bookworm dump`` is run
        internally.
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``bookworm debug`` call.
    ...
    # best-effort — any failure is swallowed so ``bookworm debug`` stays

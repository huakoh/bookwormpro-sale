"""Shell completion script generation for bookworm CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# BookwormPRO bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef bookworm
# BookwormPRO zsh completion
# Add to ~/.zshrc:
...
{top_cmds_str}
            ...
            _describe 'bookworm command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# BookwormPRO fish completion",
        "# Add to your config:",
        "#   bookworm completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.bookwormpro/profiles",
        "        ls $HOME/.bookwormpro/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c bookworm -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c bookworm -f -s p -l profile"

"""
BookwormPRO — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m bwm_cli.main web          # Start on http://127.0.0.1:9119
    python -m bwm_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["BOOKWORMPRO_WEB_DIST"]) if "BOOKWORMPRO_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="BookwormPRO", version=__version__)
...
_SESSION_HEADER_NAME = "X-BookwormPRO-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``bookworm dashboard --tui``
# or BOOKWORMPRO_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.bookwormpro/logs/<action>.log`` so
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "bookworm-update": "bookworm-update.log",
}
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``bookworm <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``bwm_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "bwm_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "BOOKWORMPRO_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``bookworm gateway restart`` in the background."""
    try:
...
@app.post("/api/bookworm/update")
async def update_hermes():
    """Kick off ``bookworm update`` in the background."""
    try:
        proc = _spawn_hermes_action(["update"], "bookworm-update")
    except Exception as exc:
        _log.exception("Failed to spawn bookworm update")
        raise HTTPException(status_code=500, detail=f"Failed to start update: {exc}")
    return {
        "ok": True,
        "pid": proc.pid,
        "name": "bookworm-update",
    }
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    BookwormPRO supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# Anthropic, device-code for BookwormPRO/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``bookworm auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    BookwormPRO resolves Anthropic creds in this order at runtime:
    1. ``~/.bookwormpro/.anthropic_oauth.json`` — BookwormPRO-managed PKCE flow
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"BookwormPRO PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into BookwormPRO even
    when they also have a separate BookwormPRO-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "bookworm auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "bookworm auth add bookwormpro",
        "docs_url": "",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "bookworm auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "bookworm auth add qwen-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from bwm_cli import auth as hauth
        if provider_id == "bookwormpro":
            raw = hauth.get_nous_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "nous_portal",
                "source_label": raw.get("portal_base_url") or "BookwormPRO Portal",
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same BookwormPRO-managed PKCE file
    ...
    if provider_id in ("anthropic", "claude-code"):
        ...
        try:
            from bwm_cli.auth import clear_provider_auth
...
#          → persists to ~/.bookwormpro/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (BookwormPRO, OpenAI Codex):
#     1. POST /api/providers/oauth/{bookwormpro|openai-codex}/start
...
# Guarded so bookworm web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both BookwormPRO file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``bookworm auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "bookworm-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow (BookwormPRO or OpenAI Codex).
    ...
    from bwm_cli import auth as hauth
    if provider_id == "bookwormpro":
        from bwm_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["bookwormpro"]
        portal_base_url = (
            os.getenv("BOOKWORMPRO_PORTAL_BASE_URL")
        ...
        sid, sess = _new_oauth_session("bookwormpro", "device_code")
...
def _nous_poller(session_id: str) -> None:
    """Background poller that drives a BookwormPRO device-code flow to completion."""
    from bwm_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: bookwormpro login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("bookwormpro device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
async def get_logs(
    ...
    from bwm_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from bwm_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from bwm_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from bwm_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from bwm_cli.tools_config import (
...
#
# The endpoint spawns the same ``bookworm --tui`` binary the CLI uses, behind
...
def _resolve_chat_argv(
    ...
    Default: whatever ``bookworm --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``BOOKWORMPRO_TUI_RESUME`` env var —
    matching what ``bwm_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``BOOKWORMPRO_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from bwm_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if resume or sidecar_url:
        ...
        if resume:
            env["BOOKWORMPRO_TUI_RESUME"] = resume

        if sidecar_url:
            env["BOOKWORMPRO_TUI_SIDECAR_URL"] = sidecar_url
...
# BOOKWORMPRO_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "BookwormPRO Teal",  "description": "Classic dark teal — the canonical BookwormPRO look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.bookwormpro/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.bookwormpro/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.bookwormpro/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as bwm_cli.plugins):
    1. User plugins:    ~/.bookwormpro/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.bookwormpro/plugins/  (only if BOOKWORMPRO_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    if os.environ.get("BOOKWORMPRO_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".bookwormpro" / "plugins", "project"))

...
from bwm_cli import auth as auth_mod
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from bwm_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if configured_mode and _provider_supports_explicit_api_mode(provider, configured_provider):
            ...
            from bwm_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("BOOKWORMPRO_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'bookworm doctor' for details."
        )
        return None
    ...
    for entry in custom_providers:
        ...
        # The /model picker groups custom entries by endpoint and strips the
        # per-model suffix ("中转站 — claude-…" → "中转站") to build its slug via
        # custom_provider_slug() → "custom:中转站". The resolver must accept that same
        # suffix-free group slug; otherwise switching TO a grouped provider fails
        # with "Unknown provider 'custom:<group>'" — the picker would emit a slug it
        # cannot itself resolve. Mirror model_switch.py's suffix stripping.
        group_name = name
        for _sep in ("—", " - "):
            if _sep in group_name:
                group_name = group_name.split(_sep)[0].strip()
                break
        group_norm = _normalize_custom_provider_name(group_name) if group_name else ""
        group_menu_key = f"custom:{group_norm}" if group_norm else ""
        if requested_norm not in {
            name_norm, menu_key, provider_key_norm, provider_menu_key,
            group_norm, group_menu_key,
        }:
            continue
        result = {
            "name": name.strip(),
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "bookwormpro":
        state = auth_mod.get_provider_auth_state("bookwormpro") or {}
        base_url = (
            explicit_base_url
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # For BookwormPRO, the pool entry's runtime_api_key is the agent_key — a
        # short-lived inference credential (~30 min TTL).  The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `bookworm auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_nous_runtime_credentials() which handles refresh + mint.
        if provider == "bookwormpro" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("BOOKWORMPRO_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
            nous_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
            }
            if not _agent_key_is_usable(nous_state, min_ttl):
                logger.debug("BookwormPRO pool entry agent_key expired/missing, falling through to runtime resolution")
                pool_api_key = ""
    ...
    if provider == "bookwormpro":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected BookwormPRO but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected BookwormPRO provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "bookworm-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:

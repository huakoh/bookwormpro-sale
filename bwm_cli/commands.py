"""Slash command definitions and autocomplete for the BookwormPRO CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("memsearch", _("全文检索持久记忆(MEMORY.md/USER.md)"), _("Info"),
               aliases=("记忆搜索", "msearch"), args_hint="<关键词>"),
    CommandDef("learn", _("从对话或指定来源蒸馏出一个可复用技能并保存"), _("Session"),
               aliases=("学习",), args_hint="[来源描述]"),
    ...
    CommandDef("fallback", _("Manage fallback provider chain (list/add/remove/clear)"), _("Configuration"), args_hint="[list|add|remove|clear]"),
    ...
    CommandDef("metrics", _("Show Gateway metrics (latency, availability, circuit trips)"), _("Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`bwm_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/bookworm`` subcommand mapping, and
    ...
    """
    try:
        from bwm_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``bookworm skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /bookworm handler.

    Maps both canonical names and aliases so /bookworm bg do stuff works
    the same as /bookworm background do stuff.

    Plugin-registered slash commands are included so ``/bookworm <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _fuzzy_file_completions(self, word: str, query: str, limit: int = 20):
        ...
        for _unused, fp in scored[:limit]:
            is_dir = fp.endswith("/")
            filename = os.path.basename(fp)
            kind = "folder" if is_dir else "file"
            meta = "dir" if is_dir else _file_size_label(
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from bwm_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]

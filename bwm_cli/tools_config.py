"""
Unified tool configuration for BookwormPRO.

`bookworm tools` and `bookworm setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.bookwormpro/config.yaml under
...
from bwm_cli.nous_subscription import (
...
from bwm_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ("web",             "[查找] Web Search & Scraping",    "web_search, web_extract"),
    ("browser",         "[网页] Browser Automation",       "navigate, click, type, scroll"),
    ("terminal",        "[系统] Terminal & Processes",      "terminal, process"),
    ("file",            "📁 File Operations",           "read, write, patch, search"),
    ("code_execution",  "* Code Execution",            "execute_code"),
    ("vision",          "👁️  Vision / Image Analysis",  "vision_analyze"),
    ("image_gen",       "[样式] Image Generation",          "image_generate"),
    ("moa",             "🧠 Mixture of Agents",         "mixture_of_agents"),
    ("tts",             "🔊 Text-to-Speech",            "text_to_speech"),
    ("skills",          "📚 Skills",                    "list, view, manage"),
    ("todo",            "[汇总] Task Planning",             "todo"),
    ("memory",          "[保存] Memory",                    "persistent memory across sessions"),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from bwm_cli.plugins import discover_plugins, get_plugin_toolsets
...
TOOL_CATEGORIES = {
    ...
    "browser": {
        ...
        "providers": [
            {
                "name": "BookwormPRO Subscription (Browser Use cloud)",
                "badge": "subscription",
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/bookwormpro && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in bookwormpro directory)")
        ...
        # Run the full `bookworm auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.bookwormpro/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from bwm_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: bookworm auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `bookworm auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: bookworm auth spotify")
        except Exception as exc:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "bookworm-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "bookworm-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # shipped as a bundled plugin but user must opt in via `bookworm tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `bookworm tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by bookworm tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (bookworm-cli, bookworm-telegram, etc.)
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    # later) so the picker lists them alongside FAL / BookwormPRO Subscription.
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_nous_auth"):
        ...
        if not features.nous_auth_present:
            _print_warning("  BookwormPRO Subscription is only available after logging into BookwormPRO Portal.")
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your BookwormPRO subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `bookworm tools` and `bookworm setup tools`.

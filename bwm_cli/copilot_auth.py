...
Token type support (per GitHub docs):
  gho_          OAuth token           [成功]  (default via copilot login)
  github_pat_   Fine-grained PAT      [成功]  (needs Copilot Requests permission)
  ghu_          GitHub App token      [成功]  (via environment variable)
  ghp_          Classic PAT           [失败]  NOT SUPPORTED
...
def validate_copilot_token(token: str) -> tuple[bool, str]:
    ...
    if token.startswith(_CLASSIC_PAT_PREFIX):
        return False, (
            ...
            "  → `copilot login` or `bookworm model` to authenticate via OAuth\n"

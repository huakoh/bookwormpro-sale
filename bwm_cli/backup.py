...
def run_backup(args) -> None:
    ...
    if not hermes_root.is_dir():
        print(f"Error: BookwormPRO home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    print(f"\nRestore with: bookworm import {out_path.name}")
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has BookwormPRO configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: bookworm profile list  (after installing bookworm)")

        # Guidance
        print()
        if not (hermes_root / "bookwormpro").is_dir():
            print("Note: The bookwormpro codebase was not included in the backup.")
            print("  If this is a fresh install, run: bookworm update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  bookworm -p {pname} gateway install")

        print("Done. Your BookwormPRO configuration has been restored.")
...
def run_backup_git(args) -> None:
    ...
    if not git_dir.is_dir():
        print(f">>> 第一层: Git 初始化 {display_hermes_home()}")
...
interrupt_debug.log
""")
        print("   [创建] .gitignore (排除 transient 文件)")
        # Initial commit
        subprocess.run(
            ["git", "add", "-A"],
            cwd=str(hermes_root),
        ...
        if result.returncode == 0:
            ...
            print(f"   [信息] {result.stdout.strip()}")

    # Step 2: Auto-commit
    print(f">>> 第二层: Git 自动提交")
    subprocess.run(
        ["git", "add", "-A"],
        cwd=str(hermes_root),
        capture_output=True,
...
def run_backup_push(args) -> None:
    ...
    if result.returncode != 0:
        print("   [警告] 未配置远程仓库 (git remote)")
        print("   配置命令: cd ~/.bookwormpro && git remote add origin <你的GitHub仓库URL>")
        return
...
def run_backup_full(args) -> None:
    ...
    print("  三层备份完成！")
    print("=" * 60)
    print(f"  1. ZIP:  本地完整快照")
    print(f"  2. Git:  增量版本控制")
    print(f"  3. Push: 异地容灾备份")
    print(f"\n  恢复: bookworm import <zipfile>")
    print(f"        OR: git clone <remote> ~/.bookwormpro/")

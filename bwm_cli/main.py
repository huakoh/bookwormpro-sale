...
BookwormPRO CLI - Main entry point.



Usage:

    bookworm                     # Interactive chat (default)

    bookworm chat                # Interactive chat

    bookworm gateway             # Run gateway in foreground

    bookworm gateway start       # Start gateway as service

    bookworm gateway stop        # Stop gateway service

    bookworm gateway status      # Show gateway status

    bookworm gateway install     # Install gateway service

    bookworm gateway uninstall   # Uninstall gateway service

    bookworm setup               # Interactive setup wizard

    bookworm logout              # Clear stored authentication

    bookworm status              # Show status of all components

    bookworm cron                # Manage cron jobs

    bookworm cron list           # List cron jobs

    bookworm cron status         # Check if cron scheduler is running

    bookworm doctor              # Check configuration and dependencies

    bookworm honcho setup                    # Configure Honcho AI memory integration

    bookworm honcho status                   # Show Honcho config and connection status

    bookworm honcho sessions                 # List directory → session name mappings

    bookworm honcho map <name>               # Map current directory to a session name

    bookworm honcho peer                     # Show peer names and dialectic settings

    bookworm honcho peer --user NAME         # Set user peer name

    bookworm honcho peer --ai NAME           # Set AI peer name

    bookworm honcho peer --reasoning LEVEL   # Set dialectic reasoning level

    bookworm honcho mode                     # Show current memory mode

    bookworm honcho mode [hybrid|honcho|local]  # Set memory mode

    bookworm honcho tokens                   # Show token budget settings

    bookworm honcho tokens --context N       # Set session.context() token cap

    bookworm honcho tokens --dialectic N     # Set dialectic result char cap

    bookworm honcho identity                 # Show AI peer identity representation

    bookworm honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)

    bookworm honcho migrate                  # Step-by-step migration guide: OpenClaw native → BookwormPRO + Honcho

    bookworm version             Show version

    bookworm update              Update to latest version

    bookworm uninstall           Uninstall BookwormPRO

    bookworm acp                 Run as an ACP server for editor integration

    bookworm sessions browse     Interactive session picker with search



    bookworm claw migrate --dry-run  # Preview migration without changes
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to BOOKWORMPRO_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (bookworm tools, bookworm setup, bookworm model) use
    ...
    if not sys.stdin.isatty():
        ...
        print(
            ...
            f"Error: 'bookworm {command_name}' requires an interactive terminal.\n"
...
# Profile override — MUST happen before any bookworm module import.

#

# Many modules cache BOOKWORMPRO_HOME at import time (module-level constants).

# We intercept --profile/-p from sys.argv here and set the env var so that

# every subsequent ``os.getenv("BOOKWORMPRO_HOME", ...)`` resolves correctly.

# The flag is stripped from sys.argv so argparse never sees it.

# Falls back to ~/.bookwormpro/active_profile for sticky default.

# ---------------------------------------------------------------------------

def _apply_profile_override() -> None:

    """Pre-parse --profile/-p and set BOOKWORMPRO_HOME before module imports."""
    ...
    # 2. If no flag, check active_profile in the bookworm root
    ...
    # 3. If we found a profile, resolve and set BOOKWORMPRO_HOME

    if profile_name is not None:

        try:

            from bwm_cli.profiles import resolve_profile_env
            ...
            # A bug in profiles.py must NEVER prevent bookworm from starting
        ...
        os.environ["BOOKWORMPRO_HOME"] = hermes_home
...
from bwm_cli.config import get_hermes_home
...
# Bridge security.redact_secrets from config.yaml → BOOKWORMPRO_REDACT_SECRETS env

# var BEFORE bwm_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    if "BOOKWORMPRO_REDACT_SECRETS" not in os.environ:
        ...
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    ...
                    os.environ["BOOKWORMPRO_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `bookworm` subcommands

# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.

try:

    from bwm_logging import setup_logging as _setup_logging
...
try:
    ...
    from bwm_cli.config import load_config as _load_config_early

    from bwm_constants import apply_ipv4_preference as _apply_ipv4
...
from bwm_cli import __version__, __release_date__

from bwm_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from bwm_cli.config import get_env_path, get_hermes_home, load_config

    from bwm_cli.auth import get_auth_status



    # Determine whether BookwormPRO itself has been explicitly configured (model
    ...
    # Check for BookwormPRO Portal OAuth credentials
    ...
    # Only count these if BookwormPRO has been explicitly configured — Claude Code

    # being installed doesn't mean the user wants BookwormPRO to use their tokens.
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'bookworm')
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                ...
                print(
                    ...
                    f"Or run: sudo bookworm {' '.join(cli_args)}",
            ...
            print(
                ...
                f"The container may be running under root. Try: sudo bookworm {' '.join(cli_args)}",
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    """True when @bookworm/ink is missing or node_modules is behind package-lock.json (post-pull)."""

    ink = root / "node_modules" / "@bookworm" / "ink" / "package.json"
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    ...
    """Directory whose dist/entry.js we should run: BOOKWORMPRO_TUI_DIR first, else repo ui-tui."""

    env = os.environ.get("BOOKWORMPRO_TUI_DIR")
...
def _hermes_ink_bundle_stale(tui_dir: Path) -> bool:
    ...
    ink_root = tui_dir / "packages" / "bookworm-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``BOOKWORMPRO_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    ...
    if os.environ.get("BOOKWORMPRO_SKIP_NODE_BOOTSTRAP"):
    ...
    hermes_home = os.environ.get("BOOKWORMPRO_HOME") or str(Path.home() / ".bookwormpro")
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "BOOKWORMPRO_HOME": hermes_home},
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    """TUI: --dev → tsx src; else node dist (BOOKWORMPRO_TUI_DIR or ui-tui, build when stale)."""
    ...
    def _node_bin(bin: str) -> str:
        ...
        if bin == "node":
            ...
            env_node = os.environ.get("BOOKWORMPRO_NODE")
    ...
    # pre-built dist + node_modules (nix / full BOOKWORMPRO_TUI_DIR) skips npm.

    if not tui_dev:

        ext_dir = os.environ.get("BOOKWORMPRO_TUI_DIR")
    ...
    if tui_dev:
        ...
        if _hermes_ink_bundle_stale(tui_dir):
            ...
            result = subprocess.run(
                ...
                [npm, "run", "build", "--prefix", "packages/bookworm-ink"],
...
def _launch_tui(resume_session_id: Optional[str] = None, tui_dev: bool = False):
    ...
    env["BOOKWORMPRO_PYTHON_SRC_ROOT"] = os.environ.get(

        "BOOKWORMPRO_PYTHON_SRC_ROOT", str(PROJECT_ROOT)

    )

    env.setdefault("BOOKWORMPRO_PYTHON", sys.executable)

    env.setdefault("BOOKWORMPRO_CWD", os.getcwd())
    ...
    if resume_session_id:
        ...
        env["BOOKWORMPRO_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("BOOKWORMPRO_TUI") == "1"
    ...
    if not _has_any_provider_configured():
        ...
        print(
            ...
            "It looks like BookwormPRO isn't configured yet -- no API keys or providers found."
    ...
    try:
        ...
        from bwm_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        ...
        os.environ["BOOKWORMPRO_YOLO_MODE"] = "1"



    # --ignore-user-config: make load_cli_config() / load_config() skip the

    # user's ~/.bookwormpro/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "ignore_user_config", False):
        ...
        os.environ["BOOKWORMPRO_IGNORE_USER_CONFIG"] = "1"
    ...
    if getattr(args, "ignore_rules", False):
        ...
        os.environ["BOOKWORMPRO_IGNORE_RULES"] = "1"
    ...
    if getattr(args, "source", None):
        ...
        os.environ["BOOKWORMPRO_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from bwm_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from bwm_cli.config import get_env_value, save_env_value
...
def cmd_setup(args):
    ...
    from bwm_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``bookworm model``) and the setup wizard
    ...
    from bwm_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        ...
        config_provider or os.getenv("BOOKWORMPRO_INFERENCE_PROVIDER") or "auto"
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                ...
                f"Unknown provider '{effective_provider}'. Check 'bookworm model' for "

                "available providers, or run 'bookworm doctor' to diagnose config "
    ...
    from bwm_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    elif selected_provider == "bookwormpro":
    ...
    # a leftover OPENAI_BASE_URL in ~/.bookwormpro/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    ...
    """Remove OPENAI_BASE_URL from ~/.bookwormpro/.env if the active provider is not 'custom'.
    ...
    from bwm_cli.config import get_env_value, save_env_value, load_config
...
# BookwormPRO uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `bookworm model` provider picker. It does NOT re-run credential setup — it

# only routes already-authenticated providers to specific aux tasks. Users

# configure new providers through the normal `bookworm model` flow first.
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``bookworm model`` flow, then route aux tasks to them here.

    """

    from bwm_cli.config import load_config

    from bwm_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from bwm_cli.auth import _prompt_model_selection

    from bwm_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    try:
        ...
        from bwm_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from bwm_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from bwm_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    """BookwormPRO Portal provider: ensure logged in, then pick model."""
    ...
    from bwm_cli.nous_subscription import prompt_enable_tool_gateway



    state = get_provider_auth_state("bookwormpro")
    ...
    if not state or not state.get("access_token"):
        ...
        try:
            ...
            _login_nous(mock_args, PROVIDER_REGISTRY["bookwormpro"])
    ...
    model_ids = _PROVIDER_MODELS.get("bookwormpro", [])
    ...
    pricing = get_pricing_for_provider("bookwormpro")
    ...
    try:
        ...
        _nous_state = get_provider_auth_state("bookwormpro")
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            ...
            from bwm_cli.auth import DEFAULT_NOUS_PORTAL_URL
    ...
    if selected:
        ...
        # Reactivate BookwormPRO as the provider and update config

        inference_url = creds.get("base_url", "")

        _update_config_for_provider("bookwormpro", inference_url)
        ...
        model_cfg["provider"] = "bookwormpro"
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from bwm_cli.codex_models import get_codex_model_ids
    ...
    # Prefer credential pool (where `bookworm auth` stores device_code tokens),
    ...
    if not _codex_token:
        ...
        try:
            ...
            from bwm_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from bwm_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.bookwormpro/config.yaml.
...
def _model_flow_custom(config):
    ...
    from bwm_cli.auth import _save_model_choice, deactivate_provider

    from bwm_cli.config import get_env_value, load_config, save_config
    ...
    try:
        ...
        # getpass echoes nothing, so a control char here is invisible to the
        # user and would ship silently and surface much later as an opaque 400.
        # Catch it at the door and say what actually happened.
        from bwm_cli.config import describe_control_chars, strip_control_chars

        while api_key and describe_control_chars(api_key):

            _found = describe_control_chars(api_key)

            print()

            print("  That key contains control characters, which would make every")

            print("  request malformed (the server would answer 400, not 401):")
            ...
            if not strip_control_chars(api_key):

                print("  Nothing else was entered — the paste did not arrive.")

            print("  Type the key, or paste via a file. Blank to leave it unset.")

            print()

            api_key = getpass.getpass("API key (retry): ").strip()
    ...
    # Was: `_looks_local and ...`, which silently exempted remote relays —
    # the 0715 incident had a remote base_url missing /v1 and was never asked.
    # _looks_local now only chooses the wording, not whether we ask at all.
    if not _url_lower.endswith("/v1"):
        ...
        if _looks_local:
            ...
            print(_("  Most OpenAI-compatible endpoints and relays require it."))
    ...
    from bwm_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"BookwormPRO will still save it."
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        from bwm_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from bwm_cli.config import save_env_value, load_config, save_config
    ...
    if not api_key:
        ...
        if choice == "1":
            ...
            try:
                ...
                from bwm_cli.copilot_auth import copilot_device_code_login
            ...
            try:
                ...
                from bwm_cli.copilot_auth import validate_copilot_token
    ...
    if live_models:
        ...
        if model_list:
            ...
            print(
                ...
                "  [警告] Could not auto-detect models from GitHub Copilot — showing defaults."
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(
            ...
            "  Set BOOKWORMPRO_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from bwm_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so bookworm knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    ...
                    "[失败] This Google API key is on the free tier "
                ...
                print(
                    ...
                    "   BookwormPRO typically makes 3-10 API calls per user turn "
                ...
                print(
                    ...
                    "   To use Gemini with BookwormPRO, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, BookwormPRO."
    ...
    if provider_id == "ollama-cloud":
        ...
        from bwm_cli.models import fetch_ollama_cloud_models
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print(
                ...
                f"    BookwormPRO will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'bookworm model'.
...
def cmd_login(args):
    ...
    """Authenticate BookwormPRO CLI with a provider."""

    from bwm_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from bwm_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from bwm_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from bwm_cli.status import show_status
...
def cmd_cron(args):
    ...
    from bwm_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from bwm_cli.webhook import webhook_command
...
def cmd_hooks(args):
    ...
    from bwm_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from bwm_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from bwm_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from bwm_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from bwm_cli.config import config_command
...
def cmd_backup(args):
    ...
    """Back up BookwormPRO home directory to a zip file.

    Supports three-layer backup: ZIP + Git + Remote push.
    """

    if getattr(args, "full", False):

        from bwm_cli.backup import run_backup_full



        run_backup_full(args)

    elif getattr(args, "push", False):

        from bwm_cli.backup import run_backup_push



        run_backup_push(args)

    elif getattr(args, "git", False):

        from bwm_cli.backup import run_backup_git
        ...
        from bwm_cli.backup import run_quick_backup
        ...
        from bwm_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from bwm_cli.backup import run_import
...
def cmd_version(args):
    ...
    try:
        ...
        from bwm_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    ...
    """Uninstall BookwormPRO."""

    _require_tty("uninstall")

    from bwm_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``bookworm update --gateway`` so interactive prompts (stash restore,
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               ...
               instead of a soft warning (used by ``bookworm web``).
    ...
    if r1.returncode != 0:
        ...
        print(
            ...
            f"  {'[失败]' if fatal else '[警告]'} Web UI npm install failed"

            + ("" if fatal else " (bookworm web will not be available)")
    ...
    if r2.returncode != 0:
        ...
        print(
            ...
            f"  {'[失败]' if fatal else '[警告]'} Web UI build failed"
...
def _update_via_zip(args):
    ...
    """Update BookwormPRO by downloading a ZIP archive.
    ...
    zip_url = (
        ...
        f"https://github.com/huakoh/BookwormPRO/archive/refs/heads/{branch}.zip"
    ...
    try:
        ...
        tmp_dir = tempfile.mkdtemp(prefix="bookworm-update-")

        zip_path = os.path.join(tmp_dir, f"bookwormpro-{branch}.zip")
        ...
        # GitHub ZIPs extract to bookwormpro-<branch>/

        extracted = os.path.join(tmp_dir, f"bookwormpro-{branch}")
    ...
    if removed:
        ...
        print(
            ...
            f"  [成功] Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        ...
        "bookworm-update-autostash-%Y%m%d-%H%M%S"
...
def _restore_stashed_changes(
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes bookworm completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        ...
        print(
            ...
            "[警告] Local changes were restored, but BookwormPRO couldn't find the stash entry to drop."
        ...
        if drop.returncode != 0:
            ...
            print(
                ...
                "[警告] Local changes were restored, but BookwormPRO couldn't drop the saved stash entry."
...
# Fork detection and upstream management for `bookworm update`
...
OFFICIAL_REPO_URLS = {
    ...
    "https://github.com/huakoh/BookwormPRO.git",

    "git@github.com:huakoh/BookwormPRO.git",

    "https://github.com/huakoh/BookwormPRO",

    "git@github.com:huakoh/BookwormPRO",

}

OFFICIAL_REPO_URL = "https://github.com/huakoh/BookwormPRO.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                ...
                print(
                    ...
                    "  [成功] Added upstream: https://github.com/huakoh/BookwormPRO.git"
            ...
            print(
                ...
                "  Skipped. Run 'git remote add upstream https://github.com/huakoh/BookwormPRO.git' to add later."
    ...
    try:
        ...
        print(
            ...
            "  [失败] Failed to pull from upstream. You may need to resolve conflicts manually."
...
def _invalidate_update_cache():
    ...
    ``bookworm update``, every profile is now current.
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "bookwormpro[matrix]" or "package-name[extra]".
    ...
    *,
    ...
    try:
        ...
        print(
            ...
            "  [警告] Optional extras failed, reinstalling base dependencies and retrying extras individually..."
    ...
    if installed_extras:
        ...
        print(
            ...
            f"  [成功] Reinstalled optional extras individually: {', '.join(installed_extras)}"
    ...
    if failed_extras:
        ...
        print(
            ...
            f"  [警告] Skipped optional extras that still failed: {', '.join(failed_extras)}"
...
class _UpdateOutputStream:
    ...
    """Stream wrapper used during ``bookworm update`` to survive terminal loss.
    ...
    * Every write is also mirrored to an append-only log file
      ...
      (``~/.bookwormpro/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``bookworm update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``bookworm update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ...
       ``~/.bookwormpro/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``bookworm update --gateway``) the update is already
    ...
    try:
        ...
        # bwm_cli.config.get_hermes_home to simulate setup failure.

        from bwm_cli.config import get_hermes_home as _get_hermes_home
...
def cmd_update(args):
    ...
    """Update BookwormPRO to the latest version.
    ...
    from bwm_cli.config import is_managed, managed_error



    if is_managed():

        managed_error("update BookwormPRO")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if fetch_result.returncode != 0:
            ...
            if "Could not resolve host" in stderr or "unable to access" in stderr:
                ...
                print(
                    ...
                    "[失败] Authentication failed — check your git credentials or SSH key."
        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                print(
                    ...
                    "  [警告] Fast-forward not possible (history diverged), resetting to match remote..."
        ...
        # the old bytecode (e.g. get_hermes_home added to bwm_constants).
        ...
        # modules in this process.  Reload bwm_constants so that any lazy
        ...
        if needs_migration:
            ...
            if missing_env:
                ...
                print(
                    ...
                    f"  [警告]  {len(missing_env)} new required setting(s) need configuration"
            ...
            if gateway_mode:
                ...
                print(
                    ...
                    "    Run 'bookworm config migrate' later to apply any new config/env options."
        ...
        # When running as ``bookworm update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            # Discover all bookworm-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            ]  # e.g. bookworm-gateway.service or bookworm-gateway-coder.service
                            ...
                            if _graceful_ok:
                                ...
                                print(
                                    ...
                                    f"  [警告] {svc_name} drained but didn't relaunch — forcing restart"
                            ...
                            if restart.returncode == 0:
                                ...
                                print(
                                    ...
                                    f"  [警告] Failed to restart {svc_name}: {restart.stderr.strip()}"
                ...
                if killed_pids:
                    ...
                    if len(killed_pids) > 1:
                        ...
                        print(
                            ...
                            "    (or: bookworm -p <profile> gateway run  for each profile)"
        ...
        # Warn if legacy BookwormPRO gateway unit files are still installed.

        # When both bookworm.service (from a pre-rename install) and the

        # current bookworm-gateway.service are enabled, they SIGTERM-fight

        # for the same bot token (see PR #11909). Flagging here means

        # every `bookworm update` surfaces the issue until the user migrates.
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``bookworm -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        # Bare `bookworm profile` — show current profile status
    ...
    if action == "list":
        ...
        try:
            ...
            if not clone_all:
                ...
                if result:
                    ...
                    print(
                        ...
                        "[警告] Skills could not be seeded. Run `{} update` to retry.".format(
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        ...
                        f"  Choose a custom alias:  bookworm profile alias {name} --name <custom>"
            ...
            if clone or clone_all:
                ...
                print(
                    ...
                    f"\n  [警告] This profile has no API keys yet. Run '{name} setup' first,"
        ...
        from bwm_cli.profiles import profile_exists
        ...
        from bwm_cli.profiles import rename_profile
        ...
        from bwm_cli.profiles import export_profile
        ...
        from bwm_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    if "BOOKWORMPRO_WEB_DIST" not in os.environ:
    ...
    from bwm_cli.web_server import start_server



    embedded_chat = args.tui or os.environ.get("BOOKWORMPRO_DASHBOARD_TUI") == "1"
...
def cmd_completion(args, parser=None):
    ...
    from bwm_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    """View and filter BookwormPRO log files."""

    from bwm_cli.logs import tail_log, list_logs
...
def main():
    ...
    """Main entry point for bookworm CLI."""

    parser = argparse.ArgumentParser(

        prog="bookworm",

        description="BookwormPRO - AI assistant with tool-calling capabilities",
...
Examples:
    ...
    bookworm                        Start interactive chat

    bookworm chat -q "Hello"        Single query mode

    bookworm -c                     Resume the most recent session

    bookworm -c "my project"        Resume a session by name (latest in lineage)

    bookworm --resume <session_id>  Resume a specific session by ID

    bookworm setup                  Run setup wizard

    bookworm logout                 Clear stored authentication

    bookworm auth add <provider>    Add a pooled credential

    bookworm auth list              List pooled credentials

    bookworm auth remove <p> <t>    Remove pooled credential by index, id, or label

    bookworm auth reset <provider>  Clear exhaustion status for a provider

    bookworm model                  Select default model

    bookworm config                 View configuration

    bookworm config edit            Edit config in $EDITOR

    bookworm config set model gpt-4 Set a config value

    bookworm gateway                Run messaging gateway

    bookworm -s bookwormpro-dev,github-auth

    bookworm -w                     Start in isolated git worktree

    bookworm gateway install        Install gateway background service

    bookworm sessions list          List past sessions

    bookworm sessions browse        Interactive session picker

    bookworm sessions rename ID T   Rename/title a session

    bookworm logs                   View agent.log (last 50 lines)

    bookworm logs -f                Follow agent.log in real time

    bookworm logs errors            View errors.log

    bookworm logs --since 1h        Lines from the last hour

    bookworm debug share             Upload debug report for support
...
""",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to BOOKWORMPRO_ACCEPT_HOOKS=1 or "
    ...
    parser.add_argument(
        ...
        help="Ignore ~/.bookwormpro/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with BookwormPRO",
    ...
    chat_parser.add_argument(
        ...
        help=(
            ...
            "without a TTY prompt (see also BOOKWORMPRO_ACCEPT_HOOKS env var and "
    ...
    chat_parser.add_argument(
        ...
        help="Ignore ~/.bookwormpro/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    ...
    model_parser.add_argument(
        ...
        help="Portal base URL for BookwormPRO login (default: production portal)",
    ...
    model_parser.add_argument(
        ...
        help="Inference API base URL for BookwormPRO login (default: production inference API)",
    ...
    model_parser.add_argument(
        ...
        help="OAuth client id to use for BookwormPRO login (default: bookworm-cli)",

    )

    model_parser.add_argument(

        "--scope", default=None, help="OAuth scope to request for BookwormPRO login"
    ...
    model_parser.add_argument(
        ...
        help="Do not attempt to open the browser automatically during BookwormPRO login",
    ...
    model_parser.add_argument(
        ...
        help="HTTP request timeout in seconds for BookwormPRO login (default: 15)",

    )

    model_parser.add_argument(

        "--ca-bundle", help="Path to CA bundle PEM file for BookwormPRO TLS verification"
    ...
    model_parser.add_argument(
        ...
        help="Disable TLS verification for BookwormPRO login (testing only)",
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        ...
        help="Remove legacy bookworm.service units from pre-rename installs",

        description=(

            "Stop, disable, and remove legacy BookwormPRO gateway unit files "

            "(e.g. bookworm.service) left over from older installs. Profile "

            "units (bookworm-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure BookwormPRO with an interactive wizard. "

        "Run a specific section: bookworm setup model|tts|terminal|gateway|tools|agent",
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for BookwormPRO CLI",
    ...
    login_parser.add_argument(
        ...
        choices=["bookwormpro", "openai-codex"],

        default=None,

        help="Provider to authenticate with (default: bookwormpro)",
    ...
    login_parser.add_argument(
        ...
        "--client-id", default=None, help="OAuth client id to use (default: bookworm-cli)"
    ...
    logout_parser.add_argument(
        ...
        choices=["bookwormpro", "openai-codex", "spotify"],
    ...
    auth_add.add_argument("--portal-url", help="BookwormPRO portal base URL")

    auth_add.add_argument("--inference-url", help="BookwormPRO inference base URL")
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate BookwormPRO with Spotify via PKCE")

    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")

    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set BOOKWORMPRO_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of BookwormPRO components",
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Inspect shell-script hooks declared in ~/.bookwormpro/config.yaml, "

            "test them against synthetic payloads, and manage the first-use "

            "consent allowlist at ~/.bookwormpro/shell-hooks-allowlist.json."
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with BookwormPRO setup",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your BookwormPRO setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for BookwormPRO. Use 'bookworm debug share' to "
...
Examples:
    ...
    bookworm debug share              Upload debug report and print URL

    bookworm debug share --lines 500  Include more log lines

    bookworm debug share --expire 30  Keep paste for 30 days

    bookworm debug share --local      Print report locally (no upload)

    bookworm debug delete <url>       Delete a previously uploaded paste
...
""",
    ...
    delete_parser = debug_sub.add_parser(
        ...
        help="Delete a paste uploaded by 'bookworm debug share'",
    ...
    backup_parser = subparsers.add_parser(
        ...
        help="Back up BookwormPRO home directory to a zip file",

        description="Create a zip archive of your entire BookwormPRO configuration, "

        "skills, sessions, and data (excludes the bookwormpro codebase). "
    ...
    backup_parser.add_argument(
        ...
        help="Output path for the zip file (default: ~/bookworm-backup-<timestamp>.zip)",
    ...
    backup_parser.add_argument(
        "--git",
        action="store_true",
        help="Git auto-commit in ~/.bookwormpro (Layer 2)",
    )
    backup_parser.add_argument(
        "--push",
        action="store_true",
        help="Git push to remote after commit (Layer 3)",
    )
    backup_parser.add_argument(
        "--full",
        action="store_true",
        help="Full three-layer backup: ZIP + Git + Remote push",
    )
    ...
    import_parser = subparsers.add_parser(
        ...
        help="Restore a BookwormPRO backup from a zip file",

        description="Extract a previously created BookwormPRO backup into your "

        "BookwormPRO home directory, restoring configuration, skills, "
    ...
    nav_parser = subparsers.add_parser(
        "navigate", help="Find the right skills for your task",
        description="Get skill recommendations based on your intent.",
    )
    nav_parser.add_argument("intent", nargs="?", help="What do you want to do?")
    nav_parser.add_argument("--learn", choices=["1","2","3"], help="Show learning path for level")
    ...
    guard_parser = subparsers.add_parser(
        "guardian", help="Scan a skill for safety issues",
        description="Security scan for installed skills.",
    )
    guard_parser.add_argument("skill", nargs="?", help="Skill name to scan (default: all)")
    ...
    audit_parser = subparsers.add_parser(
        "audit", help="View agent operation audit log",
        description="Search, view, export the audit log of agent operations.",
    )
    audit_subparsers = audit_parser.add_subparsers(dest="audit_action")
    p = audit_subparsers.add_parser("show", help="Recent audit events")
    p.add_argument("--since"); p.add_argument("--limit", type=int, default=50)
    p=audit_subparsers.add_parser("search", help="Search audit events")
    p.add_argument("query"); p.add_argument("--limit", type=int, default=50)
    p=audit_subparsers.add_parser("stats", help="Audit statistics")
    p.add_argument("--days", type=int, default=30)
    p=audit_subparsers.add_parser("export", help="Export audit events")
    p.add_argument("--output", default="audit-export.jsonl")
    p.add_argument("--since", default="90d")
    p=audit_subparsers.add_parser("cleanup", help="Remove old events")
    p.add_argument("--days", type=int, default=90)
    def cmd_audit(args):
        action = getattr(args, "audit_action", "show") or "show"
        from bwm_cli.audit import audit_show, audit_search, audit_stats, audit_export, audit_cleanup
        {"show": audit_show, "search": audit_search, "stats": audit_stats, "export": audit_export, "cleanup": audit_cleanup}[action](args)
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage BookwormPRO configuration",
    ...
    def cmd_pairing(args):
        ...
        from bwm_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "Clear a bundled skill's entry from the sync manifest (~/.bookwormpro/skills/.bundled_manifest) "

            "so future 'bookworm update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from bwm_cli.skills_config import skills_command as skills_config_command
            ...
            from bwm_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        ...
        help="Git URL or owner/repo shorthand (e.g. anpicasso/bookworm-plugin-chrome-profiles)",
    ...
    _install_enable_group.add_argument(
        ...
        help="Install disabled (skip confirmation prompt); enable later with `bookworm plugins enable <name>`",
    ...
    def cmd_plugins(args):
        ...
        from bwm_cli.plugins_cmd import plugins_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'bookworm tools' with no subcommand for the interactive configuration UI."
    ...
    # bookworm tools list [--platform cli]
    ...
    # bookworm tools disable <name...> [--platform cli]
    ...
    # bookworm tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            ...
            from bwm_cli.tools_config import tools_disable_enable_command
            ...
            from bwm_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        ...
        help="Manage MCP servers and run BookwormPRO as an MCP server",

        description=(

            "Manage MCP server connections and run BookwormPRO as an MCP server.\n\n"

            "MCP servers provide additional tools via the Model Context Protocol.\n"

            "Use 'bookworm mcp add' to connect to a new server, or\n"

            "'bookworm mcp serve' to expose BookwormPRO conversations over MCP."
    ...
    mcp_serve_p = mcp_sub.add_parser(
        ...
        help="Run BookwormPRO as an MCP server (expose conversations to other agents)",
    ...
    def cmd_mcp(args):
        ...
        from bwm_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            if hermes_bin:
                ...
                os.execvp(hermes_bin, ["bookworm", "--resume", selected_id])
                ...
                os.execvp(
                    ...
                    [sys.executable, "-m", "bwm_cli.main", "--resume", selected_id],
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to BookwormPRO",
    ...
    claw_migrate = claw_subparsers.add_parser(
        ...
        help="Migrate from OpenClaw to BookwormPRO",
    ...
    def cmd_claw(args):
        ...
        from bwm_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        ...
        help="Update BookwormPRO to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        ...
        help="Uninstall BookwormPRO",

        description="Remove BookwormPRO from your system. Can keep configs/data for reinstall.",
    ...
    acp_parser = subparsers.add_parser(
        ...
        help="Run BookwormPRO as an ACP (Agent Client Protocol) server",

        description="Start BookwormPRO in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    ...
    def cmd_acp(args):
        ...
        """Launch BookwormPRO as an ACP server."""
    ...
    profile_parser = subparsers.add_parser(
        ...
        help="Manage profiles — multiple isolated BookwormPRO instances",
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the BookwormPRO web dashboard for managing config, API keys, and sessions",
    ...
    dashboard_parser.add_argument(
        ...
        help=(
            ...
            "Expose the in-browser Chat tab (embedded `bookworm --tui` via PTY/WebSocket). "

            "Alternatively set BOOKWORMPRO_DASHBOARD_TUI=1."
    ...
    logs_parser = subparsers.add_parser(
        ...
        help="View and filter BookwormPRO log files",
...
Examples:
    ...
    bookworm logs                    Show last 50 lines of agent.log

    bookworm logs -f                 Follow agent.log in real time

    bookworm logs errors             Show last 50 lines of errors.log

    bookworm logs gateway -n 100     Show last 100 lines of gateway.log

    bookworm logs --level WARNING    Only show WARNING and above

    bookworm logs --session abc123   Filter by session ID

    bookworm logs --component tools  Only show tool-related lines

    bookworm logs --since 1h         Lines from the last hour

    bookworm logs --since 30m -f     Follow, starting from 30 min ago

    bookworm logs list               List available log files with sizes
...
""",
    ...
    # e.g. ``bookworm -c Pokemon Agent Dev`` → ``bookworm -c 'Pokemon Agent Dev'``
    ...
    from bwm_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'bookworm -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (bookworm hooks list, cron

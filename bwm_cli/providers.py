"""
Single source of truth for provider identity in BookwormPRO.
...
2. **BookwormPRO overlays** — transport type, auth patterns, aggregator flags,
...
# -- BookwormPRO overlay ----------------------------------------------------------
# BookwormPRO-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """BookwormPRO-specific provider metadata layered on top of models.dev."""
...
BOOKWORMPRO_OVERLAYS: Dict[str, HermesOverlay] = {
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="BOOKWORMPRO_QWEN_BASE_URL",
    ),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "bookworm", "user-config"
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "bookwormpro": "BookwormPRO Portal",
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. BookwormPRO overlays (for providers not in models.dev: bookwormpro, openai-codex, etc.)
      2. models.dev catalog + BookwormPRO overlay
    ...
    overlay = BOOKWORMPRO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + bookworm extra
    ...
    if overlay is not None:
        # BookwormPRO-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in BOOKWORMPRO_OVERLAYS
    if provider == "bedrock":

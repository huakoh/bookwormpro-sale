"""``bookworm plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.bookwormpro/plugins/``.
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from bwm_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _plugin_exists(name: str) -> bool:
    ...
    import bwm_cli
    repo_plugins = _P(bwm_cli.__file__).resolve().parent.parent / "plugins"
...
def _discover_all_plugins() -> list:
    ...
    import bwm_cli
    repo_plugins = Path(bwm_cli.__file__).resolve().parent.parent / "plugins"
...
def plugins_command(args) -> None:
    """Dispatch bookworm plugins subcommands."""

...
def _install_dependencies(provider_name: str) -> None:
    ...
    if not uv_path:
        print(f"  [警告] uv not found — cannot install dependencies")
        print(f"  Install uv: curl -LsSf https://astral.sh/uv/install.sh | sh")
        print(f"  Then re-run: bookworm memory setup")
        return
    ...
    try:
        ...
        print(f"  [成功] Installed {', '.join(missing)}")
    except subprocess.CalledProcessError as e:
        print(f"  [警告] Failed to install {', '.join(missing)}")
    ...
    for dep in ext_deps:
        ...
        if check_cmd:
            try:
                ...
                print(f"\n  [警告] Invalid check command for '{dep_name}': {check_cmd}")
                continue
            ...
            try:
                ...
                if install_cmd:
                    print(f"\n  [警告] '{dep_name}' not found. Install with:")
                    print(f"    {install_cmd}")
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'bookworm memory setup' to see available providers.\n")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.bookwormpro/plugins/ and try again.\n")
        return
        ...
        if found:
            ...
            for pname, _, p in providers:
                if pname == provider_name:
                    if p.is_available():
                        print(f"  Status:    available [成功]")
                    else:
                        print(f"  Status:    not available [失败]")
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.bookwormpro/plugins/")

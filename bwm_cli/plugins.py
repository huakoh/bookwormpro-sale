"""
BookwormPRO Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with bookwormpro;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.bookwormpro/plugins/<name>/``
3. **Project plugins** – ``./.bookwormpro/plugins/<name>/`` (opt-in via
   ``BOOKWORMPRO_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``bookwormpro.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "bookwormpro.plugins"
...
class PluginManifest:
    ...
    # lookups and by ``bookworm plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``bookworm honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``bookworm <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.bookwormpro/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to bwm_cli/. Two layouts are
        ...
        # 2. User plugins (~/.bookwormpro/plugins/)
        user_dir = get_hermes_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.bookwormpro/plugins/)
        if _env_enabled("BOOKWORMPRO_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".bookwormpro" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with bookworm and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `bookworm plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``bookworm tools`` TUI so plugin-provided toolsets appear
    ...
    for ts_key in sorted(toolset_tools):
        ...
        label = f"[网络] {ts_key.replace('_', ' ').title()}"

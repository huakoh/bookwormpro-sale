...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or bookworm -c.",
    ...
    "Ctrl+Z suspends BookwormPRO to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "bookworm -c resumes your most recent CLI session. bookworm -c \"project name\" resumes by title.",
    "bookworm -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "bookworm -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "bookworm chat -t web,terminal enables only specific toolsets for a focused session.",
    "bookworm chat -s github-pr-workflow preloads a skill at launch.",
    "bookworm chat -q \"query\" runs a single non-interactive query and exits.",
    "bookworm chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "bookworm chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "bookworm --yolo bypasses all dangerous command approval prompts for the entire session.",
    "bookworm chat --source telegram tags the session for filtering in bookworm sessions list.",
    "bookworm -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "bookworm doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "bookworm dump outputs a compact setup summary — great for bug reports.",
    "bookworm config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "bookworm config edit opens config.yaml in your default editor.",
    "bookworm config check scans for missing or stale configuration options.",
    "bookworm sessions browse opens an interactive session picker with search.",
    "bookworm sessions stats shows session counts by platform and database size.",
    "bookworm sessions prune --older-than 30 cleans up old sessions.",
    "bookworm skills search react --source skills-sh searches the skills.sh public directory.",
    "bookworm skills check scans installed hub skills for upstream updates.",
    "bookworm skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "bookworm skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "bookworm mcp add github --command npx adds MCP servers from the command line.",
    "bookworm mcp serve runs BookwormPRO itself as an MCP server for other agents.",
    "bookworm auth add lets you add multiple API keys for credential pool rotation.",
    "bookworm completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "bookworm logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "bookworm backup creates a zip backup of your entire BookwormPRO home directory.",
    "bookworm profile create coder creates an isolated profile that becomes its own command.",
    "bookworm profile create work --clone copies your current config and keys to a new profile.",
    "bookworm update syncs new bundled skills to ALL profiles automatically.",
    "bookworm gateway install sets up BookwormPRO as a system service (systemd/launchd).",
    "bookworm memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "bookworm webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as bookworm -w).",
    ...
    "BOOKWORMPRO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call BookwormPRO tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'bookworm profile create coder' creates the 'coder' command.",
    "bookworm profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, BookwormPRO prints a resume command with session ID and stats.",
    "bookworm sessions export backup.jsonl exports all sessions for backup or analysis.",
    "bookworm -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "bookworm skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .bookwormpro/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: bookworm cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "BookwormPRO runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "bookworm gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to BookwormPRO via a local macOS server.",
    ...
    "Context files (.bookwormpro.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.bookwormpro/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "BookwormPRO loads project context from .bookwormpro.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — bookworm tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.bookwormpro/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "bookworm chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "bookworm plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "bookworm acp runs BookwormPRO as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "BOOKWORMPRO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "bookworm login supports OAuth-based auth for BookwormPRO and OpenAI Codex providers.",
    ...
    "bookworm status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.bookwormpro/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.bookwormpro/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make BookwormPRO your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.bookwormpro/SOUL.md to customize.",
    ...
    "Before a gateway session reset, BookwormPRO auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.bookwormpro/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at BOOKWORMPRO_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "BOOKWORMPRO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in BOOKWORMPRO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.bookwormpro/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'bookworm profile create ls' would be rejected.",
    "bookworm profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "bookworm claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "BOOKWORMPRO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via bookworm tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.bookwormpro/.env.",
    "Skills from trusted repos (BookwormPRO) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.bookwormpro/skills/.hub/quarantine/ holds skills pending security review.",
]

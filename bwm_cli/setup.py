"""
Interactive setup wizard for BookwormPRO.
...
Config files are stored in ~/.bookwormpro/ for easy access.
"""
...
_DOCS_BASE = "https://bookwormpro.local/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during bookworm update)
...
from bwm_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure BookwormPRO using environment variables or config commands:")
    print_info("  bookworm config set model.provider custom")
    print_info("  bookworm config set model.base_url http://localhost:8080/v1")
    print_info("  bookworm config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'bookworm setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from bwm_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_success("  [成功] Saved")
    else:
        print_warning("  Skipped (configure later with 'bookworm setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'bookworm setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (BookwormPRO subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (BookwormPRO Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via BookwormPRO), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (BookwormPRO subscription)", True, None))
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via BookwormPRO subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'bookworm setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'bookworm setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (BookwormPRO subscription)", True, None))
    elif config.get("terminal", {}).get("backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'bookworm setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via BookwormPRO subscription)", True, None))
    ...
    # Spotify (OAuth via bookworm auth spotify — check auth.json, not env vars)
    try:
        from bwm_cli.auth import get_provider_auth_state
    ...
    for name, available, missing_var in tool_status:
        if available:
            print(f"   {color('[成功]', Colors.GREEN)} {name}")
        else:
            print(
                f"   {color('[失败]', Colors.RED)} {name} {color(f'(missing {missing_var})', Colors.DIM)}"
            )
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'bookworm setup tools' to configure them,"
        )
    ...
    print(
        color(
            "│              [成功] Setup Complete!                          │", Colors.GREEN
        )
    )
    print(
        color(
    ...
    config_path_colored = color(f"nano {get_config_path()}", Colors.DIM)
    env_path_colored = color(f"nano {get_env_path()}", Colors.DIM)
    print(_("   {config_path}").format(config_path=config_path_colored))
    print(_("   {env_path}").format(env_path=env_path_colored))
    print()
...
# between `bookworm tools` and `bookworm setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``bookworm model``)
    ...
    provider added to ``bookworm model`` is automatically available here.
    ...
    # Delegate to the shared bookworm model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from bwm_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: bookworm model")
    ...
    nous_subscription_selected = selected_provider == "bookwormpro"
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from bwm_cli.auth_commands import auth_add_command
            ...
            print_info(
                "BookwormPRO can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        _prov_names = {
            "bookwormpro-api": "BookwormPRO Portal API key",
        ...
        print_info("doesn't currently provide one BookwormPRO can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'bookworm setup' or configure AUXILIARY_VISION_* settings")


    # Tool Gateway prompt is already shown by _model_flow_nous() above.
    save_config(config)

    if not quick and selected_provider != "bookwormpro":
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("BookwormPRO Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("bookwormpro-openai")
    choices.extend(
        [
    ...
    selected_via_nous = selected == "bookwormpro-openai"
    if selected == "bookwormpro-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed BookwormPRO gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.bookwormpro/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"bookworm setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'bookworm setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where BookwormPRO runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using BookwormPRO via Telegram/Discord, this is where")
        print_info(
        ...
        if managed_modal_available:
            modal_choices = [
                "Use my BookwormPRO subscription",
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed BookwormPRO gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("BOOKWORMPRO_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `bookworm setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("BOOKWORMPRO_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("BOOKWORMPRO_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Telegram?", False):
            ...
            if not get_env_value("TELEGRAM_ALLOWED_USERS"):
                print_info("[警告]  Telegram has no user allowlist - anyone can use your bot!")
    ...
    print()
    print_info("[加锁] Security: Restrict who can use your bot")
    ...
    if allowed_users:
        ...
        print_info("[警告]  No allowlist set - anyone who finds your bot can use it!")

    print()
    print_info("📬 Home Channel: where BookwormPRO delivers cron job results,")
...
def _setup_discord():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Discord?", False):
            if not get_env_value("DISCORD_ALLOWED_USERS"):
                print_info("[警告]  Discord has no user allowlist - anyone can use your bot!")
                if prompt_yes_no("Add allowed users now?", True):
    ...
    if allowed_users:
        ...
        print_info("[警告]  No allowlist set - anyone in servers with your bot can use it!")
...
def _setup_slack():
    ...
    print_warning("   [警告] Without message.channels the bot will ONLY work in DMs,")
    ...
    print()
    print_info("   Full guide: https://bookwormpro.local/docs/user-guide/messaging/slack/")
    print()
    ...
    if allowed_users:
        ...
        print_warning("[警告]  No Slack allowlist set - unpaired users will be denied by default.")
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        if allowed_users:
            ...
            print_info("[警告]  No allowlist set - anyone who can message the bot can use it!")

        print()
        print_info("📬 Home Room: where BookwormPRO delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where BookwormPRO delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'bookworm whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'bookworm whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from bwm_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from bwm_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from bwm_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from bwm_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from bwm_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from bwm_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from bwm_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from bwm_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_bluebubbles():
    ...
    print_info("Connects BookwormPRO to iMessage via BlueBubbles — a free, open-source")
    ...
    print()
    print_info("[加锁] Security: Restrict who can message your bot")
    ...
    if allowed_users:
        ...
        print_info("[警告]  No allowlist set — anyone who can iMessage you can use the bot!")
...
def _setup_qqbot():
    ...
    from bwm_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_warning("[警告]  Webhook and SMS platforms require exposing gateway ports to the")
    print_warning("   internet. For security, run the gateway in a sandboxed environment")
    print_warning("   (Docker, VM, etc.) to limit blast radius from prompt injection.")
    print()
    print_info("   Full guide: https://bookwormpro.local/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://bookwormpro.local/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  bookworm config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with BookwormPRO from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'bookworm setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     bookworm config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: bookworm gateway install")
            else:
                print_info("  You can install later: bookworm gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo bookworm gateway install --system")
                print_info("  Or run in foreground:  bookworm gateway")
        else:
            from bwm_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   bookworm gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   bookworm gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `bookworm setup tools` and `bookworm tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from bwm_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_bookwormpro migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_bookwormpro", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and BookwormPRO.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "[警告] Gateway/messaging — this will configure BookwormPRO to use your OpenClaw messaging channels",
    "telegram": "[警告] Telegram — this will point BookwormPRO at your OpenClaw Telegram bot",
    "slack": "[警告] Slack — this will point BookwormPRO at your OpenClaw Slack workspace",
    "discord": "[警告] Discord — this will point BookwormPRO at your OpenClaw Discord bot",
    "whatsapp": "[警告] WhatsApp — this will point BookwormPRO at your OpenClaw WhatsApp connection",
    "config": "[警告] Config values — OpenClaw settings may not map 1:1 to BookwormPRO equivalents",
    "soul": "[警告] Instruction file — may contain OpenClaw-specific setup/restart procedures",
    "memory": "[警告] Memory/context file — may reference OpenClaw-specific infrastructure",
    "context": "[警告] Context file — may contain OpenClaw-specific instructions",
}
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("BookwormPRO can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: bookworm claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: bookworm claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing BookwormPRO configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing BookwormPRO config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in BookwormPRO (use bookworm claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      bookworm setup           — full or quick (auto-detected)
      bookworm setup model     — just model/provider
      bookworm setup tts       — just text-to-speech
      bookworm setup terminal  — just terminal backend
      bookworm setup gateway   — just messaging platforms
      bookworm setup tools     — just tool configuration
      bookworm setup agent     — just agent settings
    """
    ...
    print(
        color(
            "│             [BWM] BookwormPRO Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your BookwormPRO installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have BookwormPRO configured.")
        print()
        ...
        if choice == 0:
            ...
            print_info("Exiting. Run 'bookworm setup' again when ready.")
            return
        elif 2 <= choice <= 6:
        ...
        setup_mode = prompt_choice("How would you like to set up BookwormPRO?", [
    ...
    print()
    print_info("You can edit these files directly or use 'bookworm config edit'")
...
def _resolve_hermes_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``bookworm chat`` in a fresh process."""
    ...
    try:
        if importlib.util.find_spec("bwm_cli") is not None:
            return [sys.executable, "-m", "bwm_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    if not chat_argv:
        print_info("Could not relaunch BookwormPRO automatically. Run 'bookworm chat' manually.")
        return
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'bookworm setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    bookworm setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  bookworm setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'bookworm setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect BookwormPRO to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'bookworm setup gateway'.")
        ...
        for idx in selected_indices:
            ...
            emoji = {"Telegram": "📱", "Discord": "[对话]", "Slack": "💼"}.get(plat, "")
            print()

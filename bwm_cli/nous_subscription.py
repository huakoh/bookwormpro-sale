"""Helpers for BookwormPRO subscription managed-tool capabilities."""
...
from bwm_cli.config import get_env_value, load_config
...
def get_nous_subscription_features(
    ...
    provider_is_nous = str(model_cfg.get("provider") or "").strip().lower() == "bookwormpro"
    ...
    # Tool Gateway via `bookworm model`, so direct credentials should NOT
    ...
    features = {
        ...
        "image_gen": NousFeatureState(
            ...
            current_provider="FAL" if direct_fal else ("BookwormPRO Subscription" if image_managed else ""),
...
def get_gateway_eligible_tools(
    ...
    All lists are empty when the user is not a paid BookwormPRO subscriber or
    is not using BookwormPRO as their provider.
    """
    if not managed_nous_tools_enabled():
        return [], [], []
    ...
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "bookwormpro":
        return [], [], []
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from bwm_cli.setup import prompt_choice
    except Exception:
        return set()
    ...
    if already_managed:
        for k in already_managed:
            desc_parts.append(f"  [成功] {_GATEWAY_TOOL_LABELS[k]} — using Tool Gateway")
    if unconfigured:

...
- ``bwm_cli.providers``        -- canonical provider identity + overlays
- ``bwm_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    "BookwormPRO Project BookwormPRO 3 & 4 models are NOT agentic and are not designed "
    "for use with BookwormPRO. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real BookwormPRO Project BookwormPRO 3 / BookwormPRO 4 chat families.
# The previous substring check (`"bookworm" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``bookworm-brain:qwen3-14b-ctx16k`` that just
# happen to carry "bookworm" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   BookwormPRO/BookwormPRO-3-Llama-3.1-70B, bookworm-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   bookworm-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real BookwormPRO BookwormPRO 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a BookwormPRO BookwormPRO 3/4 chat model."""
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "bookwormpro")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "bookwormpro")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, BookwormPRO, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'bookworm model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'bookworm doctor' — config issues detected:"
    ...
    # Mirrors the same logic in bwm_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from bwm_cli/models.py (OPENROUTER_MODELS,
    ...
    # Build curated model lists keyed by bookworm provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "bookwormpro" shares OpenRouter's curated list if not separately defined
    if "bookwormpro" not in curated:
        curated["bookwormpro"] = curated["openrouter"]
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from bwm_cli.models import fetch_ollama_cloud_models
        curated["ollama-cloud"] = fetch_ollama_cloud_models()

    # --- 1. Check BookwormPRO-mapped providers ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (BOOKWORMPRO_OVERLAYS) with proper auth store checking.
        ...
        # show up in the picker without requiring a BookwormPRO release.
    ...
    # --- 2. Check BookwormPRO-only providers (bookwormpro, openai-codex, copilot, opencode-go) ---
    from bwm_cli.providers import BOOKWORMPRO_OVERLAYS
    from bwm_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → BookwormPRO provider ID.
    # BOOKWORMPRO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use BookwormPRO IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in BOOKWORMPRO_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve BookwormPRO slug — e.g. "github-copilot" → "copilot"
        ...
        if hermes_slug in {"copilot", "copilot-acp"}:
            ...
            # Use curated list — look up by BookwormPRO slug, fall back to overlay key
    ...
    # in PROVIDER_TO_MODELS_DEV or BOOKWORMPRO_OVERLAYS (keeps /model in sync
    # with `bookworm model`).
    try:
        from bwm_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is BookwormPRO's canonical write key (matches
            ...
            # BookwormPRO writes ``models:`` as a dict keyed by model id
            # (see bwm_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # BookwormPRO's own writer uses; a hyphen variant is accepted
            ...
            # active model. BookwormPRO's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, bwm_cli/config.py) already consume that dict.

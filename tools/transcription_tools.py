...
LOCAL_STT_COMMAND_ENV = "BOOKWORMPRO_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "BOOKWORMPRO_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set BOOKWORMPRO_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="bookworm-local-stt-") as output_dir:
            ...
            # SECURITY: shell=True retained — command template sourced from admin env var
            # (BOOKWORMPRO_LOCAL_STT_COMMAND), which may contain shell pipe syntax (e.g. "cmd1 | cmd2").
            # All four interpolated values ({input_path}, {output_dir}, {language}, {model})
            # are wrapped with shlex.quote() above, neutralising shell metacharacter injection.
            # End-users cannot influence the template string itself; only admins can set the env var.
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = str(
        ...
        or os.getenv("BOOKWORMPRO_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()

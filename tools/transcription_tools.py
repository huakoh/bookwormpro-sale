...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="bookworm-local-stt-") as output_dir:
            ...
            # Parse the user-supplied template into tokens, then substitute
            # placeholders so we can invoke the command safely without shell=True.
            try:
                ...
                return {
                    "success": False,
                    "transcript": "",
                    "error": "Invalid local STT command template (cannot parse)",
                }
            command = [
                token.format(

...
Tracks per-skill usage metadata in a sidecar JSON file (~/.bookwormpro/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.bookwormpro/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.bookwormpro/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def _mutate(skill_name: str, mutator) -> None:
    ...
    if not skill_name:
        return
    try:
        # Record usage for ALL skills (removed bundled filter 2026-07-09).
        # Curator provenance checks are done at read-time, not write-time.
        data = load_usage()
        rec = data.get(skill_name)
        if not isinstance(rec, dict):
            rec = _empty_record()
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an agent-created skill directory to ~/.bookwormpro/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.bookwormpro/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.bookwormpro/skills/<skill>/SKILL.md) and category-nested
    (~/.bookwormpro/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None

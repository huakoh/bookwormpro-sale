...
Lets the LLM write a Python script that calls BookwormPRO tools via RPC,
...
"""Auto-generated BookwormPRO tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["BOOKWORMPRO_RPC_SOCKET"])
...
"""Auto-generated BookwormPRO tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("BOOKWORMPRO_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"BOOKWORMPRO_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("BOOKWORMPRO_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    to a subset of BookwormPRO tools.
    ...
    try:
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["BOOKWORMPRO_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the bookwormpro root is importable in the sandbox so
        ...
        # BOOKWORMPRO_TIMEZONE is an internal BookwormPRO setting and must not leak
        # into child processes.
        _tz_name = os.getenv("BOOKWORMPRO_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("BOOKWORMPRO_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {BOOKWORMPRO_HOME}/home/ when that directory exists.
        from bwm_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.bookwormpro/.env')).
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (bookwormpro's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``bookworm tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and bookwormpro's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.bookwormpro/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call BookwormPRO tools programmatically. "

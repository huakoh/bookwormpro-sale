...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.bookwormpro/.env``.
"""
...
def _managed_persistence_enabled() -> bool:
    """Return whether BookwormPRO-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the BookwormPRO profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if _managed_persistence_enabled():
            ...
            session = {
                "user_id": f"bookworm_{uuid.uuid4().hex[:10]}",
                "tab_id": None,

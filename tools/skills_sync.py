...
Copies bundled skills from the repo's skills/ directory into ~/.bookwormpro/skills/
...
The manifest lives at ~/.bookwormpro/skills/.bundled_manifest.
"""
...
def _get_bundled_dir() -> Path:
    ...
    Checks BOOKWORMPRO_BUNDLED_SKILLS env var first (set by Nix wrapper),
    then falls back to the relative path from this source file.
    """
    env_override = os.getenv("BOOKWORMPRO_BUNDLED_SKILLS")
    if env_override:
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.bookwormpro/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.bookwormpro/skills/ using the manifest.
            ...
            try:
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            f"  [警告] {skill_name}: bundled version shipped but you "
                            f"already have a local skill by this name — yours "
                            f"was kept. Run `bookworm skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `bookworm skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `bookworm update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.bookwormpro/skills/ ...")

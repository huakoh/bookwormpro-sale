...
# All skills live in ~/.bookwormpro/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
BOOKWORMPRO_HOME = get_hermes_home()
SKILLS_DIR = BOOKWORMPRO_HOME / "skills"


# ── BWR Alias Resolution (消歧自动纠错) ─────────────────────────────
_SKILL_ALIASES_CACHE: Optional[Dict[str, str]] = None

def _resolve_skill_alias(name: str) -> str:
    """Resolve skill aliases from skills-index.json.
    
    When a skill has been renamed or merged, this maps the old name
    to the new target. Returns the original name if no alias exists.
    """
    global _SKILL_ALIASES_CACHE
    if _SKILL_ALIASES_CACHE is None:
        index_path = SKILLS_DIR.parent / "skills-index.json"
        if index_path.exists():
            try:
                import json as _json
                data = _json.loads(index_path.read_text(encoding="utf-8"))
                _SKILL_ALIASES_CACHE = data.get("aliases", {})
            except Exception:
                _SKILL_ALIASES_CACHE = {}
        else:
            _SKILL_ALIASES_CACHE = {}
    
    resolved = _SKILL_ALIASES_CACHE.get(name)
    if resolved:
        logging.getLogger(__name__).debug(f"Skill alias resolved: {name} → {resolved}")
        return resolved
    return name


def _record_skill_route(name: str, confidence: float = 1.0, trace_id: str = ""):
    """Record a successful skill route for drift tracking."""
    try:
        from agent.bwr_drift import get_corrector
        get_corrector().record_route(name, confidence, trace_id=trace_id)
    except Exception:
        pass  # Non-critical — never break skill loading

def _record_skill_not_found(name: str, trace_id: str = ""):
    """Record a skill-not-found event for drift tracking."""
    try:
        from agent.bwr_drift import get_corrector
        get_corrector().record_skill_not_found(name, trace_id=trace_id)
    except Exception:
        pass
...
def _is_gateway_surface() -> bool:
    if os.getenv("BOOKWORMPRO_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("BOOKWORMPRO_SESSION_PLATFORM"))
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.bookwormpro/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``BOOKWORMPRO_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("BOOKWORMPRO_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``BOOKWORMPRO_PLATFORM`` environment variable
    3. ``BOOKWORMPRO_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from bwm_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("BOOKWORMPRO_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.bookwormpro/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``bookworm skills`` config UI). Default False
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from bwm_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: bookworm plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            ...
            from bwm_cli.plugins import discover_plugins, get_plugin_manager
        ...
        # ── Alias resolution (BWR 消歧自动纠错) ──────────────────
        name = _resolve_skill_alias(name)
        ...
        # \u53ef\u4fe1\u76ee\u5f55\u540c\u65f6\u7eb3\u5165 resolve(\u7a7f\u8f6f\u94fe) \u4e0e abspath(\u4e0d\u7a7f\u8f6f\u94fe) \u4e24\u79cd\u5f62\u5f0f\uff1a
        # \u6280\u80fd\u76ee\u5f55\u53ef\u80fd\u662f\u8de8\u7cfb\u7edf\u8f6f\u94fe(~/.bookwormpro/skills -> ~/.claude/skills)\uff0c
        # \u4ec5\u7528 resolve() \u4f1a\u628a\u5408\u6cd5\u8f6f\u94fe\u76ee\u6807\u8bef\u5224\u4e3a outside\u3002
        import os as _os
        def _trust_forms(_p):
            _forms = set()
            try:
                _forms.add(Path(_p).resolve())
            except Exception:
                pass
            try:
                _forms.add(Path(_os.path.abspath(_p)))
            except Exception:
                pass
            return _forms
        _trusted_dirs = set()
        _trusted_dirs |= _trust_forms(SKILLS_DIR)
        try:
            for _d in all_dirs[1:]:
                _trusted_dirs |= _trust_forms(_d)
        except Exception:
            pass
        _trusted_dirs = list(_trusted_dirs)
        # skill_md \u4e5f\u540c\u65f6\u7528 resolve(\u7a7f\u8f6f\u94fe) \u4e0e abspath(\u4e0d\u7a7f\u8f6f\u94fe) \u4e24\u79cd\u5f62\u5f0f\u6bd4\u8f83\uff0c
        # \u4efb\u4e00\u5f62\u5f0f\u843d\u5728\u4efb\u4e00\u53ef\u4fe1\u76ee\u5f55\u5185\u5373\u89c6\u4e3a inside\u3002
        _skill_md_forms = []
        try:
            _skill_md_forms.append(skill_md.resolve())
        except Exception:
            pass
        try:
            _skill_md_forms.append(Path(_os.path.abspath(skill_md)))
        except Exception:
            pass
        for _td in _trusted_dirs:
            _matched = False
            for _smf in _skill_md_forms:
                try:
                    _smf.relative_to(_td)
                    _matched = True
                    break
                except ValueError:
                    continue
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.bookwormpro/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `bookworm skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.bookwormpro.* first (agentskills.io convention), fall back to top-level
        hermes_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("bookworm", {}) or {}
...
if __name__ == "__main__":
    ...
    print("[最终] Skills Tool Test")
    print("=" * 60)

    # Test listing skills
    print("\n[汇总] Listing all skills:")
    ...
    print("\n[文档] Viewing reference file 'axolotl/references/dataset-formats.md':")

...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.bookwormpro/.env values without failing Docker command execution."""
    try:
        from bwm_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``BOOKWORMPRO_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("BOOKWORMPRO_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using BOOKWORMPRO_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'bookworm'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.bookwormpro/sandboxes/). Non-persistent
        ...
        container_name = f"bookworm-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic BookwormPRO secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def cleanup(self):
        ...
        if self._container_id:
            # Capture locals so the thread closure doesn't race with self._container_id = None
            _docker_exe = self._docker_exe
            _container_id = self._container_id
            try:
                # Stop in background so cleanup doesn't block.
                # On failure, fall back to rm -f (matches original || logic).
                def _stop_then_rm():
                    result = subprocess.run(
                        [_docker_exe, "stop", _container_id],
                        capture_output=True, timeout=60
                    )
                    if result.returncode != 0:
                        subprocess.run(
                            [_docker_exe, "rm", "-f", _container_id],
                            capture_output=True, timeout=10
                        )
                threading.Thread(target=_stop_then_rm, daemon=True).start()
            except Exception as e:
                logger.warning("Failed to stop container %s: %s", _container_id, e)

            if not self._persistent:
                # Schedule removal after 3 s so the stop has time to complete
                def _deferred_rm():
                ...
                threading.Thread(target=_deferred_rm, daemon=True).start()

...
_FORCE_SYNC_ENV = "BOOKWORMPRO_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.bookwormpro") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.bookwormpro to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.bookwormpro", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``BOOKWORMPRO_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.bookwormpro/`` directory as a tar archive,
        ...
        # against an uninitialized remote .bookwormpro/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="bookworm-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.bookwormpro/skills/a.md`` →
        ``~/.bookwormpro/skills/a.md``, a new remote file at
        ``/root/.bookwormpro/skills/b.md`` maps to ``~/.bookwormpro/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:

...
from bwm_cli._subprocess_compat import windows_hide_flags
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (BookwormPRO convention — don't recursively snapshot siblings)
    ".worktrees/",
    ...
    "*.rar",
    "*.iso",
    # Secrets (BookwormPRO 安全基线强化: 绝不快照凭证/密钥/审计DB)
    ".env",
    ".env.*",
    ".env.local",
    ".env.*.local",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("BOOKWORMPRO_CHECKPOINT_TIMEOUT", 30)))
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "bookwormpro@local"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``BOOKWORMPRO_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at BOOKWORMPRO_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "BOOKWORMPRO_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "BOOKWORMPRO_WORKDIR"
            if wd_marker.exists():
                try:

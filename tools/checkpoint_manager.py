...
Architecture:
    ~/.bookwormpro/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        BOOKWORMPRO_WORKDIR                               — original dir path
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("BOOKWORMPRO_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal BookwormPRO infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "bookworm@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "BookwormPRO Checkpoint"], shadow_repo, working_dir)

...
discovers their tools, and registers them into the bookwormpro tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.bookwormpro/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.bookwormpro/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "BOOKWORMPRO_HOME", os.path.join(os.path.expanduser("~"), ".bookwormpro")
                )
            )
            candidates = [
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ~/.bookwormpro/logs/mcp-stderr.log.
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `bookworm mcp login {server_name}` (or delete the tokens "
            f"file under ~/.bookwormpro/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the BookwormPRO config file.
    ...
    ``os.environ`` (which includes ``~/.bookwormpro/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves BookwormPRO's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the BookwormPRO registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``bookworm tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the BookwormPRO registry.
...
def _kill_orphaned_mcp_children() -> None:
    ...
    This prevents shared-resource collisions when multiple bookworm processes

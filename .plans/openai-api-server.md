# OpenAI-Compatible API Server for BookwormPRO
...
SSE streaming. By exposing this endpoint, bookwormpro becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  bookwormpro   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.bookwormpro/.env`
2. Run `bookworm gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with bookwormpro through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns bookwormpro as a model) |
...
Authorization: Bearer bookworm-api-key-here
...
But bookwormpro sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "bookwormpro"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured bookwormpro model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of bookwormpro's core prompt
...
Once implemented, bookwormpro works as a drop-in backend for:

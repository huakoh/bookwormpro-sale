"""Centralized logging setup for BookwormPRO.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.bookwormpro/logs/`` (profile-aware via ``get_hermes_home()``).
...
from bwm_constants import get_config_path, get_hermes_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_bwm_session_injector", False):
    ...
    _session_record_factory._bwm_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``bookworm logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the BookwormPRO logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the BookwormPRO home directory.  Falls back to
    ...
    max_bytes = (max_size_mb or cfg_max_size or 2) * 1024 * 1024
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_bwm_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._bwm_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the bookworm group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from bwm_cli.config import is_managed

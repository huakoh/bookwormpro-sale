...
Checklist for integrating a new messaging platform into the BookwormPRO gateway.
...
And add it to the `bookworm-gateway` composite:

```python
"bookworm-gateway": {
    "includes": [..., "bookworm-your-platform"]
}
```
...
## 12. Status Display (`bwm_cli/status.py`)
...
## 13. Gateway Setup Wizard (`bwm_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ bwm_cli/ toolsets.py \
  --include="*.py" -l | sort -u

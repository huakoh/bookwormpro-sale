...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-BookwormPRO-Session-Id header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists bookwormpro as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to bookwormpro
...
def _derive_chat_session_id(
    ...
    the same BookwormPRO session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through bookwormpro's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``bookworm sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the bookworm-api-server default.
        """
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "bookwormpro"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return bookwormpro as an available model."""
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to continue an existing session by passing X-BookwormPRO-Session-Id.
        ...
        provided_session_id = request.headers.get("X-BookwormPRO-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-BookwormPRO-Session-Id rejected: "
            ...
            # conversation map to the same BookwormPRO session.  The first user
        ...
        if stream:
            ...
            def _on_tool_progress(event_type, name, preview, args, **kwargs):
                ...
                ``event: bookworm.tool.progress`` line that compliant frontends
        ...
        return web.json_response(response_data, headers={"X-BookwormPRO-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-BookwormPRO-Session-Id"] = session_id
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: bookworm.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: bookworm.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from bwm_cli.auth import has_usable_secret
            ...
            if not self._api_key:
                logger.warning(
                    "[%s] [警告]  No API key configured (API_SERVER_KEY / platforms.api_server.key). "

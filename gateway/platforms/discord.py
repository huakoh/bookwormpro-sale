...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("BOOKWORMPRO_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("BOOKWORMPRO_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields BookwormPRO manages."""
    ...
    async def on_processing_complete(self, event: MessageEvent, outcome: ProcessingOutcome) -> None:
        ...
        if hasattr(message, "add_reaction"):
            ...
            if outcome == ProcessingOutcome.SUCCESS:
                await self._add_reaction(message, "[成功]")
            elif outcome == ProcessingOutcome.FAILURE:
                await self._add_reaction(message, "[失败]")
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your BookwormPRO session")
        ...
        @tree.command(name="status", description="Show BookwormPRO session status")
        ...
        @tree.command(name="stop", description="Stop the running BookwormPRO agent")
        ...
        @tree.command(name="update", description="Update BookwormPRO to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the BookwormPRO gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a BookwormPRO session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to BookwormPRO in the thread",
        ...
        # bwm_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from bwm_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from bwm_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        """
        try:
            from bwm_cli.commands import discord_skill_commands_by_category
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a BookwormPRO skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new BookwormPRO session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by BookwormPRO: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "BookwormPRO"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by BookwormPRO: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            embed = discord.Embed(
                title="[警告] Command Approval Required",
                description=f"```\n{cmd_display}\n```",
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``bookworm update --gateway``
        ...
        try:
            ...
            embed = discord.Embed(
                title="[BWM] Update Needs Your Input",
...
if DISCORD_AVAILABLE:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``bookworm update`` prompts.
        ...
        @discord.ui.button(label="Yes", style=discord.ButtonStyle.green, emoji="[成功]")
        async def yes_btn(
            self, interaction: discord.Interaction, button: discord.ui.Button
        ):
            await self._respond(interaction, "y", discord.Color.green(), "Yes")

        @discord.ui.button(label="No", style=discord.ButtonStyle.red, emoji="[失败]")
        async def no_btn(
            self, interaction: discord.Interaction, button: discord.ui.Button
        ):

# Contributing to BookwormPRO

Thank you for contributing to BookwormPRO! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want BookwormPRO to work everywhere.
...
Bundled skills (in `skills/`) ship with every BookwormPRO install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `bookworm skills browse` (labeled "official") and install it with `bookworm skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [BookwormPRO Project Discord](https://discord.gg/BookwormPRO). Users can install it with `bookworm skills install`.
...
```bash
git clone --recurse-submodules https://github.com/huakoh/BookwormPRO.git
cd bookwormpro
...
```bash
mkdir -p ~/.bookwormpro/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.bookwormpro/config.yaml
touch ~/.bookwormpro/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.bookwormpro/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/bookworm" ~/.local/bin/bookworm

# Verify
bookworm doctor
bookworm chat -q "Hello"
```
...
├── cli.py                    # BookwormCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (bookworm-cli, bookworm-telegram, etc.)
├── bwm_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── bwm_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, BookwormPRO Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.bookwormpro/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (bookwormpro.local)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.bookwormpro/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.bookwormpro/`)

| Path | Purpose |
|------|---------|
| `~/.bookwormpro/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.bookwormpro/.env` | API keys and secrets |
| `~/.bookwormpro/auth.json` | OAuth credentials (BookwormPRO Portal) |
| `~/.bookwormpro/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.bookwormpro/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.bookwormpro/state.db` | SQLite session database |
| `~/.bookwormpro/sessions/` | JSON session logs |
| `~/.bookwormpro/cron/` | Scheduled job data |
| `~/.bookwormpro/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`bwm_state.py`) with full-text search and unique session titles. JSON logs go to `~/.bookwormpro/sessions/`.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (BookwormPRO Portal OAuth, OpenRouter API key, or custom endpoint).
...
Four fields are supported under `metadata.bookwormpro`:
...
The user may skip setup and keep loading the skill. BookwormPRO only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `bookworm setup` or update `~/.bookwormpro/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing BookwormPRO tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `bookworm --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

BookwormPRO uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.bookwormpro/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `bwm_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `bwm_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

BookwormPRO runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
BookwormPRO has terminal access. Security matters.
...
2. **Test manually**: Run `bookworm` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/huakoh/BookwormPRO/issues)
- Include: OS, Python version, BookwormPRO version (`bookworm version`), full error traceback
...
- **Discord**: [discord.gg/BookwormPRO](https://discord.gg/BookwormPRO) — for questions, showcasing projects, and sharing skills

"""ACP session manager — maps ACP sessions to BookwormPRO AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.bookwormpro/state.db``) so they
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["bookworm-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed BookwormPRO agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by BookwormPRO AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current BookwormPRO runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.bookwormpro/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``BOOKWORMPRO_HOME`` dynamically rather than relying on

...
This tutorial walks you through setting up a Telegram bot powered by BookwormPRO that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **BookwormPRO installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.bookwormpro/.env`

:::tip
A $5/month VPS is plenty for running the gateway. BookwormPRO itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team BookwormPRO Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by BookwormPRO. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.bookwormpro/.env`:
...
```
[Gateway] Starting BookwormPRO Gateway...
...
bookworm gateway stop
bookworm gateway status

# View live logs
journalctl --user -u bookworm-gateway -f
...
bookworm gateway start
bookworm gateway stop
tail -f ~/.bookwormpro/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `bookworm gateway install` to update the plist.
:::
...
```bash
bookworm gateway stop && bookworm gateway start
```
...
bookworm pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.bookwormpro/.env`:
...
Control how much detail the bot shows when using tools. In `~/.bookwormpro/config.yaml`:
...
Customize how the bot communicates by editing `~/.bookwormpro/SOUL.md`:

For a full guide, see [Use SOUL.md with BookwormPRO](/docs/guides/use-soul-with-bookworm).
...
```bash
# From the CLI
bookworm cron list          # View all scheduled jobs
bookworm cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u bookworm-gateway` (Linux) or `~/.bookwormpro/logs/gateway.log` (macOS) |
| Cron job output | `~/.bookwormpro/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.bookwormpro/cron/jobs.json` |
| Pairing data | `~/.bookwormpro/pairing/` |
| Session history | `~/.bookwormpro/sessions/` |

...
description: "Diagnose and fix common BookwormPRO cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`bookworm gateway` or `bookworm serve`). For one-off debugging, you can manually trigger a tick with `bookworm cron tick`.
...
```bash
date
bookworm cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.bookwormpro/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.bookwormpro/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.bookwormpro/.env` |
...
| `local` | Write access to `~/.bookwormpro/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `bookworm cron list` for updated `last_error` field (if available).
...
```bash
bookworm skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `bookworm skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `bookworm skills list`.
...
2. `~/.bookwormpro/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `bookworm cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the BookwormPRO config directory). Verify:
```bash
ls ~/.bookwormpro/scripts/your-script.py   # Must exist
bookworm cron edit <job_id> --script ~/.bookwormpro/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `bookworm skills install <skill-name>`.
...
```bash
ps aux | grep bookworm
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.bookwormpro/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.bookwormpro/cron/jobs.json
chmod 600 ~/.bookwormpro/cron/jobs.json   # Your user should own it
```
...
```bash
bookworm cron list                    # Show all jobs, states, next_run times
bookworm cron run <job_id>            # Schedule for next tick (for testing)
bookworm cron edit <job_id>           # Fix configuration issues
bookworm logs                         # View recent BookwormPRO logs
bookworm skills list                  # Verify installed skills
```
...
1. Run the job with `bookworm cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.bookwormpro/logs/agent.log` for scheduler messages and `~/.bookwormpro/logs/errors.log` for warnings
3. Open an issue at [github.com/huakoh/BookwormPRO](https://github.com/huakoh/BookwormPRO) with:

...
Copy-paste recipes for common automation patterns. Each template uses BookwormPRO's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`bookworm webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `bookworm webhook subscribe`) |
...
```bash
bookworm cron create "0 2 * * *" \
  "You are a project manager triaging the huakoh/BookwormPRO GitHub repo.

1. Run: gh issue list --repo huakoh/BookwormPRO --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
bookworm webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
bookworm cron create "0 9 * * 1" \
  "Scan the huakoh/BookwormPRO repo for documentation drift.

1. Run: gh pr list --repo huakoh/BookwormPRO --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (bwm_cli/commands.py, bwm_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (bwm_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
bookworm cron create "0 6 * * *" \
  "Run a dependency security audit on the bookwormpro project.

1. cd ~/.bookwormpro/bookwormpro && source .venv/bin/activate
...
```bash
bookworm webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
bookworm webhook subscribe alert-triage \
...
```python title="~/.bookwormpro/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "BookwormPRO-Monitor/1.0"})
...
```bash
bookworm cron create "every 30m" \
  ...
  --script ~/.bookwormpro/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
bookworm cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to BookwormPRO development." \
...
```bash
bookworm webhook subscribe github-issues \
  --events "issues" \
...
```bash
bookworm webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
bookworm webhook subscribe stripe-payments \
...
```bash
bookworm cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the bookwormpro codebase.

---
sidebar_position: 6
title: "Use MCP with BookwormPRO"
description: "A practical guide to connecting MCP servers to BookwormPRO, filtering their tools, and using them safely in real workflows"
---

# Use MCP with BookwormPRO

This guide shows how to actually use MCP with BookwormPRO in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native BookwormPRO tool
- you want BookwormPRO to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect BookwormPRO to internal APIs, databases, or company systems without modifying BookwormPRO core

Do not use MCP when:
- a built-in BookwormPRO tool already solves the job well
...
- BookwormPRO remains the agent
- MCP servers contribute tools
- BookwormPRO discovers those tools at startup or reload time
...
If you installed BookwormPRO with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- BookwormPRO banner/status should show MCP integration when configured
- ask BookwormPRO what tools it has available
...
There are two categories of MCP-exposed functionality in BookwormPRO:
...
2. BookwormPRO-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So BookwormPRO will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want BookwormPRO to reason over a bounded workspace.
...
Now BookwormPRO can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing BookwormPRO core.
...
Because BookwormPRO now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.

...
description: "Run BookwormPRO directly on an Android phone with Termux"
---

# BookwormPRO on Android with Termux

This is the tested path for running BookwormPRO directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop BookwormPRO from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

BookwormPRO now ships a Termux-aware installer path:
...
- links `bookworm` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `bookworm` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/bookworm" "$PREFIX/bin/bookworm"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `bookworm` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.bookwormpro/.env`.
...
### `bookworm doctor` says ripgrep or Node is missing

...
description: "Build wrapper CLIs that extend the BookwormPRO TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

BookwormPRO exposes protected extension hooks on `BookwormCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends BookwormPRO."""

from cli import BookwormCLI
from prompt_toolkit.layout import FormattedTextControl, Window
from prompt_toolkit.filters import Condition


class MyCLI(BookwormCLI):
    ...
    def _get_extra_tui_widgets(self):
        ...
        return [
            Window(
                FormattedTextControl(lambda: "[状态] My custom panel content"),
                height=1,
...
Called after BookwormPRO registers its own keybindings and before the layout is built. Add your keybindings to `kb`.

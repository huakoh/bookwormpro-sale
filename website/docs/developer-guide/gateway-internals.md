...
The messaging gateway is the long-running process that connects BookwormPRO to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `bwm_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
if _quick_key in self._running_agents:
    if canonical == "model":
        return "[等待] Agent is running — wait for it to finish or /stop first."
```
...
|--------|-----------------|
| `~/.bookwormpro/.env` | API keys, bot tokens, platform credentials |
| `~/.bookwormpro/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.bookwormpro/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `bookworm gateway start` / `bookworm gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.bookwormpro/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `bookworm gateway stop` stops only the current profile's gateway. `bookworm gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).

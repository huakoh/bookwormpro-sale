...
This guide covers adding a new messaging platform to the BookwormPRO gateway. A platform adapter connects BookwormPRO to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
1. **`bwm_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`bwm_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`bwm_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`bwm_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`bwm_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`bwm_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"bookworm-newplat"` toolset definition with `_HERMES_CORE_TOOLS`
2. **`toolsets.py`** — Add `"bookworm-newplat"` to the `"bookworm-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | bookworm-newplat toolset |

...
description: "How BookwormPRO stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `bwm_cli/cron.py` | CLI `bookworm cron` subcommands |
...
Jobs are stored in `~/.bookwormpro/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `bookworm cron` commands are run or during active CLI sessions.
...
```python
# ~/.bookwormpro/scripts/check_competitors.py
...
2. **Environment variable** — `BOOKWORMPRO_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.bookwormpro/cron/output/` |
...
The `bookworm cron` CLI provides direct job management:

```bash
bookworm cron list                    # Show all jobs
bookworm cron create                  # Interactive job creation (alias: add)
bookworm cron edit <job_id>           # Edit job configuration
bookworm cron pause <job_id>          # Pause a running job
bookworm cron resume <job_id>         # Resume a paused job
bookworm cron run <job_id>            # Trigger immediate execution
bookworm cron remove <job_id>         # Delete a job
```

...
sidebar_position: 1
title: "Architecture"
description: "BookwormPRO internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of BookwormPRO internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ bwm_state.py   │              │ Browser (5 backends)  │
...
├── bwm_constants.py       # BOOKWORMPRO_HOME, profile-aware paths
...
│
├── bwm_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `bookworm` subcommands (~6,000 lines)
...
│   ├── skills_config.py      # bookworm skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # bookworm tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # bookworm gateway start/stop
│
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .bookwormpro.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.bookwormpro/plugins/` (user), `.bookwormpro/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `bookworm plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-bookworm-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes BookwormPRO as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`bookworm -p <name>`) gets its own BOOKWORMPRO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

...
BookwormPRO uses a SQLite database (`~/.bookwormpro/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `bwm_state.py`


## Architecture Overview

```
~/.bookwormpro/state.db (SQLite, WAL mode)
...
Multiple bookworm processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.bookwormpro/state.db
...
Default path: `~/.bookwormpro/state.db`

This is derived from `bwm_constants.get_hermes_home()` which resolves to
`~/.bookwormpro/` by default, or the value of `BOOKWORMPRO_HOME` environment variable.

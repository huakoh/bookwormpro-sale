...
description: "How BookwormPRO builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

BookwormPRO deliberately separates:
...
1. agent identity — `SOUL.md` from `BOOKWORMPRO_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.bookwormpro/SOUL.md)
You are BookwormPRO, an AI assistant created by BookwormPRO Project.
...
`SOUL.md` lives at `~/.bookwormpro/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are BookwormPRO, an intelligent AI assistant created by BookwormPRO Project.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .bookwormpro.md / BOOKWORMPRO.md (walks to git root)
    ...
    # SOUL.md from BOOKWORMPRO_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.bookwormpro.md`, `BOOKWORMPRO.md` | CWD up to git root | BookwormPRO-native project config |
...
- **YAML frontmatter stripped** — `.bookwormpro.md` frontmatter is removed (reserved for future config overrides)
...
1. `.bookwormpro.md` / `BOOKWORMPRO.md` (walks to git root)

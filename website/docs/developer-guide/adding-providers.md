...
description: "How to add a new inference provider to BookwormPRO — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

BookwormPRO can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `bookworm model` menu entries
...
1. `bwm_cli/auth.py` decides how credentials are found.
2. `bwm_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `bwm_cli/models.py` and `bwm_cli/main.py` make the provider show up in the CLI. (`bwm_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`bwm_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `bookworm setup`.
:::
...
- `PROVIDER_REGISTRY` in `bwm_cli/auth.py`
- `_PROVIDER_LABELS` in `bwm_cli/models.py`
- `_PROVIDER_ALIASES` in both `bwm_cli/auth.py` and `bwm_cli/models.py`
- CLI `--provider` choices in `bwm_cli/main.py`
...
## Step 2: Add auth metadata in `bwm_cli/auth.py`
...
- OAuth / auth-store path: BookwormPRO, OpenAI Codex

Questions to answer here:

- What env vars should BookwormPRO check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `bwm_cli/models.py`
...
## Step 4: Resolve runtime data in `bwm_cli/runtime_provider.py`
...
Be careful with API-key precedence. BookwormPRO already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `bwm_cli/main.py`

A provider is not discoverable until it shows up in the interactive `bookworm model` flow.

Update these in `bwm_cli/main.py`:
...
:::tip
`bwm_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `bookworm model` and `bookworm setup` automatically.
:::
...
When you add a native provider, double-check that BookwormPRO is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m bwm_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `bwm_cli/auth.py`
- [ ] aliases added in `bwm_cli/auth.py` and `bwm_cli/models.py`
- [ ] model catalog added in `bwm_cli/models.py`
- [ ] runtime branch added in `bwm_cli/runtime_provider.py`
- [ ] CLI wiring added in `bwm_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `bookworm model` but not `bookworm setup`

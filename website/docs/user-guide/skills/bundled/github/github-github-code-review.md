...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.bookwormpro/.env ] && grep -q "^GITHUB_TOKEN=" ~/.bookwormpro/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.bookwormpro/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from BookwormPRO\",
    \"comments\": [
...
```bash
source "${BOOKWORMPRO_HOME:-$HOME/.bookwormpro}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by BookwormPRO. Code looks clean — good test coverage, no security concerns."

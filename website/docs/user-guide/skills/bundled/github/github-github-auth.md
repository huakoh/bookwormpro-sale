...
- Give it a name like "bookwormpro"
- Select scopes:
...
- Give it a title like "bookwormpro-&lt;machine-name>"
...
elif [ -f ~/.bookwormpro/.env ] && grep -q "^GITHUB_TOKEN=" ~/.bookwormpro/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.bookwormpro/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"

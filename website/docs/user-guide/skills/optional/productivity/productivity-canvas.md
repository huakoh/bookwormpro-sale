...
| | |
|---|---|
| Source | Optional — install with `bookworm skills install official/productivity/canvas` |
...
4. Name the token (e.g., "BookwormPRO"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.bookwormpro/.env`:
...
```bash
CANVAS="python $BOOKWORMPRO_HOME/skills/productivity/canvas/scripts/canvas_api.py"

---
title: "Telephony — Give BookwormPRO phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give BookwormPRO phone capabilities without core tool changes"
---
...
Give BookwormPRO phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `bookworm skills install official/productivity/telephony` |
| Path | `optional-skills/productivity/telephony` |
| Version | `1.0.0` |
| Author | BookwormPRO Project |
| License | MIT |
...
This optional skill gives BookwormPRO practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.bookwormpro/.env`
...
It does **not** turn BookwormPRO into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to BookwormPRO memory
...
### 1) "I want BookwormPRO to own a real phone number"
Use **Twilio**.
...
- pairs well with BookwormPRO `text_to_speech` plus a public file host or tunnel
...
### `~/.bookwormpro/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.bookwormpro/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
bookworm skills search telephony
bookworm skills install official/productivity/telephony
```
...
Then save credentials into BookwormPRO:
...
3. Buy it and save it into `~/.bookwormpro/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is BookwormPRO calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing BookwormPRO's existing `text_to_speech` support.

Use this when:
- you want the call to use BookwormPRO's configured TTS voice rather than Twilio `<Say>`
...
Recommended BookwormPRO TTS -> Twilio Play workflow:

1. Generate the audio with BookwormPRO `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask BookwormPRO to create the message audio with `text_to_speech`
...
Important note:
- BookwormPRO TTS is great for prerecorded outbound messages
...
- BookwormPRO STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to BookwormPRO memory.
...
- Do not store arbitrary third-party phone numbers in BookwormPRO memory.
...
3. persist that number to `~/.bookwormpro/.env`

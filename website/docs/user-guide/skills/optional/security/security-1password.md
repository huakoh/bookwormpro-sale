...
| | |
|---|---|
| Source | Optional — install with `bookworm skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by BookwormPRO |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during BookwormPRO terminal calls (desktop app flow only)
...
### Service Account (recommended for BookwormPRO)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.bookwormpro/.env` (the skill will prompt for this on first load).
...
## BookwormPRO Execution Pattern (desktop app flow)

BookwormPRO terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/bookworm-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/bookworm-op.sock"

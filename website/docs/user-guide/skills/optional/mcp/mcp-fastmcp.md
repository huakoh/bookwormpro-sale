...
| | |
|---|---|
| Source | Optional — install with `bookworm skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into BookwormPRO or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to BookwormPRO. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.bookwormpro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.bookwormpro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is BookwormPRO integration, either:

- configure the server in `~/.bookwormpro/config.yaml` using the `native-mcp` skill, or
...
### BookwormPRO cannot see the deployed server

The server-building part may be correct while the BookwormPRO config is not. Load the `native-mcp` skill and configure the server in `~/.bookwormpro/config.yaml`, then restart BookwormPRO.

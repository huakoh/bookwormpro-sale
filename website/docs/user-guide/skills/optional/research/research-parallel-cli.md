...
| | |
|---|---|
| Source | Optional — install with `bookworm skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a BookwormPRO core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with BookwormPRO native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer BookwormPRO native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer BookwormPRO native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended BookwormPRO workflow:
...
## Recommended BookwormPRO usage patterns
...
- Do not silently choose Parallel when BookwormPRO native tools are already sufficient.

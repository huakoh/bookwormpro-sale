---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into BookwormPRO"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into BookwormPRO"
---
...
Migrate a user's OpenClaw customization footprint into BookwormPRO. Imports BookwormPRO-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `bookworm skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | BookwormPRO (BookwormPRO Project) |
| License | MIT |
| Tags | `Migration`, `OpenClaw`, `BookwormPRO`, `Memory`, `Persona`, `Import` |
| Related skills | [`bookwormpro`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-bookwormpro) |
...
# OpenClaw -> BookwormPRO Migration

Use this skill when a user wants to move their OpenClaw setup into BookwormPRO with minimal manual cleanup.
...
```bash
bookworm claw migrate              # Full interactive migration
...
bookworm claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `bookworm setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the BookwormPRO home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into BookwormPRO memory entries
- merge OpenClaw command approval patterns into BookwormPRO `command_allowlist`
- migrate BookwormPRO-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.bookwormpro/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen BookwormPRO workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.bookwormpro/tts/`
- archive non-secret docs that do not have a direct BookwormPRO destination
...
- `~/.bookwormpro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.bookwormpro/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.bookwormpro/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of BookwormPRO-compatible secrets, currently:
...
BookwormPRO CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in BookwormPRO. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a BookwormPRO workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing BookwormPRO skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.bookwormpro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.bookwormpro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.bookwormpro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.bookwormpro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.bookwormpro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.bookwormpro/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of BookwormPRO unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty BookwormPRO targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean BookwormPRO destination. Unsupported auth blobs must still be reported as skipped.
...
- BookwormPRO persona state imported
- BookwormPRO memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.bookwormpro/skills/openclaw-imports/`

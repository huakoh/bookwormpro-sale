...
description: "Configure and use Honcho memory with BookwormPRO -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with BookwormPRO -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `bookworm skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for BookwormPRO

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every BookwormPRO profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/bookworm#running-honcho-locally-with-bookworm

### Verify

```bash
bookworm honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this BookwormPRO profile's AI peer
...
Honcho models conversations as interactions between **peers**. BookwormPRO creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this BookwormPRO instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each BookwormPRO run |
| `global` | Single session across all directories |

Manual override: `bookworm honcho map my-project-name`
...
Each BookwormPRO profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block bookworm.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `bookworm.coder` host block in `honcho.json`
...
```bash
bookworm honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for BookwormPRO when Honcho memory is active.
...
Config file: `$BOOKWORMPRO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `bookworm honcho setup`. Ensure `memory.provider: honcho` is in `~/.bookwormpro/config.yaml`.

### Memory not persisting across sessions
Check `bookworm honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `bookworm profile create <name> --clone`. For existing profiles: `bookworm honcho sync`.
...
| Command | Description |
|---------|-------------|
| `bookworm honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `bookworm honcho status` | Show resolved config, connection test, peer info for active profile |
| `bookworm honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `bookworm honcho disable` | Disable Honcho for the active profile |
| `bookworm honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `bookworm honcho peers` | Show peer identities across all profiles |
| `bookworm honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `bookworm honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `bookworm honcho sessions` | List known directory-to-session-name mappings |
| `bookworm honcho map <name>` | Map current working directory to a Honcho session name |
| `bookworm honcho identity` | Seed AI peer identity or show both peer representations |
| `bookworm honcho sync` | Create host blocks for all BookwormPRO profiles that don't have one yet |
| `bookworm honcho migrate` | Step-by-step migration guide from OpenClaw native memory to BookwormPRO + Honcho |
| `bookworm memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `bookworm memory status` | Show active memory provider and config |
| `bookworm memory off` | Disable external memory provider |

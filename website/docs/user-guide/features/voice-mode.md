...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with BookwormPRO — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

BookwormPRO supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with BookwormPRO](/docs/guides/use-voice-mode-with-bookworm).

## Prerequisites

Before using voice features, make sure you have:

1. **BookwormPRO installed** — `pip install bookwormpro` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `bookworm model` or set your preferred provider credentials in `~/.bookwormpro/.env`
3. **A working base setup** — run `bookworm` to verify the agent responds to text before enabling voice

:::tip
The `~/.bookwormpro/` directory and default `config.yaml` are created automatically the first time you run `bookworm`. You only need to create `~/.bookwormpro/.env` manually for API keys.
:::
...
pip install "bookwormpro[messaging]"

# Premium TTS (ElevenLabs)
pip install "bookwormpro[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "bookwormpro[all]"
```
...
```bash
bookworm                # Start the interactive CLI
```
...
1. Start the CLI with `bookworm` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.bookwormpro/config.yaml` (default: `ctrl+b`).
:::
...
```bash
bookworm gateway        # Start the gateway (connects to configured platforms)
bookworm gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.bookwormpro/.env`:
```bash
...
```bash
bookworm gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.bookwormpro/.env`
...
- Review gateway logs: `tail -f ~/.bookwormpro/logs/gateway.log`

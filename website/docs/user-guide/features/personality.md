...
description: "Customize BookwormPRO's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

BookwormPRO's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `BOOKWORMPRO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who BookwormPRO is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

BookwormPRO now seeds a default `SOUL.md` automatically in:

```text
~/.bookwormpro/SOUL.md
```

More precisely, it uses the current instance's `BOOKWORMPRO_HOME`, so if you run BookwormPRO with a custom home directory, it will use:

```text
$BOOKWORMPRO_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- BookwormPRO creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- BookwormPRO loads `SOUL.md` only from `BOOKWORMPRO_HOME`
- BookwormPRO does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, BookwormPRO falls back to a built-in default identity
...
If BookwormPRO loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `BOOKWORMPRO_HOME`, the personality belongs to the BookwormPRO instance itself.

That also makes it easier to teach users:
- "Edit `~/.bookwormpro/SOUL.md` to change BookwormPRO' default personality."
...
- how BookwormPRO should handle uncertainty, disagreement, or ambiguity
...
## What BookwormPRO injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, BookwormPRO falls back to a built-in default identity ("You are BookwormPRO, an intelligent AI assistant created by BookwormPRO Project..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
BookwormPRO ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain BookwormPRO, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives BookwormPRO its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.bookwormpro/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.bookwormpro/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-bookworm)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how BookwormPRO speaks
- `display.skin` and `/skin` affect how BookwormPRO looks in the terminal

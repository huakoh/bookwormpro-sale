...
Type `@` followed by a reference to inject content directly into your message. BookwormPRO expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- BookwormPRO env: `$BOOKWORMPRO_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$BOOKWORMPRO_HOME/skills/.hub/`

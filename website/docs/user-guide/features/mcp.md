...
description: "Connect BookwormPRO to external tool servers via MCP — and control exactly which MCP tools BookwormPRO loads"
---

# MCP (Model Context Protocol)

MCP lets BookwormPRO connect to external tool servers so the agent can use tools that live outside BookwormPRO itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted BookwormPRO to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native BookwormPRO tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want BookwormPRO to see
...
2. Add an MCP server to `~/.bookwormpro/config.yaml`:
...
4. Ask BookwormPRO to use the MCP-backed capability.
...
BookwormPRO will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints BookwormPRO connects to directly.
...
- you do not want BookwormPRO spawning a local subprocess for that integration

## Basic configuration reference

BookwormPRO reads MCP config from `~/.bookwormpro/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, BookwormPRO skips the server entirely |
...
## How BookwormPRO registers MCP tools

BookwormPRO prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — BookwormPRO sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, BookwormPRO also registers utility tools around MCP resources and prompts:
...
- BookwormPRO only registers resource utilities if the MCP session actually supports resource operations
- BookwormPRO only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to BookwormPRO, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, BookwormPRO skips the server completely and does not even attempt a connection.
...
You can also separately disable BookwormPRO-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, BookwormPRO does not create an empty runtime MCP toolset for that server.
...
BookwormPRO discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify BookwormPRO when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When BookwormPRO receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, BookwormPRO does not blindly pass your full shell environment.
...
Then verify your config and restart BookwormPRO.
...
Because BookwormPRO now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from BookwormPRO via the `sampling/createMessage` protocol. This allows an MCP server to ask BookwormPRO to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running BookwormPRO as an MCP server

In addition to connecting **to** MCP servers, BookwormPRO can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use BookwormPRO's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through BookwormPRO
- You want a single MCP server that bridges to all of BookwormPRO's connected messaging platforms at once
- You already have a running BookwormPRO gateway with connected platforms
...
Add BookwormPRO to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed BookwormPRO in a specific location:

```json
{
  "mcpServers": {
    "bookworm": {
      "command": "/home/user/.bookwormpro/bookwormpro/venv/bin/bookworm",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a BookwormPRO-specific channel browser:
...
The MCP server includes a live event bridge that polls BookwormPRO's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
bookworm mcp serve              # Normal mode
bookworm mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from BookwormPRO's session store (`~/.bookwormpro/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the BookwormPRO agent itself.

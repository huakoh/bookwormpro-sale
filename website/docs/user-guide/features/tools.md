...
description: "Overview of BookwormPRO's tools — what's available, how toolsets work, and terminal backends"
---
...
BookwormPRO ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
Paid [BookwormPRO Portal]() subscribers can use web search, image generation, TTS, and browser automation through the **[Tool Gateway](tool-gateway.md)** — no separate API keys needed. Run `bookworm model` to enable it, or configure individual tools with `bookworm tools`.
:::
...
bookworm chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `bookworm-cli`, `bookworm-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
```
```bash
# Set credentials in ~/.bookwormpro/.env
...
# Configure
bookworm config set terminal.backend singularity
bookworm config set terminal.singularity_image ~/python.sif
```
...
modal setup
bookworm config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.bookwormpro/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.bookwormpro/.env`.
:::

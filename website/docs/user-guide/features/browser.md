...
BookwormPRO includes a full browser automation toolset with multiple backend options:
...
:::tip BookwormPRO Subscribers
If you have a paid [BookwormPRO Portal]() subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. Run `bookworm model` or `bookworm tools` to enable it.
:::
...
Then set in `~/.bookwormpro/.env`:

```bash
CAMOFOX_URL=http://localhost:9377
```

Or configure via `bookworm tools` → Browser Automation → Camofox.
...
By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.bookwormpro/config.yaml`:
...
Then fully restart BookwormPRO so the new config is picked up.

:::warning Nested path matters
BookwormPRO reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# [失败] Wrong — BookwormPRO ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, BookwormPRO silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::

##### What BookwormPRO does
...
- Scopes the `userId` to the active BookwormPRO profile, so different BookwormPRO profiles get different browser profiles (profile isolation).

##### What BookwormPRO does not do
- It does not force persistence on the Camofox server. BookwormPRO only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), BookwormPRO cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start BookwormPRO and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted BookwormPRO after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

BookwormPRO derives the stable `userId` from the profile-scoped directory `~/.bookwormpro/browser_auth/camofox/` (or the equivalent under `$BOOKWORMPRO_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding BookwormPRO profile's state directory.

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. BookwormPRO automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chrome via CDP (`/browser connect`)

Instead of a cloud provider, you can attach BookwormPRO browser tools to your own running Chrome instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start BookwormPRO from the terminal (`bookworm` or `bookworm chat`) and issue `/browser connect` there.
:::
...
If Chrome isn't already running with remote debugging, BookwormPRO will attempt to auto-launch it with `--remote-debugging-port=9222`.
...
google-chrome \
  ...
  --user-data-dir=$HOME/.bookwormpro/chrome-debug \
  --no-first-run \
...
Then launch the BookwormPRO CLI and run `/browser connect`.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, BookwormPRO can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `bookworm config set toolsets '["bookworm-cli", "browser"]'`.
:::
...
```
Navigate to https://github.com/BookwormPRO
```
...
```
Type "bookworm agent" into the search field @e3
```
...
Screenshots are stored in `~/.bookwormpro/cache/screenshots/` and automatically cleaned up after 24 hours.
...
|---|---|---|
| Local Chrome via `/browser connect` or `browser.cdp_url` | [成功] | [成功] full workflow |
| Browserbase | [成功] | [成功] full workflow (via injected XHR bridge) |
| Camofox / default local agent-browser | [失败] | [失败] (no CDP endpoint) |
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.bookwormpro/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, BookwormPRO automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::

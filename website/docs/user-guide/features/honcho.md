...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of BookwormPRO's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple BookwormPRO instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
bookworm memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$BOOKWORMPRO_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `bookworm` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — BookwormPRO syncs them back at session init.
...
```bash
bookworm honcho status          # Connection status, config, and key settings
bookworm honcho setup           # Interactive setup wizard
bookworm honcho strategy        # Show or set session strategy
bookworm honcho peer            # Update peer names for multi-agent setups
bookworm honcho mode            # Show or set recall mode
bookworm honcho tokens          # Show or set context token budget
bookworm honcho identity        # Show Honcho peer identity
bookworm honcho sync            # Sync host blocks for all profiles
bookworm honcho enable          # Enable Honcho
bookworm honcho disable         # Disable Honcho
```

## Migrating from `bookworm honcho`

If you previously used the standalone `bookworm honcho setup`:
...
No re-login or re-setup needed. Run `bookworm memory setup` and select "honcho" — the wizard detects your existing config.

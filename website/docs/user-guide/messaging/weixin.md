...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect BookwormPRO to personal WeChat accounts via the iLink Bot API"
---

# Weixin (WeChat)

Connect BookwormPRO to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
- Terminal QR rendering is included when BookwormPRO is installed with the `messaging` extra
...
5. Save the account credentials automatically to `~/.bookwormpro/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.bookwormpro/.env`:
...
- **QR code login** — scan-to-connect setup via `bookworm gateway setup`
...
- Tokens are saved per account+peer to `~/.bookwormpro/weixin/accounts/<account_id>.context-tokens.json`
...
|----------|----------|---------|-------------|
| `WEIXIN_ACCOUNT_ID` | [成功] | — | iLink Bot account ID (from QR login) |
| `WEIXIN_TOKEN` | [成功] | — | iLink Bot token (auto-saved from QR login) |
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `bookworm gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `bookworm gateway setup` |
| `Another local BookwormPRO gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `bookworm gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install bookwormpro[messaging]`. Alternatively, open the URL printed above the QR |

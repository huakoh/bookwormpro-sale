...
sidebar_position: 4
title: "Slack"
description: "Set up BookwormPRO as a Slack bot using Socket Mode"
---

# Slack Setup

Connect BookwormPRO to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your BookwormPRO instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. BookwormPRO uses the modern
...
4. Enter an app name (e.g., "BookwormPRO") and select your workspace
5. Click **Create App**
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `bookworm-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a BookwormPRO configuration issue.
:::
...
BookwormPRO uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
bookworm gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
bookworm gateway              # Foreground
bookworm gateway install      # Install as a user service
sudo bookworm gateway install --system   # Linux only: boot-time system service
```
...
Understanding how BookwormPRO behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@BookwormPRO what time is it?`). In channels, BookwormPRO replies in a thread attached to that message. |
| **Threads** | If you @mention BookwormPRO inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.bookwormpro/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to BookwormPRO in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where BookwormPRO will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @BookwormPRO`).

---

## Multi-Workspace Support

BookwormPRO can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, BookwormPRO also loads tokens from an **OAuth token file** at:

```
~/.bookwormpro/slack_tokens.json
```
...
- When a message arrives, BookwormPRO uses the correct workspace-specific client to respond.
...
BookwormPRO supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @BookwormPRO` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @BookwormPRO` |
...
1. [成功] `message.channels` event is subscribed (for public channels)
2. [成功] `message.groups` event is subscribed (for private channels)
3. [成功] `app_mention` event is subscribed
4. [成功] `channels:history` scope is added (for public channels)
5. [成功] `groups:history` scope is added (for private channels)
6. [成功] App was **reinstalled** after adding scopes/events
7. [成功] Bot was **invited** to the channel (`/invite @BookwormPRO`)
8. [成功] You are **@mentioning** the bot in your message
...
- Tokens should be stored in `~/.bookwormpro/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your BookwormPRO config directory

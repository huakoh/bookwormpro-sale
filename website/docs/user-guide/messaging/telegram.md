...
sidebar_position: 1
title: "Telegram"
description: "Set up BookwormPRO as a Telegram bot"
---

# Telegram Setup

BookwormPRO integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "BookwormPRO") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_bookworm_bot`)
...
BookwormPRO uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
By default, BookwormPRO connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
You can also set it manually in `~/.bookwormpro/.env`:
...
Voice messages you send on Telegram are automatically transcribed by BookwormPRO's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running BookwormPRO — no API key required
...
BookwormPRO works in Telegram group chats with a few considerations:
...
- Use `telegram.ignored_threads` to keep BookwormPRO silent in specific Telegram forum topics, even when the group would otherwise allow free responses or mention-triggered replies
- If `telegram.require_mention` is left unset or false, BookwormPRO keeps the previous open-group behavior and responds to normal group messages it can see

### Example group trigger configuration

Add this to `~/.bookwormpro/config.yaml`:
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with BookwormPRO.
...
1. Open your private chat with the BookwormPRO bot in Telegram
2. Tap the bot's name at the top to open chat info
3. Enable **Topics** (the toggle to turn the chat into a forum)

Without this, BookwormPRO will log `The chat is not a forum` on startup and skip topic creation. This is a Telegram client-side setting — the bot cannot enable it programmatically.
:::

Add topics under `platforms.telegram.extra.dm_topics` in `~/.bookwormpro/config.yaml`:
...
1. On gateway startup, BookwormPRO calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.bookwormpro/config.yaml`:
...
1. When a message arrives in a mapped group topic, BookwormPRO looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | BookwormPRO creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
| Skill binding | [成功] | [成功] |
| Session isolation | [成功] | [成功] (already built-in for forum topics) |
...
When you send `/model` with no arguments in a Telegram chat, BookwormPRO shows an interactive inline keyboard for switching models:

1. **Provider selection** — buttons showing each available provider with model counts (e.g., "OpenAI (15)", "[成功] Anthropic (12)" for the current provider).
...
Or add it to `~/.bookwormpro/.env`:

```bash
HTTPS_PROXY=http://proxy.example.com:8080
```

The proxy applies to both the primary transport and all fallback IP transports. No additional BookwormPRO configuration is needed — if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that BookwormPRO uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
...
- [成功] when the response is delivered successfully
- [失败] if an error occurs during processing
...
:::note
Unlike Discord (where reactions are additive), Telegram's Bot API replaces all bot reactions in a single call. The transition from 👀 to [成功]/[失败] happens atomically — you won't see both at once.
:::
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `bookworm gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.bookwormpro/.env`. |
...
> [警告] This command is potentially dangerous (recursive delete). Reply "yes" to approve.

...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to BookwormPRO via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With BookwormPRO's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["bookwormpro<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to BookwormPRO's API server just like it would connect to OpenAI. Your agent handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

Open WebUI talks to BookwormPRO server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see your agent in the model dropdown (named after your profile, or **bookwormpro** for the default profile). Start chatting!
...
Your agent model should now appear in the model dropdown (named after your profile, or **bookwormpro** for the default profile).
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and BookwormPRO responds accordingly. Each request includes the full conversation history.
...
2. Edit your bookwormpro connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and BookwormPRO can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, BookwormPRO also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. BookwormPRO creates an AIAgent instance with its full toolset
3. The agent processes your request — it may call tools (terminal, file operations, web search, etc.)
4. As tools execute, **inline progress messages stream to the UI** so you can see what the agent is doing (e.g. `` `[系统] ls -la` ``, `` `[查找] Python 3.12 release` ``)
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | BookwormPRO's API URL (include `/v1`) |
...
- **Check model listing**: `curl http://localhost:8642/v1/models` should return a list with `bookwormpro`
...
BookwormPRO may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in BookwormPRO.

## Multi-User Setup with Profiles

To run separate BookwormPRO instances per user — each with their own config, memory, and skills — use [profiles](/docs/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.

### 1. Create profiles and configure API servers

```bash
bookworm profile create alice
bookworm -p alice config set API_SERVER_ENABLED true
bookworm -p alice config set API_SERVER_PORT 8643
bookworm -p alice config set API_SERVER_KEY alice-secret

bookworm profile create bob
bookworm -p bob config set API_SERVER_ENABLED true
bookworm -p bob config set API_SERVER_PORT 8644
bookworm -p bob config set API_SERVER_KEY bob-secret
```

### 2. Start each gateway

```bash
bookworm -p alice gateway &
bookworm -p bob gateway &
```
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated BookwormPRO agent.
...
```bash
bookworm -p alice config set API_SERVER_MODEL_NAME "Alice's Agent"
```
:::

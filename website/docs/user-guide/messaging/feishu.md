...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up BookwormPRO as a Feishu or Lark bot"
---

# Feishu / Lark Setup

BookwormPRO integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; BookwormPRO opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | BookwormPRO responds to every message. |
| Group chats | BookwormPRO responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. BookwormPRO will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `bookworm gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when BookwormPRO runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run BookwormPRO behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, BookwormPRO starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how BookwormPRO responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | BookwormPRO responds to @mentions from any user in any group. |
| `allowlist` | BookwormPRO only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | BookwormPRO ignores all group messages entirely. |
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, BookwormPRO reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.bookwormpro/feishu_comment_rules.json` (pairing grants in `~/.bookwormpro/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
|---------|---------|---------|
| Quiet period | `BOOKWORMPRO_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `BOOKWORMPRO_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `BOOKWORMPRO_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `BOOKWORMPRO_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.bookwormpro/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `BOOKWORMPRO_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |

## All Environment Variables

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `FEISHU_APP_ID` | [成功] | — | Feishu/Lark App ID |
| `FEISHU_APP_SECRET` | [成功] | — | Feishu/Lark App Secret |
...
| `BOOKWORMPRO_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `BOOKWORMPRO_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `BOOKWORMPRO_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `BOOKWORMPRO_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `BOOKWORMPRO_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `bookworm gateway setup` |
| `Another local BookwormPRO gateway is already using this Feishu app_id` | Only one BookwormPRO instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `bookworm-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.

...
Connect BookwormPRO to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between BookwormPRO and the BlueBubbles server
...
Or set environment variables directly in `~/.bookwormpro/.env`:
...
When someone messages your iMessage, BookwormPRO automatically sends them a pairing code. Approve it with:
```bash
bookworm pairing approve bluebubbles <CODE>
```
Use `bookworm pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.bookwormpro/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.bookwormpro/.env`):
```bash
...
BookwormPRO will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → BookwormPRO
BookwormPRO → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** BookwormPRO sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — BookwormPRO resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `bookworm logs gateway` for webhook errors (or `bookworm logs -f` to follow in real-time)

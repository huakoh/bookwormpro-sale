...
sidebar_position: 13
title: "Webhooks"
description: "Receive events from GitHub, GitLab, and other services to trigger BookwormPRO agent runs"
---

# Webhooks

Receive events from external services (GitHub, GitLab, JIRA, Stripe, etc.) and trigger BookwormPRO agent runs automatically. The webhook adapter runs an HTTP server that accepts POST requests, validates HMAC signatures, transforms payloads into agent prompts, and routes responses back to the source or to another configured platform.
...
1. Enable via `bookworm gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `bookworm webhook subscribe`
...
Add the `github-pr` route to your `~/.bookwormpro/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, BookwormPRO processes the event, and posts a review comment on the PR.
    ...
    extra:
      ...
      routes:
        antenna-matches:
          ...
          deliver: "telegram"
          deliver_only: true
          prompt: "[完成] New match: {match.user_name} matched with you!"
          deliver_extra:
...
```bash
bookworm webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `bookworm webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
bookworm webhook remove github-issues
```

### Test a subscription

```bash
bookworm webhook test github-issues
bookworm webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.bookwormpro/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `bookworm webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `bookworm gateway run`

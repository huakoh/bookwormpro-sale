...
BookwormPRO is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, BookwormPRO checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.bookwormpro/config.yaml`:
...
1. **CLI flag**: Start a session with `bookworm --yolo` or `bookworm chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `BOOKWORMPRO_YOLO_MODE=1`
...
> /yolo
  [警告] YOLO mode OFF — dangerous commands will require approval.
```

YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `BOOKWORMPRO_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.bookwormpro/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.bookwormpro/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.bookwormpro/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` bookworm/gateway | Self-termination prevention |
...
```
  [警告]  DANGEROUS COMMAND: recursive delete
...
The `BOOKWORMPRO_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.bookwormpro/config.yaml`:
...
:::tip
Use `bookworm config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, BookwormPRO controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.bookwormpro/.env`:
...
For more flexible authorization, BookwormPRO includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `bookworm pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.bookwormpro/config.yaml`:
...
bookworm pairing list

# Approve a pairing code
bookworm pairing approve telegram ABC12DEF

# Revoke a user's access
bookworm pairing revoke telegram 123456789

# Clear all pending codes
bookworm pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.bookwormpro/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, BookwormPRO applies strict security hardening to every container.
...
Container resources are configurable in `~/.bookwormpro/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.bookwormpro/sandboxes/docker/<task_id>/`
...
|---------|-----------|-------------------|----------|
| **local** | None — runs on host | [成功] Yes | Development, trusted users |
| **ssh** | Remote machine | [成功] Yes | Running on a separate server |
| **docker** | Container | [失败] Skipped (container is boundary) | Production gateway |
| **singularity** | Container | [失败] Skipped | HPC environments |
| **modal** | Cloud sandbox | [失败] Skipped | Scalable cloud isolation |
| **daytona** | Cloud sandbox | [失败] Skipped | Persistent cloud workspaces |
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `BOOKWORMPRO_HOME`. Skills declare these in frontmatter:
...
When loaded, BookwormPRO checks if these files exist in the active profile's `BOOKWORMPRO_HOME` and registers them for mounting:
...
Paths are relative to `~/.bookwormpro/`. Files are mounted to `/root/.bookwormpro/` inside the container.

### What Each Sandbox Filters

| Sandbox | Default Filter | Passthrough Override |
|---------|---------------|---------------------|
| **execute_code** | Blocks vars containing `KEY`, `TOKEN`, `SECRET`, `PASSWORD`, `CREDENTIAL`, `PASSWD`, `AUTH` in name; only allows safe-prefix vars through | [成功] Passthrough vars bypass both checks |
| **terminal** (local) | Blocks explicit BookwormPRO infrastructure vars (provider keys, gateway tokens, tool API keys) | [成功] Passthrough vars bypass the blocklist |
| **terminal** (Docker) | No host env vars by default | [成功] Passthrough vars + `docker_forward_env` forwarded via `-e` |
| **terminal** (Modal) | No host env/files by default | [成功] Credential files mounted; env passthrough via sync |
| **MCP** | Blocks everything except safe system vars + explicitly configured `env` | [失败] Not affected by passthrough (use MCP `env` config instead) |
...
- BookwormPRO infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
BookwormPRO integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.bookwormpro/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.bookwormpro/logs/` for unauthorized access attempts
10. **Keep updated** — run `bookworm update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.bookwormpro/.env

...
sidebar_position: 1
title: "CLI Interface"
description: "Master the BookwormPRO terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

BookwormPRO's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
BookwormPRO also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `bookworm --tui` — see the [TUI](tui.md) guide.
:::
...
bookworm chat --model "anthropic/claude-sonnet-4"

# With a specific provider
bookworm chat --provider bookwormpro        # Use BookwormPRO Portal
bookworm chat --provider openrouter  # Force OpenRouter
...
bookworm chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
bookworm --continue             # Resume the most recent CLI session (-c)
bookworm --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
bookworm chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
bookworm -w                         # Interactive mode in worktree
bookworm -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the BookwormPRO CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The BookwormPRO CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
```
 [BWM] claude-sonnet-4-20250514 │ 12.4K/200K │ [██████░░░░] 6% │ $0.06 │ 15m
```
...
When resuming a previous session (`bookworm -c` or `bookworm --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend BookwormPRO to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. BookwormPRO supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for BookwormPRO replies |
...
```bash
bookworm -s bookwormpro-dev,github-auth
bookworm chat -s github-pr-workflow -s github-auth
```

BookwormPRO loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.bookwormpro/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.bookwormpro/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain BookwormPRO..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend BookwormPRO to the background — just like any terminal process. The shell prints a confirmation:

```
BookwormPRO has been suspended. Run `fg` to bring BookwormPRO back.
```
...
Resume this session with:
  bookworm --resume 20260225_143052_a1b2c3
...
```bash
bookworm --continue                          # Resume the most recent CLI session
bookworm -c                                  # Short form
bookworm -c "my project"                     # Resume a named session (latest in lineage)
bookworm --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
bookworm --resume "refactoring auth"         # Resume by title
bookworm -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `bookworm sessions rename <id> <title>` from the command line. Use `bookworm sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in BookwormPRO's SQLite state database under `~/.bookwormpro/state.db`. The database keeps:
...
BookwormPRO immediately confirms the task and gives you back the prompt:

```
[调用] Background task #1 started: "Analyze the logs in /var/log and summarize..."

...
title: "Git Worktrees"
description: "Run multiple BookwormPRO agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

BookwormPRO is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with BookwormPRO so each session has a clean, isolated working directory.

## Why Use Worktrees with BookwormPRO?

BookwormPRO treats the **current working directory** as the project root:

- CLI: the directory where you run `bookworm` or `bookworm chat`
...
git worktree add ../repo-feature feature/bookworm-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/bookworm-experiment` checked out in that directory

Now you can `cd` into the new worktree and run BookwormPRO there:

```bash
cd ../repo-feature

# Start BookwormPRO in the worktree
bookworm
```
...
git worktree add ../repo-experiment-a feature/bookworm-a
git worktree add ../repo-experiment-b feature/bookworm-b
```
...
Each BookwormPRO process:

- Works on its own branch (`feature/bookworm-a` vs `feature/bookworm-b`).
...
- BookwormPRO checkpoint data under `~/.bookwormpro/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per BookwormPRO experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/bookworm-checkpoints-docs`, `feature/bookworm-refactor-tests`.
- **Commit frequently**
...
- **Avoid running BookwormPRO from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `bookworm -w` (Automatic Worktree Mode)

BookwormPRO has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `bookworm/bookworm-<hash>`).
...
```bash
bookworm -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `bookworm -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each BookwormPRO session its own clean checkout.

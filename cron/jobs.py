...
# 连续失败熔断阈值：一个 cron job 连续失败达到此次数后自动暂停（state=circuit_open），
# 防止无效任务无限重试死循环。单个 job 可用 failure_limit 字段覆盖；设为 0 关闭熔断。
CRON_FAILURE_LIMIT = 5
    ...
    with _jobs_file_lock:
        ...
        for i, job in enumerate(jobs):
            if job["id"] == job_id:
                ...
                # ── 连续失败熔断（借鉴 kanban circuit breaker）──
                # 成功清零，失败累加；达阈值自动暂停 job 防止无效重试死循环。
                if success:
                    job["consecutive_failures"] = 0
                else:
                    job["consecutive_failures"] = job.get("consecutive_failures", 0) + 1
                    _cf_limit = job.get("failure_limit", CRON_FAILURE_LIMIT)
                    if _cf_limit and job["consecutive_failures"] >= _cf_limit:
                        job["enabled"] = False
                        job["state"] = "circuit_open"
                        logger.warning(
                            "cron job %s 连续失败 %d 次，触发熔断（已自动暂停）",
                ...
                if job["next_run_at"] is None:
                    job["enabled"] = False
                    job["state"] = "completed"
                elif job.get("state") not in ("paused", "circuit_open"):
                    job["state"] = "scheduled"

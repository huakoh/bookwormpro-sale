#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for BookwormPRO.
...
TOKEN_PATH = BOOKWORMPRO_HOME / "google_token.json"
CLIENT_SECRET_PATH = BOOKWORMPRO_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = BOOKWORMPRO_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same BookwormPRO profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'bookwormpro[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to BookwormPRO home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for BookwormPRO")

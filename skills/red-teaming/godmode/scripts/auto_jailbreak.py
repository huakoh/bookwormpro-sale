...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("BOOKWORMPRO_HOME", os.path.expanduser("~/.bookwormpro")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("BOOKWORMPRO_HOME", Path.home() / ".bookwormpro")) / "skills" / "red-teaming" / "godmode"
...
BOOKWORMPRO_HOME = Path(os.getenv("BOOKWORMPRO_HOME", Path.home() / ".bookwormpro"))
CONFIG_PATH = BOOKWORMPRO_HOME / "config.yaml"
PREFILL_PATH = BOOKWORMPRO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # BookwormPRO/BookwormPRO models — already uncensored, just needs clean prompt
    "bookworm": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "bookworm" in model_lower or "bookwormpro" in model_lower:
        return "bookworm"
...
def _get_current_model() -> tuple:
    """Read current model and provider from BookwormPRO config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.bookwormpro/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart BookwormPRO for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart BookwormPRO for changes to take effect.")

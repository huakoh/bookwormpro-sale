...
## 5. GODMODE FAST — BookwormPRO 4 405B (Zero Refusal)

**Model:** `nousresearch/bookworm-4-405b`
**Strategy:** BookwormPRO models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with BookwormPRO

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.bookwormpro/config.yaml`:

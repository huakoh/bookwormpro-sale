...
**HermesAgentBaseEnv** (`hermes_base_env.py`) extends BaseEnv with bookwormpro specifics:
...
`HermesAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as bookwormpro's `run_agent.py`:

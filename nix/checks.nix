...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      bookwormpro = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "bookworm-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "bookworm-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "bookworm-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${bookwormpro}/bin/bookworm || (echo "FAIL: bookworm binary missing"; exit 1)
          test -x ${bookwormpro}/bin/bookwormpro || (echo "FAIL: bookwormpro binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${bookwormpro}/bin/bookworm version 2>&1 | grep -qi "bookworm" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "bookworm-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in bookworm bookwormpro bookworm-acp; do
            test -x ${bookwormpro}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "bookworm-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking bookworm --help ==="
          ${bookwormpro}/bin/bookworm --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${bookwormpro}/bin/bookworm --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "bookworm-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${bookwormpro}/share/bookwormpro/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${bookwormpro}/share/bookwormpro/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "BOOKWORMPRO_BUNDLED_SKILLS" ${bookwormpro}/bin/bookworm || \
            (echo "FAIL: BOOKWORMPRO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: BOOKWORMPRO_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "bookworm-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${bookwormpro}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${bookwormpro}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          test -d ${bookwormpro}/ui-tui/node_modules || (echo "FAIL: node_modules missing"; exit 1)
          echo "PASS: node_modules present"

          grep -q "BOOKWORMPRO_TUI_DIR" ${bookwormpro}/bin/bookworm || \
            (echo "FAIL: BOOKWORMPRO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: BOOKWORMPRO_TUI_DIR set in wrapper"
        ...
        # Verify BOOKWORMPRO_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        bookworm-node = pkgs.runCommand "bookworm-node-version" { } ''
          set -e
          echo "=== Checking BOOKWORMPRO_NODE in wrapper ==="
          grep -q "BOOKWORMPRO_NODE" ${bookwormpro}/bin/bookworm || \
            (echo "FAIL: BOOKWORMPRO_NODE not set in wrapper"; exit 1)
          echo "PASS: BOOKWORMPRO_NODE present in wrapper"

          BOOKWORMPRO_NODE=$(sed -n "s/^export BOOKWORMPRO_NODE='\(.*\)'/\1/p" ${bookwormpro}/bin/bookworm)
          test -x "$BOOKWORMPRO_NODE" || (echo "FAIL: BOOKWORMPRO_NODE=$BOOKWORMPRO_NODE not executable"; exit 1)
          echo "PASS: BOOKWORMPRO_NODE executable at $BOOKWORMPRO_NODE"

          NODE_MAJOR=$("$BOOKWORMPRO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All BOOKWORMPRO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify BOOKWORMPRO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "bookworm-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(BOOKWORMPRO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking BOOKWORMPRO_MANAGED guards ==="
          check_blocked "config set" ${bookwormpro}/bin/bookworm config set model foo
          check_blocked "config edit" ${bookwormpro}/bin/bookworm config edit
        ...
        in pkgs.runCommand "bookworm-config-roundtrip" {
          ...
          merge_and_load() {
            local hermes_home="$1"
            export BOOKWORMPRO_HOME="$hermes_home"

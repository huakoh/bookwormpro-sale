# nix/nixosModules.nix — NixOS module for bookwormpro
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: bookworm runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $BOOKWORMPRO_HOME/.env
# and read by bookworm at startup — no container recreation needed for env changes.
#
# Tool resolution: the bookworm wrapper uses --suffix PATH for nix store tools,
...
#     environmentFiles = [ config.sops.secrets."bookwormpro/cli".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.bookwormpro;
    bookwormpro = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-bookwormpro)
    deepConfigType = lib.types.mkOptionType {
      name = "bookworm-config-attrs";
      description = "BookwormPRO YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "bookworm-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "bookworm-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'BOOKWORMPRO_DOC_EOF'\n${value}\nHERMES_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "bookwormpro";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/bookworm";
    ...
    # bookworm user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "bookworm-container-entrypoint" ''
      set -eu

      BOOKWORMPRO_UID="''${BOOKWORMPRO_UID:?BOOKWORMPRO_UID must be set}"
      BOOKWORMPRO_GID="''${BOOKWORMPRO_GID:?BOOKWORMPRO_GID must be set}"

      # ── Group: ensure a group with GID=$BOOKWORMPRO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$BOOKWORMPRO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="bookworm"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$BOOKWORMPRO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$BOOKWORMPRO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$BOOKWORMPRO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$BOOKWORMPRO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="bookworm"
        TARGET_HOME="/home/bookworm"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$BOOKWORMPRO_UID" -g "$BOOKWORMPRO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$BOOKWORMPRO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$BOOKWORMPRO_UID:$BOOKWORMPRO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure BOOKWORMPRO_HOME is owned by the target user
      if [ -n "''${BOOKWORMPRO_HOME:-}" ] && [ -d "$BOOKWORMPRO_HOME" ]; then
        chown -R "$BOOKWORMPRO_UID:$BOOKWORMPRO_GID" "$BOOKWORMPRO_HOME"
      fi
      ...
      if [ ! -f /var/lib/bookworm-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/bookworm-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/bookworm ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/bookworm
        chmod 0440 /etc/sudoers.d/bookworm
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$BOOKWORMPRO_UID" --regid="$BOOKWORMPRO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $BOOKWORMPRO_HOME/.env.
    ...
    # Default: /var/lib/bookworm/workspace → /data/workspace.
  ...
  in {
    options.services.bookwormpro = with lib; {
      enable = mkEnableOption "BookwormPRO gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = bookwormpro;
        description = "The bookwormpro package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/bookworm";
        description = "State directory. Contains .bookwormpro/ subdir (BOOKWORMPRO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative BookwormPRO config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $BOOKWORMPRO_HOME/.env at activation time.
          BookwormPRO reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $BOOKWORMPRO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $BOOKWORMPRO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `bookworm gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the bookworm CLI to environment.systemPackages and export
          BOOKWORMPRO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.bookwormpro symlink to the service
            stateDir. These users are automatically added to the bookworm group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.bookwormpro.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the bookworm CLI to system PATH and export BOOKWORMPRO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.bookwormpro/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.BOOKWORMPRO_HOME = "${cfg.stateDir}/.bookwormpro";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.bookwormpro: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed bookworm
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure bookworm is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.bookwormpro        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bookwormpro/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bookwormpro/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bookwormpro/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.bookwormpro/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."bookwormpro-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.bookwormpro
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.bookwormpro ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.bookwormpro ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.bookwormpro -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.bookwormpro/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.bookwormpro/$_subdir"
            chmod 2770 "${cfg.stateDir}/.bookwormpro/$_subdir"
            find "${cfg.stateDir}/.bookwormpro/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.bookwormpro/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.bookwormpro/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.bookwormpro/config.yaml
            chmod 0640 ${cfg.stateDir}/.bookwormpro/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.bookwormpro/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.bookwormpro/.managed
          chmod 0644 ${cfg.stateDir}/.bookwormpro/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.bookwormpro/.container-mode <<'BOOKWORMPRO_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
hermes_bin=${containerDataDir}/current-package/bin/bookworm
BOOKWORMPRO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.bookwormpro/.container-mode
            chmod 0644 ${cfg.stateDir}/.bookwormpro/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.bookwormpro/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.bookwormpro";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.bookwormpro" ]; then
                  rm -f "${symlinkPath}"
                  echo "bookwormpro: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.bookwormpro -> stateDir/.bookwormpro for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.bookwormpro";
                target = "${cfg.stateDir}/.bookwormpro";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "bookwormpro: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.bookwormpro/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.bookwormpro/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.bookwormpro/auth.json
              fi
            ''}
          ''}
          ...
          # BookwormPRO reads $BOOKWORMPRO_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.bookwormpro/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'BOOKWORMPRO_NIX_ENV_EOF'
${envFileContent}
BOOKWORMPRO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.bookwormpro = {
          description = "BookwormPRO Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            BOOKWORMPRO_HOME = "${cfg.stateDir}/.bookwormpro";
            BOOKWORMPRO_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $BOOKWORMPRO_HOME/.env by the activation script. load_hermes_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/bookworm"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the bookworm group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.bookwormpro = {
          description = "BookwormPRO Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              BOOKWORMPRO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              BOOKWORMPRO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env BOOKWORMPRO_UID="$BOOKWORMPRO_UID" \
                --env BOOKWORMPRO_GID="$BOOKWORMPRO_GID" \
                --env BOOKWORMPRO_HOME=${containerDataDir}/.bookwormpro \
                --env BOOKWORMPRO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/bookworm gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

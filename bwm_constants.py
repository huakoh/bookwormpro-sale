"""Shared constants for BookwormPRO.
...
def get_default_hermes_root() -> Path:
    """Return the root BookwormPRO directory for profile-level operations.

    In standard deployments this is ``~/.bookwormpro``.

    In Docker or custom deployments where ``BOOKWORMPRO_HOME`` points outside
    ``~/.bookwormpro`` (e.g. ``/opt/data``), returns ``BOOKWORMPRO_HOME`` directly
    — that IS the root.

    In profile mode where ``BOOKWORMPRO_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.bookwormpro/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = Path.home() / ".bookwormpro"
    env_home = os.environ.get("BOOKWORMPRO_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # BOOKWORMPRO_HOME is under ~/.bookwormpro (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — BOOKWORMPRO_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``BOOKWORMPRO_OPTIONAL_SKILLS``.
    """
    override = os.getenv("BOOKWORMPRO_OPTIONAL_SKILLS", "").strip()
    if override:
...
def get_hermes_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a BookwormPRO subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to BOOKWORMPRO_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to BOOKWORMPRO_HOME (e.g. ``"image_cache"``).
...
def display_hermes_home() -> str:
    """Return a user-friendly display string for the current BOOKWORMPRO_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.bookwormpro``
        profile:  ``~/.bookwormpro/profiles/coder``
        custom:   ``/opt/bookworm-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.bookwormpro``.  For code that needs a real ``Path``, use
...
def get_subprocess_home() -> str | None:
    ...
    When ``{BOOKWORMPRO_HOME}/home/`` exists on disk, subprocesses should use it
    as ``HOME`` so system tools (git, ssh, gh, npm …) write their configs
    inside the BookwormPRO data directory instead of the OS-level ``/root`` or
    ``~/``.  This provides:
    ...
    **never** modified — only subprocess environments should inject this value.
    ...
    """
    hermes_home = os.getenv("BOOKWORMPRO_HOME")
    if not hermes_home:
        return None
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under BOOKWORMPRO_HOME.

    Replaces the ``get_hermes_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, bwm_logging.py, bwm_time.py, etc.).
    """
    return get_hermes_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under BOOKWORMPRO_HOME."""

...
``_check_spotify_available()`` — when the user has not run ``bookworm auth
spotify``, the tools remain registered (so they appear in ``bookworm
...
The Spotify auth flow (``bookworm auth spotify``), CLI plumbing, and docs

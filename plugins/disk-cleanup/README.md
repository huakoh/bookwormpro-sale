...
Auto-tracks and cleans up ephemeral files created during BookwormPRO
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$BOOKWORMPRO_HOME` and `/tmp/bookworm-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `BOOKWORMPRO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `BOOKWORMPRO_HOME` or `/tmp/bookworm-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$BOOKWORMPRO_HOME/disk-cleanup/` is itself excluded
- `$BOOKWORMPRO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

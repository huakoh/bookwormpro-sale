"""disk_cleanup — ephemeral file cleanup for BookwormPRO.
...
Rules:
  ...
  - empty dirs    → always delete (under BOOKWORMPRO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly BOOKWORMPRO_HOME and /tmp/bookworm-*
Never touches: ~/.bookwormpro/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("BOOKWORMPRO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".bookwormpro").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$BOOKWORMPRO_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$BOOKWORMPRO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under BOOKWORMPRO_HOME or ``/tmp/bookworm-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/bookworm-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("bookworm-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside BOOKWORMPRO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under BOOKWORMPRO_HOME (but leave BOOKWORMPRO_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "bookwormpro", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under BOOKWORMPRO_HOME (e.g. /tmp/bookworm-*) — fall through.
        pass

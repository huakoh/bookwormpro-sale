...
Resolution order for config file:
  1. $BOOKWORMPRO_HOME/honcho.json  (instance-local, enables isolated BookwormPRO instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active BookwormPRO profile.

    Resolution order:
      1. BOOKWORMPRO_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``bookworm.<profile>``
      3. Fallback: ``"bookworm"`` (default profile)
    """
    explicit = os.environ.get("BOOKWORMPRO_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $BOOKWORMPRO_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.bookwormpro/honcho.json          (default profile — shared host blocks live here)
    ...
    default_path = Path.home() / ".bookwormpro" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into BookwormPRO system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.bookwormpro
    ...
    def from_global_config(
        ...
        Resolution: $BOOKWORMPRO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active BookwormPRO profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.bookwormpro block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. BookwormPRO session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — BookwormPRO session_id ({timestamp}_{hex})
        ...
        # per-session: inherit BookwormPRO session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'bookworm honcho setup' or set HONCHO_API_KEY. "

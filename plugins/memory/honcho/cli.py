...
Handles: bookworm honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "bookworm" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all BookwormPRO profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'bookworm honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'bookworm honcho setup' first.\n")
        return
    ...
    for p in profiles:
        ...
        if clone_honcho_for_profile(p.name):
            print(f"  + {p.name} -> bookworm.{p.name}")
            created += 1
        else:
            skipped += 1
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `bookworm update` -- no output, no exceptions.
    """
    try:
        from bwm_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current BookwormPRO profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $BOOKWORMPRO_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives BookwormPRO persistent cross-session memory.")
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'bookworm honcho setup' again once you have a key.\n")
            return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "bookworm")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "bookworm")
    ...
    try:
        ...
        print("  Run: bookworm config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    bookworm honcho status     -- show full config")
    print("    bookworm honcho mode       -- change recall/observation mode")
    print("    bookworm honcho tokens     -- tune context and dialectic budgets")
    print("    bookworm honcho peer       -- update peer names")
    print("    bookworm honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active BookwormPRO profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: bookworm honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'bookworm honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: bookworm honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        bookworm = hosts.get(_host_key(), {})
        user = bookworm.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = bookworm.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = bookworm.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = bookworm.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    BookwormPRO' identity in Honcho. Seed with 'bookworm honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: bookworm honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: bookworm honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = bookworm.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = bookworm.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = bookworm.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. BookwormPRO asks Honcho's AI peer a question")
        ...
        print("\n  Set with: bookworm honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'bookworm honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'bookworm honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    bookworm honcho identity --show        — show both peer representations")
        print("    bookworm honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → BookwormPRO + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → BookwormPRO\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives BookwormPRO persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  bookworm honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'bookworm honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'bookworm honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  bookworm honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'bookworm'")
        print("  with Honcho configured and no prior session history.")
        print("  (BookwormPRO calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    bookworm honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'bookworm honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In BookwormPRO, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'bookworm honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    bookworm honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  bookworm honcho identity <path/to/SOUL.md>")
    ...
    print("    BookwormPRO:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    BookwormPRO:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    BookwormPRO:   Honcho observes every message and updates representations")
    ...
    print("    BookwormPRO:   per-session by default — each run gets its own session")
    print("              Map a custom name:  bookworm honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. bookworm honcho setup              — configure API key (required)")
        print("  2. bookworm honcho migrate            — re-run this walkthrough")
    else:
        print("  1. bookworm honcho status             — verify Honcho connection")
        print("  2. bookworm                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. bookworm honcho identity --show    — verify AI peer representation")
        print("  4. bookworm honcho tokens             — tune context and dialectic budgets")
        print("  5. bookworm honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'bookworm memory setup'...\n")
        from bwm_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``bookworm honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``bookworm honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to bookworm memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to BookwormPRO Honcho",
    )

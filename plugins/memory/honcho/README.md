...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/bookworm>
...
```bash
bookworm honcho setup    # full interactive wizard (cloud or local)
bookworm memory setup    # generic picker, also works
```

Or manually:
```bash
bookworm config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.bookwormpro/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$BOOKWORMPRO_HOME/honcho.json` | Profile-local (isolated BookwormPRO instances) |
| 2 | `~/.bookwormpro/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active BookwormPRO profile: `bookworm` (default) or `bookworm.<profile>`.
...
| 4 | `per-session` strategy | BookwormPRO session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`bookwormpro`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`bookworm`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-bookwormpro`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening bookworm in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — BookwormPRO session ID (timestamp + hex). Every `bookworm` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple BookwormPRO profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`bookworm`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active BookwormPRO profile: `bookworm` (default) or `bookworm.<profile>` (e.g. `bookworm -p coder` → host key `bookworm.coder`).
...
| `BOOKWORMPRO_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `bookworm honcho setup` | Full interactive setup wizard |
| `bookworm honcho status` | Show resolved config for active profile |
| `bookworm honcho enable` / `disable` | Toggle Honcho for active profile |
| `bookworm honcho mode <mode>` | Change recall or observation mode |
| `bookworm honcho peer --user <name>` | Update user peer name |
| `bookworm honcho peer --ai <name>` | Update AI peer name |
| `bookworm honcho tokens --context <N>` | Set context token budget |
| `bookworm honcho tokens --dialectic <N>` | Set dialectic max chars |
| `bookworm honcho map <name>` | Map current directory to a session name |
| `bookworm honcho sync` | Create host blocks for all BookwormPRO profiles |

...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with bookwormpro)
2. User-installed providers: ``$BOOKWORMPRO_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$BOOKWORMPRO_HOME/plugins/`` or None if unavailable."""
    try:
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$BOOKWORMPRO_HOME/plugins/<name>/``) directories.  Bundled takes

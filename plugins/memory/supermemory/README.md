...
Config file: `$BOOKWORMPRO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `bookworm` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `bookworm-{identity}` → `bookworm-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per BookwormPRO profile:

```json
{
  "container_tag": "bookworm-{identity}"
}
```

For a profile named `coder`, this resolves to `bookworm-coder`. The default profile resolves to `bookworm-default`. Without `{identity}`, all profiles share the same container.

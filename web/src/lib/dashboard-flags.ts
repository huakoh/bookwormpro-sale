declare global {
  interface Window {
    /** Set true by the server only for `bookworm dashboard --tui` (or BOOKWORMPRO_DASHBOARD_TUI=1). */
...
/** True only when the dashboard was started with embedded TUI Chat (`bookworm dashboard --tui`). */

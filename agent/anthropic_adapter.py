"""Anthropic Messages API adapter for BookwormPRO.

Translates between BookwormPRO's internal OpenAI-style message format and
...
# BookwormPRO effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def read_claude_code_credentials() -> Optional[Dict[str, Any]]:
    ...
    Returns dict with {accessToken, refreshToken?, expiresAt?} or None.
    """
    # BookwormPRO opt-out: when BOOKWORMPRO_IGNORE_CLAUDE_CREDENTIALS=1, skip Claude Code
    # credential lookup entirely so users running both BookwormPRO (with custom relay)
    # and Claude Code (with OAuth) can keep them isolated.
    if os.getenv("BOOKWORMPRO_IGNORE_CLAUDE_CREDENTIALS", "").strip() in ("1", "true", "yes"):
        return None
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    BookwormPRO historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by BookwormPRO)
    ...
    # 1. BookwormPRO-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration BookwormPRO configs.
...
# ── BookwormPRO-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.bookwormpro/.anthropic_oauth.json (our own file).
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run BookwormPRO-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read BookwormPRO-managed OAuth credentials from ~/.bookwormpro/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read BookwormPRO OAuth credentials: %s", e)
    return None
...
def convert_messages_to_anthropic(
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See bookwormpro#13848.
            #
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("BookwormPRO", "Claude Code")
                text = text.replace("BookwormPRO agent", "Claude Code")
                text = text.replace("bookwormpro", "claude-code")
                text = text.replace("BookwormPRO Project", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that BookwormPRO surfaces in its CLI. We

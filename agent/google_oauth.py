...
Storage (``~/.bookwormpro/auth/google_oauth.json``, chmod 0o600):
...
#
# Resolution order:
#   1. BOOKWORMPRO_GEMINI_CLIENT_ID / BOOKWORMPRO_GEMINI_CLIENT_SECRET env vars (power users)
...
_HEADLESS_ENV_VARS = ("SSH_CONNECTION", "SSH_CLIENT", "SSH_TTY", "BOOKWORMPRO_HEADLESS")
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            "BookwormPRO looks for a locally installed gemini-cli to source the OAuth client. "
            "Either:\n"
            "  1. Install it: npm install -g @google/gemini-cli  (or brew install gemini-cli)\n"
            "  2. Set BOOKWORMPRO_GEMINI_CLIENT_ID and BOOKWORMPRO_GEMINI_CLIENT_SECRET in ~/.bookwormpro/.env\n"
            "\n"
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    if creds is None:
        raise GoogleOAuthError(
            "No Google OAuth credentials found. Run `bookworm login --provider google-gemini-cli` first.",
...
<html><head><meta charset="utf-8"><title>BookwormPRO — signed in</title>
<style>
...
<html><head><meta charset="utf-8"><title>BookwormPRO — sign-in failed</title>
<style>
body {{ font: 16px/1.5 system-ui, sans-serif; margin: 10vh auto; max-width: 32rem; text-align: center; color: #222; }}
h1 {{ color: #b42318; }} p {{ color: #555; }}
</style></head>
<body><h1>Sign-in failed</h1><p>{message}</p>
<p>Return to your terminal — BookwormPRO will walk you through a manual paste fallback.</p></body></html>
"""

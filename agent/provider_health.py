...
# HTTP request timeout for the health probe.  Kept generous because slow
# aggregating relays behind a proxy (e.g. bww.letcareme.com) legitimately take
# 6-9s just to complete the TLS handshake + /models response; a 5s cap turned
# a healthy-but-slow provider into repeated handshake timeouts → 3 consecutive
# failures → DEAD → circuit trip → silent fallback to deepseek on every call.
# The probe runs at most once per _PROBE_COOLDOWN and is off the hot path, so a
# larger ceiling costs nothing for fast providers (they still answer in 1-4s).
_PROBE_TIMEOUT = 12.0          # HTTP request timeout

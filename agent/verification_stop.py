...
def _session_is_messaging_surface() -> bool:
    ...
    ``BOOKWORMPRO_SESSION_PLATFORM``; the CLI and TUI set ``BOOKWORMPRO_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``BOOKWORMPRO_PLATFORM``
    ...
    try:
        ...
        platform = (
            os.getenv("BOOKWORMPRO_PLATFORM")
            or get_session_env("BOOKWORMPRO_SESSION_PLATFORM", "")
        )
        source = get_session_env("BOOKWORMPRO_SESSION_SOURCE", "")
    except Exception:
        platform = os.getenv("BOOKWORMPRO_PLATFORM", "") or os.environ.get(
            "BOOKWORMPRO_SESSION_PLATFORM", ""
        )
        source = os.environ.get("BOOKWORMPRO_SESSION_SOURCE", "")
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``BOOKWORMPRO_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("BOOKWORMPRO_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `bookwormpro-verify-` filename prefix, run it "

...
from bwm_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``BOOKWORMPRO_PLATFORM`` or
            ``BOOKWORMPRO_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("BOOKWORMPRO_PLATFORM")
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.bookwormpro/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.bookwormpro/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    bookworm = metadata.get("bookworm") or {}
    if not isinstance(bookworm, dict):
        bookworm = {}
    return {
        "fallback_for_toolsets": bookworm.get("fallback_for_toolsets", []),
        "requires_toolsets": bookworm.get("requires_toolsets", []),
        "fallback_for_tools": bookworm.get("fallback_for_tools", []),
        "requires_tools": bookworm.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    bookworm = metadata.get("bookworm")
    if not isinstance(bookworm, dict):
        return []
    raw = bookworm.get("config")
    if not raw:
        return []

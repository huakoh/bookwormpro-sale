...
## Client ID
- Need to register a "Desktop app" OAuth client on a BookwormPRO Project GCP project
...
## Token Lifecycle
- Store at `~/.bookwormpro/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `bwm_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `bwm_cli/models.py` — Add Gemini model catalog
4. `bwm_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `bwm_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `bwm_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- BookwormPRO Project GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via BOOKWORMPRO_GEMINI_CLIENT_ID env var
...
- bookwormpro Copilot OAuth: `bwm_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
